...
await pg.evaluate(()=>{window.__apex.go();window.__apex.camera("chase");window.__apex.jump(0.05,55,0);});
await pg.waitForTimeout(2000);
console.log(await pg.evaluate(()=>{
  const cs=window.__apex.camState();
  const ws=window.__apex.wsInfo();
  return { camEye: cs.eye.map(v=>+v.toFixed(1)), camTgt: (cs.tgt||cs.target).map(v=>+v.toFixed(1)),
           carWorld: ws.pos, head: ws.head, debugCam: cs.debug };
}));
