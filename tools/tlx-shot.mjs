#!/usr/bin/env node
// tlx-shot.mjs — Three.js/TSL backend screenshots (WebGL2 pin on software GPUs).
//
// Without tlxForceGL, Chromium desktop auto-picks three's WebGPU backend, which
// paints black on SwiftShader/headless — tlx.js now auto-pins WebGL2 there;
// this tool still sets tlxForceGL=1 so CI repros match tests/specs/tlx-probes.
//
// Usage:
//   node tools/tlx-shot.mjs [trackId] [--lite] [--out DIR] [--cam orbit|eye|park]
//   node tools/tlx-shot.mjs montreal --webgpu   # force three WebGPU (software = black)
...
const outDir = outArg >= 0 ? args[outArg + 1] : join("artifacts", "tmp", "tlx-shots", track);
...
try {
  ...
  await page.addInitScript(({ wantLite, wantWebgpu }) => {
    localStorage.setItem("apex26.gfxBackend", "three");
    if (wantWebgpu) localStorage.setItem("apex26.tlxForceGL", "0");
    else localStorage.setItem("apex26.tlxForceGL", "1");
    if (wantLite) localStorage.setItem("apex26.gfxWgxLite", "1");
  }, { wantLite: lite, wantWebgpu: webgpu });
  ...
  const payload = await page.evaluate(() => {
    ...
    return {
      ...
      hdr: typeof GLX !== "undefined" && GLX.hdrMode ? GLX.hdrMode() : null,
      tlx: GLX.__tlx && GLX.__tlx.backendState ? GLX.__tlx.backendState() : null,
  ...
  writeFileSync(join(outDir, "state.json"), JSON.stringify({
    track, lite, cam, webgpu, backend: payload.backend, hdr: payload.hdr,
    tlx: payload.tlx, coveragePct: payload.coveragePct, light: payload.light,
    files: ["canvas.png", "view-raster.png", "page-hud.png", "view.txt", "state.json"],
  }, null, 2));

  const ok = payload.backend === "three";
  if (!ok) exitCode = 1;
  console.log(JSON.stringify({ ok, outDir, track, lite, cam, webgpu, ...payload }, null, 2));
} catch (e) {
  console.error("tlx-shot failed:", (e && e.message) || e);
  exitCode = 1;
} finally {
  await shutdown();
}
