 ...
 *
 * The localhost half of tools/mcp/apex-report.js. Two things it buys over
 * `npx serve`:
 *
 ...
 *
 * Serving the tree also means tools/mcp/apex-report.js is a same-origin path, so
 * the console line is `fetch("/tools/mcp/apex-report.js")` with no GitHub in it.
...
function publicPath(local) {
  return PUBLIC_FILES.has(local) || PUBLIC_DIRS.some((prefix) => local.startsWith(prefix))
    || local === "tools/mcp/apex-report.js";
}
...
(function () {
  ...
  b.addEventListener("click", function () {
    ...
    fetch("/tools/mcp/apex-report.js").then(function (r) { return r.text(); })
