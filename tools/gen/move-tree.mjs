...
//
//   node tools/gen/move-tree.mjs <moves.json> --plan     # what would move, what would be rewritten
//   node tools/gen/move-tree.mjs <moves.json>            # do it (git mv + sweep + MOVED + gen-shell)
//   node tools/gen/move-tree.mjs <moves.json> --root D   # operate on another checkout (tests)
//
...
export const SWEEP_ROOTS = ["js", "tools", "tests", "docs", "types", "assets", ".claude", ".cursor", ".github",
...
// `assets/` is NOT skipped: it was, for its binaries, but SWEEP_EXT already
// filters those out and the four text files it does hold (pack/manifest.json,
// pack/webbake.js, pack/CREDITS.md, atlases/README.md) cite tools by path.
// Phase 4's tools move relocated the asset baker and the model synthesiser
// and left all four pointing at their old paths — the same shape of miss as
// .mcp.json above. (Named by role, not by path: a comment that spells a
// pre-move path out is itself an exact token the next sweep rewrites, which
// silently turns the war story into a false one.) Found 2026-09-03.
// Skip a tree for being another checkout or a data record, never
// for being mostly binary; the extension filter is what handles binary.
export const SWEEP_SKIP = /^(docs\/archive|node_modules|artifacts|scratch|vendor|\.claude\/worktrees|\.git|tools\/moves)(\/|$)/;
...
export function apply(root, moves, { plan = false, git = fs.existsSync(path.join(root, ".git")), genShell = true } = {}) {
  ...
  if (!plan) {
    ...
    if (genShell && fs.existsSync(path.join(root, "tools/gen/gen-shell.mjs"))) {
      const r = spawnSync(process.execPath, ["tools/gen/gen-shell.mjs"], { cwd: root, encoding: "utf8" });
...
function main() {
  ...
  if (!file) { console.error("usage: node tools/gen/move-tree.mjs <moves.json> [--plan] [--root DIR]"); process.exit(2); }
