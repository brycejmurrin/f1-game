...
//
// Usage: node tools/track/survey-track.mjs <id> [label] [fracs] [--oblique]
...
try {
  ...
  if (!ok) {
    console.error(`FAILED to load ${id} — scenery(api) may throw. Run: node tools/track/verify-track.cjs ${id}`);
    await shutdown();
    process.exit(1);
  }
  await sleep(1400);
