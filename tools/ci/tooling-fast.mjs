...
// NOT "tooling-fast.log": that is the path tools/ci/test-bg.mjs gives the
...
export const TOOLING_FAST_FILES = Object.freeze([
  ...
  // ~5 s, and here for the reason the car-mesh note above gives: the browser
  // spec holding this invariant (agent-determinism) is not gated at all, so its
  // last break reached the deploy tip and went red on somebody elses push. Four
  // leaks of one class so far (drivetrain, _prevS, accSm, lane), every one a
  // per-car field a re-grid did not restore. This catches the fifth in the edit
  // loop, where it costs seconds instead of a browser group.
  "tests/unit/replay-determinism-vm.test.mjs",
