...
//   node tools/ratchets.mjs --json     # {ok, rows:[{file, metric, value, ceiling, over, slack}]}
//
...
//   gMembers   members of the `G` façade literal (tools/check-gctx.mjs scanGameCtx)
...
async function main() {
  ...
  for (const r of v.loose) console.log(`LOOSE  ${r.file} ${r.metric}: ${r.value} but ceiling ${r.ceiling} (slack ${r.slack} > ${r.slackMax}) — lower it: node tools/ratchets.mjs --update`);
