...
| **cross-file-paths.mjs** | *Every relative reference between files resolves to a file that exists.* Built for the `tests/` split, whose risk is a MISSED path rewrite rather than a wrong one. Most misses announce themselves; the ones that don't are the point — `fit-audit.mjs` and `menu-fit.mjs` import `../tests/f1-api-mock.js` inside `try { … } catch { /* degrades */ }`, so after a move the import fails forever, both tools quietly audit an empty data hub, and nothing goes red. Covers static imports, dynamic `import()`, `require()` and `new URL(rel, import.meta.url)` across `tests/` and `tools/`. Parsed with espree, not grepped: several test files build fixtures out of source text containing import statements, and a guard with false positives is a guard that gets switched off. |
| **ci-coverage.mjs** | *What does the deploy gate actually execute?* Reads `ci.yml` and resolves every `npm run test:*` and by-path invocation against the specs on disk, so it cannot drift from the workflow the way a comment does. Today: **2 of 110 Playwright specs**; 108 are gated by nothing before deploy. That is a deliberate choice — the full suite is ~40 minutes of SwiftShader — but it was previously visible only by reading four job definitions and expanding every npm script by hand, which is how `physics-characterization` sat in NO job through 175 commits of physics work. **A report, never a gate**: the obvious version (run `test-observed.mjs` in CI, fail on the never-observed count) is broken by construction, since `artifacts/` is gitignored and every title is legitimately unobserved there — the same environment-fact trap that reddened the guards job once already. `tests/ci-coverage.test.mjs` guards the tool and pins the MECHANISM, never the number. |
| **select-budget.mjs** | *Can a change-aware CI job actually run what it selects?* TEST-AUDIT's CI design is blocked on gaps 7/8/13 and asks for exactly this: the budget re-derived against measured per-spec counts. Measured on CI run 31197770813 — smoke, 9 declared tests in 11m57s at one worker, so **79.7 s per test**. The term the design omitted is the cost of a FAILURE: `retries: 1` with `--timeout=240000` makes one timing-out test cost **480 s**, so a 15-minute budget holds 11 tests all-passing but only **6** if it must survive a single timeout — the step dies exactly when the selection finds something. Median spec is 6 declared tests and the largest is 117, which settles per-spec over per-group by arithmetic rather than by preference. Also models the lever: dropping the retry buys almost as much as doubling the budget, because a selector is advisory and a retry only doubles the cost of the news it exists to deliver. `tests/select-budget.test.mjs` guards the MODEL, not the constants — and caught a real defect in this tool on its first run (a missing file threw ENOENT instead of returning null). |
