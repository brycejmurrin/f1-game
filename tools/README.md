...
| **cdmcp-measure.py** | Background-friendly Chromium MCP measure suite — boot (network/console/LCP/lighthouse), ui floors, full(+heap). Logs `artifacts/logs/cdmcp-measure.log` with `= run passed|failed` terminal line + JSON sidecar. `--bg` detaches. | mcp-probe |
| **cdmcp-lamps.py** | Night lamp screenshot suite via chrome-devtools MCP + localhost — Qatar dens 1/2, Singapore, Bahrain, Monza dens 1/2. Logs `artifacts/logs/cdmcp-lamps.log` with `= run passed|failed` terminal line; PNGs under `scratch/captures/cdmcp-lamps/`. `--bg` / `--status`. Needs `cdmcp-cli` MCP `roots` for `filePath` writes. | mcp-probe |
...
| **ui-mcp-session.py** | Multi-step MCP session: settings → garage → career at phone-landscape 115%, with Escape. Writes `scratch/ui-mcp-session.json`. Navigates `?v=` from `version.json`. | mcp-probe / ui-menu-a11y |
| **ui-fix-validate-mcp.py** | Post-fix gate via chrome-devtools MCP — MORE door tap floors, `#cs-unlimited` chip height, settings body scroll-or-RESOLUTION, career guide reachability. Exit 0 only when every gate passes. | mcp-probe / ui-menu-a11y |
...
| **assets.mjs** | Asset bake CLI (AUTHOR-TIME ONLY — never loaded by the game). `bake-synthetic` rebuilds material layers with no network; `bake-synthetic-models` rebuilds AX26 scenery models from `tools/synth-models.mjs`; `verify` checks the licence allow-list, md5s and size budget. | — |
| **synth-models.mjs** | Procedural AX26 model catalog used by `assets.mjs bake-synthetic-models`. Same ids as circuit `bakedModel()` calls. | — |
| **tuner-sweep.mjs** | Does each LIGHTING TUNER slider actually change the image? SHARDED and RESUMABLE — one shard per (condition, knob range), flushed per knob, so a kill costs one knob. Uses PAIRED A→B→A' sampling: a single stale baseline measures elapsed time rather than the knob (measured: `starBright` scored 24.18 on a DAYLIGHT frame). `--list` / `--cond=` / `--from= --to=` / `--report` / `--force`. | lighting-tuner |
