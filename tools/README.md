...
| **coplanar-baseline.json** | The per-circuit z-fighting caps `coplanar-audit.cjs --gate` reads, so the tool and any future gate test can never disagree. Sibling to `clip-baseline.json`. | — |
