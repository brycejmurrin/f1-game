...
| **chrome-devtools-mcp.sh** | Wrapper for the local `scratch/chrome-devtools-mcp` clone (`clone`/`build`/`run`/`verify`/`status`). Prefers the Playwright Chromium on this box; `.mcp.json` points here so Cursor talks to a built binary instead of a one-shot `npx`. | mcp-probe |
