...
| **layout-audit.mjs** | Every menu screen crossed with every shape of display, measured rather than eyeballed — what escapes its clipper, what no scroll can reach, what is under WCAG's 24px, what scrolls sideways. Writes `artifacts/layout-audit/{audit.json,index.html}`; `--shots` adds a PNG per cell. `--screens=`/`--viewports=` top the grid up rather than replacing it. See docs/LAYOUT-AUDIT.md. | — |
