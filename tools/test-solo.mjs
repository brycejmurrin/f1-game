...
//
// Usage:
//   node tools/test-solo.mjs tests/specs/career.spec.js
//   node tools/test-solo.mjs tests/specs/career.spec.js -g "the sheet reports"
//   node tools/test-solo.mjs --max-load 6 tests/specs/career.spec.js   # raise the gate
//   node tools/test-solo.mjs --force tests/specs/career.spec.js        # skip the gate
//
...
if (!rest.length || rest.includes("--help") || rest.includes("-h")) {
  console.error("usage: node tools/test-solo.mjs [--force] [--max-load N] <spec.js> [-g <pattern>]");
  process.exit(2);
}
...
const runner = path.join(ROOT, "tools", "run-playwright.mjs");
