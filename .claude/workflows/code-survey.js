...
- No build step, no ES modules. Every file is a \`"use strict"\` IIFE assigning ONE global (sole exception: the vendored three.js island under js/render/three/vendor/). Load order lives in tools/manifest.cjs and is mirrored by index.html script tags.
...
const R1 = [
  { k: 'game-loop',      m: 'opus',   s: 'js/game.js lines 1-4300 — read them ALL, in chunks. This is the entry point: the frame loop, physics integration, race flow. Highest-value scope in the tree. Emphases: per-frame allocation inside the loop, dead branches left by module extractions, G-façade properties nothing consumes, stale closures over race state, work done every frame that only changes per lap.' },
  { k: 'game-flow',      m: 'opus',   s: 'js/game.js lines 4300-8522 — read them ALL, in chunks. Same emphases as the first half. Also: the AI and race-flow sections, and every `Module.create(G)` handoff — does each module actually receive what its file reads off G?' },
  ...
  { k: 'track-scenery',  m: 'sonnet', s: 'every file in js/track/scenery/ (city, nature, structures, identity, models, graph, data, circuit-kit, landmark-kit, themes). The 112-member scenery(api) contract is test-frozen — a member used by no circuit is still contract, report it as low severity only. Emphases: per-prop allocation during build, duplicated placement maths, unreachable emitter branches.' },
