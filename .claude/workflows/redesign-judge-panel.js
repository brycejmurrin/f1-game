...
Context: an unofficial WebGL2 F1 fan game. ~156 JS files, no build step, every file a "use strict" IIFE assigning one global; load order hand-maintained in tools/manifest.cjs mirrored by index.html script tags; a ~7,956-line js/game.js owns all closure state and hands extracted modules a "G" façade of live getters/setters (Module.create(G)); three renderer backends installed by descriptor-copy onto the GLX global; 111 Playwright specs + 49 unit suites; a family of structural guard tests substitutes for a compiler (load-order, docs-integrity, contract freezes, ratchets). Read docs/ARCHITECTURE-REVIEW.md (the standing review: the founding bet, the governing law "what a test asserts stays true; what only prose says drifts", open defects, backlog), docs/ARCHITECTURE.md, CLAUDE.md, tools/manifest.cjs, and skim js/game.js's G façade region (~lines 2540-2770) plus one or two js/game/ modules to ground yourself. The codebase is heavily developed BY AI AGENTS in parallel sessions — the architecture must protect against agents drifting out of sync, which is the root cause of most historical damage. Constraints that are non-negotiable: GitHub Pages static hosting; physics determinism (tests/physics-characterization.spec.js pins bit-stable laps; render()/updateCar() must not be carved up); the game must keep working on phones; the existing test suite is the safety net and must survive any migration incrementally (no big-bang rewrite — the repo's mess came from exactly that kind of unsynchronized bulk change).`
