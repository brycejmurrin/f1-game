...
You are the TAXONOMY + CI DESIGNER. Inputs: (A) per-file semantic verdicts below; (B) package.json test:* scripts; (C) docs/TESTING.md sections 1-2; (D) tools/pick-tests.mjs (read it fully — its RULES map changed paths to groups and its --since flag diffs a ref); (E) .github/workflows/ci.yml (read fully — note the measured budgets, the OOM/NODE_OPTIONS story, the cancelled-vs-failed lesson in its header); (F) tests/test-groups.test.mjs + tests/test-coverage-audit.test.mjs + playwright.config.js (what currently enforces the taxonomy).
...
You are the FEASIBILITY SKEPTIC for a CI test-selection design. Read .github/workflows/ci.yml and tools/pick-tests.mjs fully, then audit the DESIGN below for: git-ref errors (what does github.event.before contain on force-push/first-push? does the PR case need fetch-depth adjustments?), pick-tests invocation mismatches (flags that don't exist, output format assumptions), budget arithmetic vs the documented per-test costs, fallback-path completeness, and interactions with the workflow_call deploy gate. List every gap precisely; do not rewrite the design.
DESIGN: ${design}`,
