...
|---|---|---|
| Screen shake / camera impulse | `js/game.js` — `shake` var + decay block (~line 5079) | Wall hits: `shake = Math.min(1, shake + 0.1 + incidence * 0.3)` in the wall collision path (~4159). Car-car: `shake += impact * 0.45` in `collideFx()`. Kerb: `KERB_SHAKE` constant. |
| Kerb strike (weak-feeling kerb hits) | `js/game.js` `onKerb` block, ~line 3668-3676 | **Not particles.** The sticky `kerbCueT` hold layers `shake = Math.max(shake, KERB_SHAKE)`, `GameAudio.rumble()` (gated by `kerbSndT` cooldown), and haptics (`navigator.vibrate(15)` + `Input.rumble(0.25, 90)`, gated by `kerbHapT`) into one continuous cue instead of re-arming every ~4 m node. If "kerb hits feel weak", tune `KERB_SHAKE` / the rumble args / the haptic amount **here** — this is the one block that owns kerb feedback. |
...
| Wall/car-car sparks | `js/game/particles.js` / `Particles.sparks` | Wall scrape sparks: render loop in `game.js` (~6068) reads `Tracks.wallAt` proximity. Collision sparks via `c.fxSparkI` flag set in physics, consumed in render. |
| Off-track dust/grass kickup | `js/game/particles.js` / `Particles.kickup` | Fires only when `c.offroad` is true (`Math.abs(c.x) > hw && !c.onKerb` — **excludes riding a kerb**, ~line 6112). A car on a kerb (not off-track) gets the kerb row above, never `kickup`; don't add kickup particles to "fix" kerb feel — that channel is offroad-only by design. |
