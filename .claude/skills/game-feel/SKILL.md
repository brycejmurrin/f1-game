...
license: Apache-2.0
compatibility: Engine-agnostic techniques; snippets in GDScript (Godot 4.x) and C# (Unity 6) with pseudocode — translate the math/timing to this repo's plain-JS game loop.
metadata:
  engine: none
  category: disciplines
  difficulty: intermediate
  source: github.com/gamedev-skills/awesome-gamedev-agent-skills (skills/disciplines/game-feel), vendored 2026-07
---
...
## In this repo (Apex 26)

The snippets above are GDScript/C#, but every pattern maps onto this codebase:

- **Camera shake** → drive a decaying trauma value in `js/game.js` and apply it
  as a camera-rig offset (chase-cam smoothing path), never to the car's
  simulated position. Validate framing with the **debug-cameras** hooks.
- **Hit-stop / time scale** → scale the `dt` fed to the physics step for a few
  REAL frames; the render clock (`_skyT`) must keep advancing so sky/audio
  don't freeze. `__apex.freeze()` is the debug analogue, not the mechanism.
