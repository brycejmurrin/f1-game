...
offer `plate|wordmark|duo|slash` (from the 2026 launch photos; `bigmark` + `duo` is the RB22). `drawTailGraphic` clips to its region. `wingCarbon` (`paint|carbon`) puts every flap, front and rear, on `SURFACES.carbon` — the flap sites
