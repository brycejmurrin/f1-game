...
6. Per-circuit gate: `node tools/track/verify-track.cjs <id>`. Parent ship (optional fleet): `node tools/ci/test-bg.mjs circuits` + `node tools/gen/gen-shell.mjs --check` (no cache bump: tags read `?v=dev` and the deploy stamps the hashes; after a `tools/manifest.cjs` change run `node tools/gen/gen-shell.mjs`). The
