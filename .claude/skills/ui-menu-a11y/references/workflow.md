...
8. **Verify.** `npm run test:tooling-fast`, then `node tools/ci/test-bg.mjs ui`.
   If JS/CSS changed, `node tools/gen/gen-shell.mjs --check` (no cache bump is needed (tags read `?v=dev`; `pages.yml` stamps the hashes at deploy) — after a `tools/manifest.cjs` change run `node tools/gen/gen-shell.mjs`) before commit.
