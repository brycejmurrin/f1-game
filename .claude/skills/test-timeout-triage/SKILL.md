---
name: test-timeout-triage
description: Use when a Playwright test times out, hangs, or fails with "Test timeout of 120000ms exceeded" (including "while setting up context"), when deciding whether a failure is real or machine load, when tests pass loaded but fail solo (or vice versa), or when a waitForFunction overruns its declared timeout.
---

# Triage a test timeout — machine, wait, budget, or bug?
...
- ONE Playwright process, ONE browser group per batch (`tools/test-bg.mjs`
  enforces the cap). Browser+browser pairing is the measured source of the
  entire 120 s class.
