...
3. **Hard-reload with the cache disabled after a CSS/JS edit** — tags read
   `?v=dev`, so a soft reload may serve the cached sheet. Cost: a
   `min-height` fix measured as "not applied" because the browser served
   cached `menus.css`.
