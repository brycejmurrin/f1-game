...
7. **Ship** — commit `assets/pack/` when regenerated. Bump `?v=N` +
   `version.json` only if you changed `js/` or `css/` (`node tools/gen/gen-shell.mjs --check` (no cache bump is needed (tags read `?v=dev`; `pages.yml` stamps the hashes at deploy) — after a `tools/manifest.cjs` change run `node tools/gen/gen-shell.mjs`)). Pack
