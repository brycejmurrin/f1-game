...
7. **Ship** — commit `assets/pack/` when regenerated. Bump `?v=N` +
   `version.json` only if you changed `js/` or `css/` (`node tools/gen-shell.mjs --check` (no cache bump: tags read `?v=dev` and the deploy stamps the hashes; after a `tools/manifest.cjs` change run `node tools/gen-shell.mjs`)). Pack
