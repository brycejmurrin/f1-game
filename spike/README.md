...
| *(no `vendor/`)* | three.js resolves to the game's own `vendor/three-0.186.0/`, MIT — see below |
...
`physics/` harnesses now import those directly (`../vendor/three-0.186.0/`,
`../../vendor/rapier-0.19.3/`) and the duplicates are gone. Nothing about how the
spikes run changed.

The third, **Babylon.js 9.19.0** (8 MB, `babylon-spike.html/.js`), was the
