...
function postHarness(factoryThrows = false) {
  ...
  vm.runInNewContext(read("spike/backends/three/tlx-post.js"), sandbox,
...
test("TLX post fallback invalidates reads and disposes the retained chain", () => {
  ...
  assert.match(read("spike/backends/three/tsl-post.js"), /ctx\.trackMaterial\(m\)/);
});

test("TLX and WGX remove timed-out software-present waiters", () => {
  for (const file of ["spike/backends/three/tlx.js", "spike/backends/webgpu/wgx.js"]) {
    const src = read(file);
...
test("WGX soft present permits one staging read and drops pre-resize pixels", () => {
  const src = read("spike/backends/webgpu/wgx.js");
...
test("WGX and TLX distrust the CSS-size cache after a viewport change", () => {
  ...
  for (const [file, dirtyVar, wVar] of [
    ["spike/backends/webgpu/wgx.js", "_cssDirty", "_cssW"],
    ["spike/backends/three/tlx.js", "cssDirty", "cssW"],
  ]) {
    const src = read(file);
