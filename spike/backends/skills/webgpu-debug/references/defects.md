...
1. **Late sky erased the world.** Sky pipelines must use
   ...
   `spike/backends/tools/wgx-validate.mjs --static` asserts this without a browser.
...
2. **WGSL derivative uniformity — the NaN-white road.** Unlike GLSL, WGSL
   ...
   Enforced by `spike/backends/tests/unit/webgpu-lifecycle.test.mjs` (helpers must contain
...
`spike/backends/tools/wgx-validate.mjs --static` covers sky compare, no MSAA 2, bloom
