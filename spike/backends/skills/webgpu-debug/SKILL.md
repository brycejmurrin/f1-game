...
```sh
node spike/backends/tools/wgx-validate.mjs --static             # ALWAYS this first (no browser)
# parent session only — these launch Chromium:
# node spike/backends/tools/wgx-validate.mjs
# node spike/backends/tools/wgx-validate.mjs --lite
# node spike/backends/tools/wgx-validate.mjs --no-rg11b10
```
...
2D blit on `#game` — probe with `node spike/backends/tools/gfx-probe.mjs --backend webgpu`
(`awaitSoftPresent` + `#game` luma). Readback oracle:
`node spike/backends/tools/wgx-capture.mjs <track>` → `frame.png` via `GLX.capturePixels()`
...
- `spike/backends/tests/unit/webgpu-lifecycle.test.mjs` — mock-GPU + static WGSL uniformity.
- `tests/unit/gfx-backend-canary.test.mjs` — boot canary / probe-revert.
- `tests/unit/backend-surface-parity.test.mjs` +
  `spike/backends/docs/WEBGPU-PARITY.md` — Gfx façade × 3 backends.
...
`node spike/backends/tools/gfx-probe.mjs --backend webgpu <track>` (`#game` after
`awaitSoftPresent`). Multi-track gallery: `node spike/backends/tools/wgx-shot.mjs --gallery`
(or `npm run wgx:gallery`).
Readback oracle: `node spike/backends/tools/wgx-capture.mjs <track>`.
