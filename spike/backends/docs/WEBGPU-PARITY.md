...
(`spike/backends/webgpu/wgx.js` + `wgsl-*.js`).

**Status (2026-08): most of the §3 inventory is implemented** in
`spike/backends/webgpu/wgx.js` + `wgsl-*.js`, including lacquer ENV absorb,
...
are now gated by `spike/backends/tests/unit/webgpu-lifecycle.test.mjs` ("every sampleCount…",
...
reads the same texture as optional RGBA oracle — `spike/backends/tools/wgx-capture.mjs` →
`frame.png`; primary visible gate is `spike/backends/tools/gfx-probe.mjs` → `canvas.png`.
...
submit. Guarded by `spike/backends/tests/unit/webgpu-lifecycle.test.mjs` ("pipelines that
...
(`spike/backends/webgpu/wgx.js`) — as a real function when implemented, or as
...
which is why `node spike/backends/tools/wgx-validate.mjs` (real Dawn WGSL + pipeline
...
CI: `spike/backends/tests/unit/webgpu-lifecycle.test.mjs` and
...
- `spike/backends/webgpu/wgx.js` — backend, `requestDevice({requiredFeatures})`, MSAA 1\|4
- `spike/backends/webgpu/wgsl-chunks.js` — LIT Poisson-8 PCSS, `params8.x` lamp-fog, env LOD
- `spike/backends/webgpu/wgsl-post.js` — world-space god-ray, SSAO denoise, composite FX
...
2. ~~Shared road vertex buffer~~ **LANDED (2026-08-28)**. The ribbon is now
   ...
   **0.97**, ALL draws 65.44 -> **36.93** (the road fell from ~48% of the
   ...
   Evidence, in descending strength: (a) `spike/backends/tools/wgx-vid-repro.mjs` 30/30 OK
...
8. **Shared road vertex buffer / indexed road — UNBLOCKED by measurement
   ...
   shallow-clone graft. `spike/backends/tools/wgx-vid-repro.mjs` (committed as the
