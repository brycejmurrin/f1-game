/* Apex 26 — the G ctx façade, as a machine-checked contract.
   ...
   `const G = { … }` at js/game.js:2562 — and until this file existed nothing
...
/** One grid car, as built by makeCars() at js/game.js:1451. Fields listed are the
...
/** The one object js/game.js hands to every extracted module. Member order and
    grouping follow `const G = {…}` at js/game.js:2562-2812 exactly. */
interface GameCtx {
...
/* The modules constructed with the ctx itself — `X.create(G)` in game.js, plus
   ...
   (js/game/agentview.js:1100 hands it a bespoke bag, not the ctx) and
