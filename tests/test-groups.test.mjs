 ...
 *
 * Run: node --test tests/test-groups.test.mjs   (npm run test:tooling-fast)
 */
...
test("docs/TESTING.md's file counts match the files on disk", () => {
  ...
  const specs = fs.readdirSync(path.join(ROOT, "tests")).filter((f) => f.endsWith(".spec.js")).length;
  const units = fs.readdirSync(path.join(ROOT, "tests")).filter((f) => f.endsWith(".test.mjs")).length;
...
test("RENDER_SPECS partitions the root specs — every spec lands in exactly one project", () => {
  ...
  const onDisk = new Set(fs.readdirSync(path.join(ROOT, "tests"))
