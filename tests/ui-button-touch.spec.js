...
test("HOW TO PLAY names every way to play", async ({ page }) => {
  ...
  // Touch steering is absolute from screen centre (press left/right to turn).
  // Keep the copy honest so iOS "touch does nothing" reports are not
  // self-inflicted by stale half-screen or drag-only wording.
  expect(body.toUpperCase()).toContain("TOUCH");
  expect(body.toUpperCase()).toContain("CENTRE");
