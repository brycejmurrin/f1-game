...
test.describe("Parts module — catalog structure", () => {
  test("has exactly 8 categories", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("category IDs are correct", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("GEARBOX category exists with F1 Spec option", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("FUEL category exists with Qualifying Mix option", async ({ racePage }) => {
    const page = racePage;
    await load(page);
    const found = await page.evaluate(() => {
  ...
  test("all categories have at least 3 options", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("budget is 600", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("DEFAULTS includes gearbox and fuel", async ({ racePage }) => {
    const page = racePage;
    await load(page);
...
test.describe("Parts module — getMods()", () => {
  test("all defaults return near-1.0 multipliers", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("race engine increases speed and accel", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("extreme aero boosts cornering and reduces speed", async ({ racePage }) => {
    const page = racePage;
    await load(page);
    const base = await page.evaluate(() => Parts.getMods({}, ""));
  ...
  test("f1_spec gearbox increases accel and cornering", async ({ racePage }) => {
    const page = racePage;
    await load(page);
    const base = await page.evaluate(() => Parts.getMods({}, ""));
  ...
  test("quali_mix fuel increases speed and accel", async ({ racePage }) => {
    const page = racePage;
    await load(page);
    const base = await page.evaluate(() => Parts.getMods({}, ""));
  ...
  test("combining gearbox + fuel stacks multipliers", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("carbon ceramic brakes significantly improve braking", async ({ racePage }) => {
    const page = racePage;
    await load(page);
    const base = await page.evaluate(() => Parts.getMods({}, ""));
  ...
  test("supplier option ignored when team engine doesn't match", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("supplier option applied when team engine matches", async ({ racePage }) => {
    const page = racePage;
    await load(page);
    const base = await page.evaluate(() => Parts.getMods({}, "Mercedes"));
...
test.describe("Parts module — getCost()", () => {
  test("all defaults cost 0", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("race engine costs 160", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("f1_spec gearbox costs 180", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("custom_formula fuel costs 200", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("costs add up correctly across multiple categories", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("max setup exceeds budget of 600", async ({ racePage }) => {
    const page = racePage;
    await load(page);
...
test.describe("Parts module — statMult()", () => {
  test("stat 0 → ~0.85 multiplier", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("stat 100 → 1.00 multiplier", async ({ racePage }) => {
    const page = racePage;
    await load(page);
  ...
  test("stat 50 → ~0.925 multiplier", async ({ racePage }) => {
    const page = racePage;
    await load(page);
