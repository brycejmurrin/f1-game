 ...
 * `ctx.noteSpan(...)`, and tests/agent-view.spec.js asserts every kind it sees
 ...
 *
 * Run: node --test tests/span-kinds.test.mjs   (npm run test:tooling-fast)
 */
...
function specKinds() {
  const src = read("tests/agent-view.spec.js");
  const m = src.match(/const SPAN_KINDS = \[([\s\S]*?)\];/);
  assert.ok(m, "tests/agent-view.spec.js must declare a SPAN_KINDS array");
