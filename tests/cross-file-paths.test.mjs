...
// import `../tests/f1-api-mock.js` inside `try { … } catch { /* degrades */ }`.
...
test("a SWALLOWED dynamic import is found — the ⚠ case this exists for", () => {
  const src = `let m = null;
    try { ({ m } = await import("../tests/f1-api-mock.js")); } catch { /* degrades */ }`;
  const { refs } = referencesIn(src, "tools/fit-audit.mjs");
  assert.deepEqual(refs.map((r) => r.spec), ["../tests/f1-api-mock.js"]);
});
...
test("new URL(rel, import.meta.url) is found — the data-file shape a grep misses", () => {
  ...
  const { refs } = referencesIn(src, "tests/f1-track-accuracy.spec.js");
...
test("an import inside a STRING is not a reference", () => {
  // Not hypothetical: tests/assert-audit.test.mjs and tests/test-observed.test.mjs
