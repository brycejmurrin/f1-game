...
| `throttle-rescue-probe.spec.js` | **is the throttle really on after an off-track rescue in BUTTONS mode, and who is asserting it?** A player report from the deploy build: go off track in buttons steering, get auto-recovered, throttle reads always-on afterwards. input.js's three anti-latch nets are all shipped, so this is a fourth mechanism or not an input latch at all. Presses GAS with a REAL touch via CDP (never `setInput`, which bypasses `Input.throttle()` entirely), beaches the car, waits for RECOVERED, lifts the finger, then samples `inputState()` + `physState().speed` — the verdict names the asserting source (btn/key/pad) or convicts the physics side. A no-touch control run splits the two if the first reproduces |
