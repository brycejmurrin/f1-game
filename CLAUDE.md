...
js/game/         — game modules (each created with the G ctx façade from game.js) —
  ...
  wakelock.js    WakeLock       holds the screen awake for the duration of a
                                  race (hold/drop from startRace/endRace/
                                  quitToMenu). TILT is why: it sends NO input
                                  events by construction, so the idle timer
                                  never resets and the screen dims mid-race.
                                  Owns its own visibilitychange re-acquire —
                                  the platform drops the lock on every hide
