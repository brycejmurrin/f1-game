...
3. THE GATE IS A LADDER, EACH RUNG A SUBSET — green below never means green above:
   `test:guards` (hook-enforced, every commit) ⊂ `test:tooling-fast` (231 of 306 unit files)
   ⊂ `deploy.mjs --gate-only`, the only pre-push check that runs what the deploy runs (pushes nothing, dirty tree fine). The other 75 have taken deploys red three times — `docs/notes/PREPUSH-GATE-LADDER.md`. A commit whose every staged path is prose (`docs/`, `*.md`, skills, agents — no generated doc) runs only `docs-integrity`, and no ratchet raise.
