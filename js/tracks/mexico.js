  ...
  (window.TrackDefs = window.TrackDefs || []).push(
  {
    ...
    scenery: function (api) {
      const { out, n, place, backdrop, groundPlane,
              addBox, addCyl, every, onTrack, hash, vadd, anchor,
      ...
      // --- s=0.06 R far: park tree line along the long straight ---
      hedge(0.04, 0.10, 1, 26, 3.0, TREEGRN);
      for (const s of [0.04, 0.06, 0.08, 0.10]) {
        const k = K(s);
      ...
      // --- s=0.34 L far: distant Mexico City sprawl (pale grey-tan blocky skyline) ---
      every(34, (k) => {
        const near = Math.min((k - K(0.34) + n) % n, (K(0.34) - k + n) % n) < n * 0.14;
        if (!near && hash(k * 71) > 0.3) return;
        const side = -1, cluster = near ? 2 : 1;
        for (let c = 0; c < cluster; c++) {
          const d = 200 + c * 28 + hash(k * 72 + c) * 110;
          const p = anchor(k, side, d);
          if (onTrack(p.c[0], p.c[2], 12)) continue;
          const tone = 0.62 + hash(k * 73 + c) * 0.14;
          const h = 30 + hash(k * 74 + c) * 50, w = 14 + hash(k * 75 + c) * 10;
          building(k, side, d, w, h, w, { wall: [tone, tone * 0.97, tone * 0.92],
                   window: [tone * 0.66, tone * 0.70, tone * 0.74], floor: 5 });
        }
      });
      ...
      stadiumBowl(0.77, -1); stadiumBowl(0.77, 1);
      stadiumBowl(0.83, -1); stadiumBowl(0.83, 1);
      ...
      every(34, (k) => {
        for (const side of [-1, 1]) {
          if (hash(k * 91 + side) > 0.42) continue;
          const d = 26 + hash(k * 92 + side) * 50;
          const p = anchor(k, side, d);
          if (onTrack(p.c[0], p.c[2], 9)) continue;
          tree(k, side, d, 8 + hash(k * 94 + side) * 5, TREEGRN);
        }
      });
      ...
      // --- Distant hazed skyline ring (pale, flat city, no mountains) ---
      every(40, (k) => {
        for (const side of [-1, 1]) {
          if (hash(k * 81 + side) > 0.5) continue;
          backdrop(k, side, 340 + hash(k * 82 + side) * 140,
                   [70, 26 + hash(k * 83 + side) * 22, 28], [0.66, 0.68, 0.70]);
        }
      });
    },
  }
