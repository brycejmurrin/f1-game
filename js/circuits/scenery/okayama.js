...
(window.TrackScenery = window.TrackScenery || {})["okayama"] =
  function (api) {
      ...
      const { px, pz, addBox, out, vadd, terrainYAt } = api;

      // DRAPE — a flat decal laid ON the rendered terrain (what runoffApron /
      // groundPatch cannot do: both take ONE height per box, so a 26 m apron or
      // a 7 m clay pad laid by place() sank its whole top under the slope). The footprint (depth
