...
(window.TrackScenery = window.TrackScenery || {})["dijon"] =
  function (api) {
      ...
      const { px, pz, along, addBox, out, vadd } = api;
      // DRAPE — a flat decal laid ON the rendered terrain (what runoffApron /
      // groundPatch cannot do: both take ONE height per box, so Dijon's crop
      // fields and vine rows went up to 20 m under the hillside). The footprint (depth
        ...
        along(s - span / 2 / lapLen, s + span / 2 / lapLen, 20, (k, sp) => {
          ...
          if (dk * lapLen / n <= sz[2] / 2 + sp / 2)
            drape(k, side, gap, [sz[0], sz[1], sp * 1.04], col,
              Object.assign({ res, phase: st }, extra));
          st++;
        });
      };
