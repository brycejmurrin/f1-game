...
const TLX = (function () {
  ...
  async function create(canvas /*, opts */) {
    try {
      ...
      const lineStream = { geo: null, ib: null, cap: 0, min: 2048 * 2, stride: 6,
        attrs: [["position", 3, 0], ["lineAcross", 1, 3], ["lineSpeed", 1, 4], ["lineZone", 1, 5]] };
      ...
      const backend = {
        ...
        drawDrivingLine(verts, vertCount, dirty, opts) {
          ...
          if (dirty || fresh) {
            lineStream.ib.array.set(verts.subarray(0, vertCount * 6));
            uploadStream(lineStream, vertCount * 6);
