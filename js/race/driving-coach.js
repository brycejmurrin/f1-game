...
const DrivingCoach = (function () {
  function create(G) {
    ...
    function paint() {
      ...
      const select = $("pm-pit-choice"), note = $("pm-pit-estimate");
      if (select) {
        select.textContent = "";
        const option = (id, text) => { const o = document.createElement("option"); o.value = id; o.textContent = text; select.appendChild(o); };
        option("auto", "AUTOMATIC COMPOUND");
        for (const r of G.pits.choices(G.player)) option(r.id, r.code + " · " + r.id.replace(/_/g, " "));
        select.value = G.player && G.player.pitNext ? G.player.pitNext.id : "auto";
        select.disabled = !(G.player && G.tyres.on()) || G.player.pitState === "box";
      }
      if (note) {
