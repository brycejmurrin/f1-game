...
const LIV_ROW_HINT = {
  ...
  c2: "ACCENT — the second BASE colour. Unset optional rows (WINGS, REAR WING, TAIL FIN, T-CAM, DETAIL, SPINE TINT) fall back to this — not to each other.",
  accent: "DETAIL — a THIRD colour for trim only: the cover-flank pinstripe, the sidepod flash, the nose flank spans, the floor-edge lip and the fin strip. Unset = ACCENT. Does not recolour BODY STRIPE or SPINE TINT.",
  stripe: "BODY STRIPE — the full spine, nose tip to engine cover, broken only by the cockpit. This row only — it does not recolour the SPINE TOP saddle/band or the number plate.",
  ...
  spineTint: "SPINE TINT — the SPINE TOP band on the cover crown, and the saddle's flank half, ALONE. Unset = ACCENT (secondary), contrast-checked against ENGINE COVER. Never inherits BODY STRIPE.",
  ...
  plateTint: "PLATE PANEL — the board SPINE SIDE's PLATE design paints the number on. Unset = DETAIL or ACCENT, re-picked to separate from the flank — not BODY STRIPE.",
