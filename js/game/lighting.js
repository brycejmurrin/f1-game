...
const TUNE_DEFS = [
  ...
  { id: "sunElev",      label: "SUN ELEVATION",   group: "SUN & MOON", min: -90, max: 90, step: 0.1, def: 0, fmt: "signed", help: "Sun/moon height offset from the time-of-day default (deg). − lowers it for longer raking shadows + more god-rays. 0 = as-shipped. Capped at ±60° — the renderer clamps elevation to ±88° internally, so a wider slider would just hit that ceiling once combined with the time-of-day base." },
  ...
  { id: "roadRough",    label: "TARMAC ROUGHNESS",group: "ROAD & REFLECTIONS", min: 0.05, max: 1.8, step: 0.001, def: 1.0, help: "Scales dry-tarmac roughness — lower = glossier asphalt with a tighter sun streak. Bounds now track the shader's actual roughness clamp (0.85×value clamped to 0.04..1): below ~0.05 or above ~1.2 the old wider slider was just dead range that did nothing extra." },
  ...
  // ── SKY & WEATHER ──
  { id: "cloudCover",   label: "CLOUD COVER",     group: "SKY & WEATHER", min: -1.5, max: 1.5, step: 0.001, def: 0.0, fmt: "signed", help: "Shifts cloud amount up/down from the weather default (also drives cloud shadows). 0 = as-shipped. Capped at ±1 — the underlying cloud amount is itself clamped to 0..1, so a full ±1 swing already overrides any base value to clear or overcast; more range would be a no-op." },
  ...
  { id: "starDensity",  label: "STAR DENSITY",    group: "SKY & WEATHER", min: 0.2, max: 9, step: 0.01, def: 1.0, u: "uStarDensity", help: "How MANY stars fill the night sky (independent of STAR BRIGHTNESS, which sets how bright each one is). 1 = as-shipped, lower = a sparse sky, higher = a dense field. Capped so a huge value can't reject every cell to a blank sky." },
  ...
  { id: "wetness",      label: "WETNESS",         group: "SKY & WEATHER", min: -0.05, max: 1, step: 0.005, def: -0.05, fmt: "auto", help: "Override the road wetness ramp (AUTO = follow weather; ramps in over a few seconds after a weather flip). Already an exact 0..1 ramp plus its AUTO sentinel — left untouched." },
