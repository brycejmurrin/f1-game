...
const TUNE_DEFS = [
  ...
  { id: "matTexMix",    label: "BAKED MATERIALS", group: "ROAD & REFLECTIONS", min: 0, max: 1, step: 0.02, def: 0.0, u: "uMatTexMix", help: "Blends the BAKED PBR material maps (assets/pack — albedo, roughness and normal, indexed by the surface's MAT id) over the procedural noise. 0 = as-shipped, pure procedural. 1 = full baked detail. Multiplicative, so the per-track tarmac tint and racing-line wear survive at any setting. Does nothing when no asset pack is installed, or on the WebGPU backend (which has not ported the material system). Bake a pack with `node tools/assets.mjs`." },
