...
const TUNE_DEFS = [
  ...
  { id: "daySkyBlue",   label: "DAY SKY BLUE",    group: "SKY & WEATHER", min: 0, max: 2, step: 0.002, def: 1.0, u: "uDaySkyBlue", help: "Strength of the deep saturated-blue band pushed into the low/mid daytime sky so the gameplay sky strip isn't a flat pale wash. 0 = off (plain gradient), 1 = as-shipped. Ceiling is 2 — the shader clamps the mix at 1, so the peak of the band saturates there and extra range only punches the band edges; the bound was re-derived from measured consumer saturation (past 3, bandLM < 1/3, it is fully dead travel). Day-only; fades out under overcast and at dawn/dusk/night." },
  ...
  { id: "vignetteSoft", label: "VIGNETTE REACH",  group: "IMAGE & COLOUR", min: 0.1, max: 0.69, step: 0.001, def: 0.35, u: "uVigSoft", help: "How far the vignette reaches in from the corners (the inner edge of the darkening ramp). 0.35 = as-shipped. Lower = broad, soft gradient creeping toward the centre; higher = a thin dark ring hugging only the extreme corners. Ceiling is 0.69 — the shader does `smoothstep(0.95, min(this, 0.94), r)`, so anything above 0.94 is a no-op and the bound was re-derived below that from measured saturation. Pairs with VIGNETTE (which sets how dark)." },
