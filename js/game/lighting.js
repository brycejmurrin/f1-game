...
const TUNE_DEFS = [
  // ── SUN & MOON ──
  { id: "keyMul",       label: "KEY LIGHT (SUN)", group: "SUN & MOON", min: 0, max: 6, step: 0.005,  def: 1.0,  u: "uKeyMul", help: "Direct sun/moon intensity — diffuse + speculars + shadows. Ambient, fog and sky reflection are untouched, so the scene stays coherent when dimmed. Floor stays 0 (already fully off); headroom is on the bright end." },
  { id: "sunTemp",      label: "SUN / MOON WARMTH", group: "SUN & MOON", min: -3, max: 3, step: 0.005, def: 0.0, fmt: "signed", help: "White-balance of the direct key light (sun by day, moonlight at night). − warm sunrise/sodium, + cool overcast/moonlight. The tint math is a plain unclamped mix, so both ends scale evenly past the old ±1." },
  { id: "sunElev",      label: "SUN ELEVATION",   group: "SUN & MOON", min: -90, max: 90, step: 0.5, def: 0, fmt: "signed", help: "Sun/moon height offset from the time-of-day default (deg). − lowers it for longer raking shadows + more god-rays. 0 = as-shipped. Capped at ±60° — the renderer clamps elevation to ±88° internally, so a wider slider would just hit that ceiling once combined with the time-of-day base." },
  { id: "sunAzim",      label: "SUN AZIMUTH",     group: "SUN & MOON", min: -180, max: 180, step: 1, def: 0, fmt: "signed", help: "Rotates the key-light compass direction from the default — swings shadow direction across the track. 0 = as-shipped. Already a full compass turn; only the step got finer." },
  { id: "moonBright",   label: "MOON BRIGHTNESS", group: "SUN & MOON", min: 0, max: 4.5, step: 0.005, def: 1.0, help: "Moon disc/halo + its soft blue fill on the night sky." },
  { id: "grMul",        label: "SUN GOD-RAYS",    group: "SUN & MOON", min: 0, max: 6, step: 0.005, def: 1.0,  help: "Volumetric sun-shaft strength (dawn/dusk drama)." },
  { id: "godrayAniso",  label: "GOD-RAY FOCUS",   group: "SUN & MOON", min: 0, max: 0.95, step: 0.005, def: 0.60, u: "uHgAniso", help: "Shape of the volumetric SUN GOD-RAYS (Henyey-Greenstein forward-scatter anisotropy). 0 = diffuse glow spread evenly across the sky, 0.60 = as-shipped (a broad forward shaft), higher = a tight beam that only blazes when you look toward the sun. Only affects the volumetric god-ray pass." },
  { id: "godrayFloor",  label: "GOD-RAY HAZE",    group: "SUN & MOON", min: 0, max: 0.2, step: 0.001, def: 0.020, u: "uHgFloor", help: "Isotropic scatter floor added under the god-ray forward lobe — the ambient shaft haze that glows everywhere, not just toward the sun. 0 = shafts only where the sun's lobe reaches, 0.020 = as-shipped, higher = a milkier volumetric wash filling the whole sky." },
  { id: "godrayLowBoost", label: "GOD-RAY LOW-SUN DRAMA", group: "SUN & MOON", min: 0, max: 3, step: 0.005, def: 0.55, help: "How much STRONGER the volumetric sun-shafts get at dawn/dusk (low sun) than at midday. Base shaft strength is GOD-RAY BASE (def 0.38); this adds up to this much more as the sun drops toward the horizon. 0.55 = as-shipped, 0 = flat shaft strength all day (no low-sun drama), higher = blazing god-rays at sunrise/sunset. Scales only the low-sun boost — SUN GOD-RAYS scales the whole thing." },
  { id: "godrayBase",   label: "GOD-RAY BASE",    group: "SUN & MOON", min: 0, max: 2.25, step: 0.01, def: 0.38, help: "The flat baseline strength of the volumetric sun-shafts at ALL sun heights, before GOD-RAY LOW-SUN DRAMA adds the dawn/dusk boost on top. 0.38 = as-shipped, 0 = shafts appear only when the sun is low (pure low-sun drama), higher = strong god-rays even at midday. Rides SUN GOD-RAYS (the master) like the rest of the shaft strength." },
  { id: "sunShaftMul",  label: "SCREEN SUN-SHAFT",group: "SUN & MOON", min: 0, max: 6, step: 0.01, def: 1.0,  help: "Screen-space crepuscular rays streaming radially from the sun disc (a separate post pass from the volumetric SUN GOD-RAYS). 0 = off, 1 = as-shipped, higher = dramatic light-ray fan. Only shows when the sun is up and bright." },
  { id: "sunShaftDecay",label: "SUN-SHAFT REACH", group: "SUN & MOON", min: 0.4, max: 0.98, step: 0.005, def: 0.82, u: "uShaftDecay", help: "How far the screen-space SUN-SHAFT rays reach out from the sun (per-tap falloff of the radial march). 0.82 = as-shipped; lower = short stubby rays hugging the sun disc, higher = long shafts streaming across the frame. Pairs with SCREEN SUN-SHAFT (which sets strength)." },
  // ── AMBIENT & BOUNCE ──
  { id: "ambientMul",   label: "AMBIENT FILL",    group: "AMBIENT & BOUNCE", min: 0, max: 6, step: 0.005,  def: 1.0,  help: "Hemisphere ambient multiplier — the shadow/unlit fill and night readability floor. Floor is true 0 now (was 0.3): with no direct light in a shadow, 0 ambient means that shadow reads pure black — the actual 'crushed shadow' extreme, not just 'dim'." },
  { id: "ambTemp",      label: "AMBIENT WARMTH",  group: "AMBIENT & BOUNCE", min: -3, max: 3, step: 0.005, def: 0.0, fmt: "signed", help: "White-balance of the hemisphere fill (shadow/unlit areas). − warm bounce, + cool sky fill. Unclamped mix, so widened symmetrically like the other white-balance knobs." },
  { id: "ambBalance",   label: "SKY / GROUND FILL", group: "AMBIENT & BOUNCE", min: -3, max: 3, step: 0.01, def: 0.0, fmt: "signed", help: "Tips ambient toward ground bounce (−) or sky dome (+) — which side of shadows reads warm vs cool." },
  { id: "nightAmbLift", label: "NIGHT AMBIENT",   group: "AMBIENT & BOUNCE", min: 0, max: 6, step: 0.01, def: 1.0, help: "Scales the moody-night ambient floor/cap band directly (the 'how dark is night' master, applied BEFORE AMBIENT FILL). 0 = crush night ambient toward pure black so only the lamps/neon read; 1 = as-shipped; higher = lift the whole night out of the shadows. Only affects night sessions." },
  { id: "bounceK",      label: "LAMP BOUNCE",     group: "AMBIENT & BOUNCE", min: 0, max: 0.3, step: 0.00125, def: 0.04, u: "uBounceK", help: "Pool light bounced onto walls/kerbs/car flanks outside the beam cone." },
  // ── SHADOWS ──
  { id: "shadowStr",    label: "SHADOW DARKNESS", group: "SHADOWS", min: 0, max: 3, step: 0.005, def: 1.15, u: "uShadowStr", help: "How much direct sun the cast shadow removes. 1 = full shadow (ambient fill only inside it), 0 = shadows gone — full sun bleeds back in. Floor stays 0 (already the 'no shadow' extreme); above 1 crushes shadows past neutral toward black for a harder, more graphic look." },
  { id: "shadowRange",  label: "SHADOW DISTANCE", group: "SHADOWS", min: 16, max: 450, step: 1, def: 80, u: "uShadowRange", help: "Half-size of the sun shadow box (m). Lower = crisper nearby shadows; higher = shadows reach further before fading. Car shadow reach and shadow-map texel density scale with this too, so very high values soften/blur shadows in exchange for reach." },
  { id: "pcssPen",      label: "SHADOW SOFTEN",   group: "SHADOWS", min: 5, max: 750, step: 2.5, def: 80, u: "uPcssPen", help: "How fast shadows soften with caster distance (PCSS penumbra growth)." },
  { id: "shadowBias",   label: "SHADOW BIAS",     group: "SHADOWS", min: 0, max: 0.01, step: 0.00005, def: 0.001, u: "uShadowBias", help: "Depth offset. Too low = shadow acne (self-shadow shimmer); too high = shadows detach from feet. Repair tool." },
  { id: "shadowTintAmt",label: "SHADOW COOLNESS", group: "SHADOWS", min: 0, max: 2.25, step: 0.005, def: 0.0, u: "uShadowTintAmt", help: "Tints shadowed / ambient-only areas cool blue for a sunny-day contrast look. 0 = neutral." },
  { id: "moonShadow",   label: "MOON SHADOWS",    group: "SHADOWS", min: 0, max: 1, step: 0.005, def: 0.25, help: "Floor for cast-shadow strength under a bright CLEAR moon — soft moonlight shadows at night. Fades out by itself as cloud rolls in, the road gets wet, or fog sets; 0 = night shadows fade fully off (old behaviour). Above 0.5, this ALSO starts overriding the clear-moon weather gate so buildings/props cast shadows at night through cloud/fog/wet too — 1.0 = shadows cast regardless of weather." },
  ...
  { id: "aoStr",        label: "AMBIENT OCCLUSION", group: "SHADOWS", min: 0, max: 4.5, step: 0.01, def: 1.0, help: "Crease/contact darkening (SSAO). Floor 0 = off (already the minimum); above 1 pushes creases toward crushed black." },
  { id: "ssaoRadius",   label: "AO RADIUS",       group: "SHADOWS", min: 0.1, max: 6.15, step: 0.025, def: 0.6, u: "uRadius", help: "World-space reach of the AO sampling. Small = tight contact shading; large = broad soft occlusion. The screen-space sample radius saturates at close range past the default, so extra headroom mainly extends how far into the distance strong AO still reaches." },
  { id: "contactStr",   label: "CONTACT SHADOW",  group: "SHADOWS", min: 0, max: 4.5, step: 0.01, def: 1.0, help: "Grounding shadow under the car/props where the sun map can't reach." },
  { id: "ambContactDark", label: "AMBIENT CONTACT DARK", group: "SHADOWS", min: 0, max: 4.5, step: 0.01, def: 1.0, u: "uAmbContactDark", help: "How much the ambient-occlusion term darkens the AMBIENT fill in creases/contact points (distinct from AMBIENT OCCLUSION, which darkens the whole shaded result). 1 = as-shipped (up to 12% ambient drop in deep creases), 0 = ambient fill is untouched by AO (flatter, brighter corners), higher = crushes contact points toward black. WebGL2 renderer only." },
  ...
  { id: "lampLevel",    label: "LAMP LEVEL",      group: "LAMPS", section: "POOLS", min: 0.005, max: 2.25, step: 0.005, def: 0.26, help: "Overall lamp brightness ceiling (on top of the twilight ramp). Applies to every track lamp — street posts and flood banks alike." },
  { id: "lampDensity",  label: "LAMP DENSITY",    group: "LAMPS", section: "POOLS", min: 0.5, max: 3.75, step: 0.025, def: 1.0, rebuild: true, help: "How densely lamps are placed along the track (1 = shipped ~22 m stride). Higher = more poles/pools per kilometre; lower = sparser. Light pools update live; pole geometry refreshes on the next track load. Distinct from LAMP COUNT (how many nearest lamps the shader keeps)." },
  { id: "floodDay",     label: "DAYTIME LAMPS",   group: "LAMPS", section: "POOLS", min: 0, max: 2.25, step: 0.005, def: 0.0, help: "Light track lamps during DAY sessions (normally off — the sun dominates). 0 = off (as-shipped), higher = brighter daytime pools. Lets you run a lit stadium look under a blue sky. Brightness still rides LAMP LEVEL and the per-lamp POOL ENERGY on top of this." },
  { id: "poolEnergy",   label: "POOL ENERGY",     group: "LAMPS", section: "POOLS", min: 0.04, max: 3, step: 0.005, def: 0.55, rebuild: true, help: "Per-lamp pool luminance scale (physical energy per fixture). min/step chosen so the grid lands on every hundredth: at the old min 0.05 / step 0.02 the notches fell on ODD hundredths, so the max was unreachable (topped out at 1.99) and 22 shipped presets sat between notches." },
  { id: "lampRadiusMul",label: "POOL RADIUS",     group: "LAMPS", section: "POOLS", min: 0.3, max: 4.5, step: 0.01, def: 1.0,  rebuild: true, help: "Reach of each lamp pool. Too small and the far pool corner dies." },
  { id: "bleedMul",     label: "VALLEY BLEED",    group: "LAMPS", section: "POOLS", min: 0, max: 7.5, step: 0.025,  def: 1.0,  rebuild: true, help: "Out-of-beam light floor — lifts the dark valleys between pools. Floor stays 0 (pitch-dark valleys already the minimum); headroom is on the bright side." },
  { id: "glareStr",     label: "LENS GLARE",      group: "LAMPS", section: "POOLS", min: 0, max: 2.25, step: 0.005, def: 0.12, help: "Lens-halo billboard strength at every active lamp." },
  { id: "lampTemp",     label: "LAMP TEMPERATURE",group: "LAMPS", section: "POOLS", min: -3, max: 3, step: 0.005, def: 0.0, fmt: "signed", help: "White-balance of ALL track lamps (street posts and flood banks). − warms toward sodium/amber, + cools toward LED/broadcast white. Layers over each lamp's own colour. Unclamped mix, widened symmetrically like the other white-balance knobs." },
  { id: "lampFlicker",  label: "LAMP FLICKER",    group: "LAMPS", section: "POOLS", min: 0, max: 0.6, step: 0.0025, def: 0.10, help: "How much aging lamps pulse. 0 = rock-steady, higher = strong buzz on the odd tube." },
  { id: "beamCone",     label: "BEAM CONE WIDTH", group: "LAMPS", section: "POOLS", min: 0.4, max: 3.3, step: 0.005, def: 1.0, rebuild: true, help: "Width of every lamp's illuminated cone. Wider = softer spread, narrower = tight hotspots." },
  { id: "lampWallSpill", label: "LAMP WALL SPILL", group: "LAMPS", section: "POOLS", min: 0, max: 4.5, step: 0.01, def: 1.0, u: "uLampWallSpill", help: "How much a lamp's light spills onto surfaces OUTSIDE its beam cone — the dim wash on walls/kerbs beside the lit pool (before the bright in-beam term takes over). 1 = as-shipped (a touch stronger on wet nights), 0 = the light is confined hard to its cone with pitch-black surroundings, higher = a broad ambient spill that lifts the whole surround. WebGL2 renderer only." },
  // Twilight ramp, warm-up, and the per-frame nearest-N cull
  { id: "twilightFloor", label: "TWILIGHT FLOOR",  group: "LAMPS", section: "BEHAVIOUR", min: 0.05, max: 1, step: 0.025, def: 0.30, help: "Minimum lamp level at bright dawn/dusk while the sun is still up. Low = lamps stay dim until the sun drops; high = pools are already strong under a lit twilight sky. Only affects dawn/dusk sessions." },
  { id: "twilightRamp",  label: "TWILIGHT RAMP",   group: "LAMPS", section: "BEHAVIOUR", min: 2, max: 21, step: 0.25, def: 6, help: "How fast lamps climb from the twilight floor to full as the sun sets (steepness vs sun elevation). Higher = a quicker snap to full night lighting; lower = a long, gradual build. Dawn/dusk only." },
  { id: "twilightWarm",  label: "TWILIGHT WARMTH", group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 4.5, step: 0.025, def: 1.0, help: "How amber the lamps glow at twilight (the 'just switched on' sodium warmth that fades toward neutral by deep night). 0 = neutral lamps at dusk, 1 = as-shipped, higher = strong orange twilight cast. Dawn/dusk only." },
  { id: "lampWarmup",    label: "LAMP WARM-UP",    group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 6, step: 0.05, def: 1.0, help: "How long lamps take to ramp from a dim sodium-orange strike to full cool brightness when they switch on (scales the ~4-8 s warm-up). 0 = instant full brightness, 1 = as-shipped, higher = a long, staggered warm-up glow." },
  { id: "lampWarmupDim", label: "WARM-UP DIP",     group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 0.9, step: 0.025, def: 0.30, help: "How dim a freshly-struck lamp starts before warming to full (depth of the strike dip). 0 = lamps strike at full brightness, 0.30 = as-shipped (start at 70%), higher = a deeper cold start." },
  { id: "lampWarmupWarm",label: "WARM-UP WARMTH",  group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 4.5, step: 0.025, def: 1.0, help: "How orange a freshly-struck lamp glows before settling to its true colour (strike-warmth amount). 0 = strikes at final colour, 1 = as-shipped sodium-warm start, higher = a strong amber ignition." },
  ...
  { id: "lampCullFade",  label: "LAMP CULL FADE",  group: "LAMPS", section: "BEHAVIOUR", min: 0.1, max: 0.9, step: 0.025, def: 0.35, help: "How far inside the nearest-lamp boundary a lamp reaches full brightness (the distance fade that hides lamps entering/leaving the set at speed). Low = a thin fade band, a sharper edge to the lit zone; high = a broad, gentle falloff into the dark." },
  ...
  { id: "perChunkLights", label: "PER-CHUNK LAMPS", group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 1, step: 0.005, def: 0, help: "EXPERIMENTAL. 0 = off. ABOVE 0 it turns the feature on AND sets how strongly the per-chunk lamps light the scene \u2014 it is an amount, not a switch, because turning it on genuinely delivers more light: a fragment that used to see the handful of the global 32 that actually reach it now sees up to 32 that all do, so the whole night reads far too hot at the same LAMP LEVEL. Start around 0.2-0.4 and raise it; 1.0 is the original un-dimmed behaviour. Car tail-lights are NOT scaled by this.  Gives every chunk of scenery its OWN nearest-32 lamps instead of making the whole visible scene share one set of 32. The 32-lamp ceiling is the fragment shader\'s uniform-array size, so it limits lamps per DRAW, not per scene — and because lamps are baked per track and chunk bounds are fixed, each chunk\'s set is computed once and cached. On a dense night circuit this lights the far road without LAMP COUNT / LAMP REACH AHEAD having to ration slots, and per-fragment cost drops (a chunk binds only lamps that reach it). Costs one uniform upload per chunk draw, so it trades a little CPU for a lot of reach. 0 = as-shipped single global set." },
  { id: "lampReach",     label: "LAMP REACH AHEAD", group: "LAMPS", section: "BEHAVIOUR", min: 1, max: 6, step: 0.05, def: 1.0, help: "How much extra priority lamps AHEAD of the camera get in the nearest-lamp cull, so the lit zone reaches further down the road before a dense track's lamp budget runs out. The number is the REACH MULTIPLIER for a lamp dead ahead: 4 = a lamp four times further away still wins a slot (lamps to the side are unaffected, and 1 = as-shipped, pure distance + BEHIND-CAM BIAS). Only matters once a track has more lamps in range than the LAMP COUNT budget (32 shader slots, 28 with traffic) — where it does, expect the far end of a lit straight to extend rather than the near road to brighten." },
  { id: "lampNearClamp", label: "LAMP NEAR CLAMP", group: "LAMPS", section: "BEHAVIOUR", min: 1, max: 18, step: 0.125, def: 4.0, u: "uLampNearClamp", help: "Minimum distance (m) used in each lamp's inverse-square falloff, so a surface right under a fixture can't blow out to infinite brightness. 4 = as-shipped, lower = a hot, tight pool with a fierce hotspot directly beneath the lamp, higher = a softer, flatter pool that never over-brightens up close. Both renderers." },
  // ── NIGHT GLOW & BLOOM ──
  { id: "floodEmitMul", label: "LIT GEOMETRY",    group: "NIGHT GLOW & BLOOM", min: 0, max: 4.5, step: 0.005,  def: 1.0,  help: "How lit the night buildings/windows/signage render (prop emissive ramp)." },
  { id: "glowAmp",      label: "EMISSIVE GLOW",   group: "NIGHT GLOW & BLOOM", min: 0.2, max: 9, step: 0.005,   def: 2.3,  u: "uGlowAmp", help: "HDR push for windows / lenses / neon — roughly half the night frame energy." },
  { id: "neonBoost",    label: "NEON & LENS BLOOM", group: "NIGHT GLOW & BLOOM", min: 0, max: 4.5, step: 0.01, def: 0.6, u: "uBloomBoost", help: "Extra HDR glow for authored light-source surfaces — neon bands, signage panes, floodlight lenses (anything baked BRIGHTER than white). Scales with how far past white the surface is, so generic emissive (lit concrete, warm windows, night road glow) is untouched — neon pops without raising EMISSIVE GLOW or dropping BLOOM THRESHOLD for the whole scene. 0 = the old uniform push." },
  { id: "cityGlowMul",  label: "CITY SKYGLOW",    group: "NIGHT GLOW & BLOOM", min: 0, max: 7.5, step: 0.005,  def: 1.0, help: "Light-pollution dome hugging the horizon over lit circuits/cities." },
  { id: "cityGlowWarm", label: "SKYGLOW WARMTH",  group: "NIGHT GLOW & BLOOM", min: -3, max: 3, step: 0.01, def: 0.0, fmt: "signed", help: "White-balance of the city skyglow dome AND the warm hue it casts into the night ambient. − cools toward LED/mercury white-blue, + warms toward sodium amber. 0 = the per-theme shipped tint (magenta-ish neon canyons, amber towns)." },
  { id: "cityGlowTint", label: "SKYGLOW ON AMBIENT", group: "NIGHT GLOW & BLOOM", min: 0, max: 2.25, step: 0.01, def: 0.28, help: "How strongly the city skyglow's colour bleeds into the night ambient fill — the neon/sodium hue you see wash the shadows in a lit city. 0 = neutral night ambient (glow only in the sky dome), 0.28 = as-shipped, higher = the whole night reads in the city's colour." },
  { id: "bloomMul",     label: "BLOOM AMOUNT",    group: "NIGHT GLOW & BLOOM", min: 0, max: 6, step: 0.005,  def: 1.0,  help: "Halo strength around bright HDR sources (lamps, neon, windows)." },
  { id: "bloomSpread",  label: "BLOOM SPREAD",    group: "NIGHT GLOW & BLOOM", min: 0.3, max: 6, step: 0.005, def: 1.0, u: "uSpread", help: "Halo WIDTH, independent of amount. Higher = wider, dreamier glow; lower = tight core." },
  { id: "threshOff",    label: "BLOOM THRESHOLD", group: "NIGHT GLOW & BLOOM", min: -0.5, max: 0.2, step: 0.0025,  def: 0.0,  help: "Offset on what counts as bright enough to bloom. Lower = mid-tones glow (fog-of-glow)." },
  { id: "bloomKnee",    label: "BLOOM ON HIGHLIGHTS", group: "NIGHT GLOW & BLOOM", min: 0, max: 1, step: 0.01, def: 0.5, u: "uBloomKnee", help: "How much bloom is SUPPRESSED over already-bright pixels (highlight knee). 0 = bloom everything evenly (milky, dreamy); 1 = strongly hold bloom off blown highlights so only mid-bright HDR sources halo (crisp). 0.5 = as-shipped." },
  // ── ATMOSPHERE ──
  { id: "fogDensityMul",label: "FOG DENSITY",     group: "ATMOSPHERE", min: 0, max: 7.5, step: 0.005, def: 1.0, u: "uFogDensity", help: "Scales atmospheric haze depth — how fast distance fades into fog. 1 = as-shipped." },
  { id: "fogHeight",    label: "FOG HEIGHT FALLOFF", group: "ATMOSPHERE", min: 0, max: 0.2, step: 0.0005, def: 0.018, u: "uFogHeight", help: "How fast fog thins with altitude. 0 = uniform wall; higher = fog pools low and clears overhead." },
  { id: "fogTint",      label: "FOG WARM / COOL", group: "ATMOSPHERE", min: -3, max: 3, step: 0.005, def: 0.0, u: "uFogTint", fmt: "signed", help: "White-balance of the distance haze. + warm (amber/dusty), − cool (blue/overcast). Unclamped mix in-shader, so widened like the other white-balance knobs." },
  { id: "fogColorSat",  label: "FOG COLOUR SATURATION", group: "ATMOSPHERE", min: 0, max: 4.5, step: 0.01, def: 1.0, help: "How colourful vs neutral-grey the base fog/haze colour is. 1 = as-shipped, 0 = a flat achromatic grey haze, higher = a vivid coloured murk (e.g. the amber dawn / blue overcast fog reads much stronger). Distinct from FOG WARM / COOL (which shifts the hue) — this sets how far the fog sits from grey. Applied to the condition-derived fog colour before the warm/cool balance." },
  { id: "mistDensity",  label: "GROUND MIST",     group: "ATMOSPHERE", min: 0, max: 6, step: 0.005, def: 1.0, u: "uGroundMist", help: "Amount of low-lying drifting ground mist (dawn/humid/fog). 0 = none, higher = thick rolling bank." },
  { id: "mistHeight",   label: "MIST HEIGHT BAND",group: "ATMOSPHERE", min: 0.04, max: 1.8, step: 0.005, def: 0.30, u: "uMistHeight", help: "How tall the ground-mist layer stands. Low = ankle fog, high = deep bank up to the eyeline." },
  { id: "lampFogBase",  label: "FOG GLOW BASE",   group: "ATMOSPHERE", min: 0, max: 0.9, step: 0.005, def: 0.45, help: "How strongly lamps tint the distant fog wall on a clear night. Tops out at 0.9 because its only consumer sums it with FOG GLOW HAZE under a Math.min(0.9, ...) ceiling (js/game.js) — the slider used to run to 1.5, so its top 40% moved nothing at all." },
  { id: "lampFogHaze",  label: "FOG GLOW HAZE",   group: "ATMOSPHERE", min: 0, max: 3.75, step: 0.005, def: 0.6,  help: "Extra lamp-fog glow added as ground mist / fog weather thickens." },
  { id: "mistShare",    label: "MIST GLOW SHARE", group: "ATMOSPHERE", min: 0, max: 9, step: 0.025,  def: 1.5,  u: "uMistShare", help: "Ground-mist share of the lamp glow vs the air-fog share." },
  { id: "hazeWetShare", label: "WET HAZE SHARE",  group: "ATMOSPHERE", min: 0, max: 2.25, step: 0.005, def: 0.22, help: "How much a WET road feeds the volumetric haze that god-rays and lamp-beams scatter through (spray/humidity hanging in the air). 0.22 = as-shipped, 0 = wet roads add no airborne haze (shafts only respond to mist/cloud), higher = rain sessions fill with thick light-catching spray. Separate from GROUND MIST and the on-road wetness sheen." },
  { id: "hazeCloudShare", label: "CLOUD HAZE SHARE", group: "ATMOSPHERE", min: 0, max: 2.25, step: 0.005, def: 0.12, help: "How much CLOUD COVER feeds the volumetric haze that god-rays and lamp-beams scatter through (an overcast sky's diffuse murk). 0.12 = as-shipped, 0 = cloud adds no airborne haze, higher = heavy overcast fills the air with light-catching haze. Separate from cloud SHADOWS and the sky dome cover." },
  { id: "fogClip",      label: "FOG GLOW CLIP",   group: "ATMOSPHERE", min: 0, max: 3.75, step: 0.01, def: 0.7,  u: "uLampFogClip", help: "Soft shoulder stopping lamp clusters pushing the fog wall to white." },
  { id: "fogSunCore",   label: "FOG SUN CORE",    group: "ATMOSPHERE", min: 0, max: 4.5, step: 0.01, def: 0.6, u: "uFogSunCore", help: "The tight hot bloom of the sun burning through haze — a concentrated bright core right at the sun's position in the fog (on top of the broad sun-glow scatter). 0.6 = as-shipped, 0 = no hot core (just the wide glow), higher = a fierce sun burning through the murk. Best seen at low dawn/dusk sun through fog." },
  { id: "overcastFogMul", label: "OVERCAST FOG BOOST", group: "ATMOSPHERE", min: 1, max: 9, step: 0.025, def: 1.7, help: "How much thicker the atmospheric haze gets in OVERCAST weather (multiplies the base fog density). 1.7 = as-shipped, 1 = overcast adds no extra haze (clear-day fog depth), higher = a soupier grey murk. Only affects overcast sessions; FOG BOOST does the same for fog weather." },
  { id: "fogWxMul",     label: "FOG BOOST",       group: "ATMOSPHERE", min: 1, max: 12, step: 0.025, def: 3.0, help: "How much thicker the atmospheric haze gets in FOG weather (multiplies the base fog density). 3.0 = as-shipped, 1 = fog weather adds no extra haze, higher = near-zero-visibility pea-soup. Only affects fog sessions; OVERCAST FOG BOOST does the same for overcast." },
  { id: "lampVolBase",  label: "BEAMS (CLEAR)",   group: "ATMOSPHERE", min: 0, max: 0.8, step: 0.0025, def: 0.05, help: "Volumetric lamp-beam strength in clear night air. Mobile tier renders no beams regardless (the volumetric pass is shed for GPU headroom)." },
  { id: "lampVolHaze",  label: "BEAMS (HAZE)",    group: "ATMOSPHERE", min: 0, max: 3.75, step: 0.025, def: 0.65, help: "How much haze/rain swells the lamp beams." },
  { id: "lampVolCap",   label: "BEAM CEILING",    group: "ATMOSPHERE", min: 0, max: 2.25, step: 0.005, def: 0.70, help: "Hard cap on volumetric beam strength." },
  { id: "renderDistMul", label: "RENDER DISTANCE", group: "ATMOSPHERE", min: 0.5, max: 3, step: 0.025, def: 1.0, help: "Scales how far the camera can see — the far clipping plane AND the scenery draw-distance cull move together. 1 = as-shipped (900 m). Higher reveals more distant track/scenery at a real GPU cost (more chunks drawn every frame); lower saves performance on weak devices. Fog still fades distant geometry out visually regardless of this setting." },
  // ── REFLECTIONS ──
  { id: "ssrWetMul",    label: "WET MIRROR",      group: "ROAD & REFLECTIONS", min: 0, max: 3.75, step: 0.005, def: 1.0,  help: "Wet-road scene-mirror strength (scales the wetness ramp)." },
  { id: "ssrDryNight",  label: "DRY NIGHT SHEEN", group: "ROAD & REFLECTIONS", min: 0, max: 1, step: 0.0025, def: 0.08, help: "Dry tarmac lamp/neon sheen at night." },
  { id: "ssrDryDay",    label: "DRY DAY SHEEN",   group: "ROAD & REFLECTIONS", min: 0, max: 0.6, step: 0.0025, def: 0.07, help: "Faint tower-and-sky mirror on dry day tarmac." },
  { id: "roadRough",    label: "TARMAC ROUGHNESS",group: "ROAD & REFLECTIONS", min: 0.05, max: 1.8, step: 0.005, def: 1.0, help: "Scales dry-tarmac roughness — lower = glossier asphalt with a tighter sun streak. Bounds now track the shader's actual roughness clamp (0.85×value clamped to 0.04..1): below ~0.05 or above ~1.2 the old wider slider was just dead range that did nothing extra." },
  { id: "surfDetail",   label: "SURFACE DETAIL",  group: "ROAD & REFLECTIONS", min: 0, max: 5.25, step: 0.01, def: 1.0, help: "Road/terrain procedural grain + micro-normal relief (aggregate, patches, cracks). 0 = flat." },
  { id: "matTexMix",    label: "BAKED MATERIALS", group: "ROAD & REFLECTIONS", min: 0, max: 1, step: 0.01, def: 1.0, u: "uMatTexMix", help: "Blends the BAKED PBR material maps (assets/pack — real CC0 photoscans: albedo, roughness and normal, indexed by the surface's MAT id) over the procedural noise. 1 = as-shipped, full baked detail. 0 = pure procedural, the pre-scan look — and at 0 the pack is not downloaded at all. Multiplicative, so the per-track tarmac tint and racing-line wear survive at any setting. Does nothing when no asset pack is installed, or on the WebGPU backend (which has not ported the material system). If tarmac ever crawls or shimmers at speed, this is the slider to pull." },
  { id: "ssrThick",     label: "SSR THICKNESS",   group: "ROAD & REFLECTIONS", min: 0.02, max: 7.5, step: 0.01, def: 0.20, u: "uSsrThick", help: "Depth tolerance for a wet-road reflection hit. Lower = crisper but more gaps; higher = fewer holes, more smear. Ceiling raised to 5 — the hit-test's own far-plane reject means values above that would be a no-op." },
  { id: "wetDark",      label: "WET ROAD DARKEN", group: "ROAD & REFLECTIONS", min: 0, max: 3, step: 0.005, def: 1.0, u: "uWetDark", help: "How much darker wet asphalt reads (water absorption). Independent of the wetness amount." },
  { id: "windowSunFlash", label: "WINDOW SUN FLASH", group: "ROAD & REFLECTIONS", min: 0, max: 4.5, step: 0.01, def: 1.0, u: "uWindowSunFlash", help: "The mirror-bright flash of the sun off building glass — the hot specular pinpoint that slides across reflective façades as you pass. 1 = as-shipped, 0 = flat matte glass with no sun hotspot, higher = a blazing glare off every window. Day-only; scales with how directly the glass faces the sun. WebGL2 renderer only." },
  { id: "skyRimGlow",   label: "SKY RIM GLOW",    group: "ROAD & REFLECTIONS", min: 0, max: 4.5, step: 0.01, def: 1.0, u: "uSkyRimGlow", help: "Fresnel rim light along the grazing edges of reflective surfaces (glass, wet bodywork) picking up the sky — the bright sheen where a surface curves away from you. 1 = as-shipped, 0 = no edge sheen (matte silhouettes), higher = a strong glassy rim halo. Strongest on smooth surfaces at grazing angles. WebGL2 renderer only." },
  // ── CAR ──
  { id: "carReflect",   label: "CAR REFLECTION",  group: "CAR", min: 0, max: 3.75, step: 0.005, def: 0.05, u: "uCarReflect", help: "How strongly the world (track, sky, lights) mirrors on the car bodywork." },
  ...
  { id: "carGloss",     label: "PAINT GLOSS",     group: "CAR", min: 0, max: 2.4, step: 0.005, def: 1.0,  u: "uCarGloss", help: "Sharpness of the paint's highlights & reflections. Higher = glassier (lower roughness). Range now tracks the SSR-streak formula's own clamp window (roughly 0..1.4): below 0 or above ~1.4 the old slider was pushing a value the shader already saturates on." },
  { id: "carSpecular",  label: "PAINT SPECULAR",  group: "CAR", min: 0, max: 5.25, step: 0.01, def: 1.0,  help: "Brightness of the specular highlight rolling over the bodywork." },
  { id: "carClearcoat", label: "CLEARCOAT",       group: "CAR", min: 0, max: 5.25, step: 0.005, def: 0.05, help: "Lacquer coat that catches crisp sun / lamp glints over the base colour." },
  { id: "carMetal",     label: "PAINT METALNESS", group: "CAR", min: 0, max: 7.5, step: 0.01, def: 1.0,  help: "How metallic the paint reads — reflection tint and grazing falloff." },
  { id: "carGlow",      label: "BODY GLOW",       group: "CAR", min: 0, max: 7.5, step: 0.01, def: 1.0,  help: "Self-lit body glow after dark (only the night / wet liveries carry it)." },
  { id: "tailLightMul", label: "TAIL-LIGHT GLOW", group: "CAR", min: 0, max: 7.5, step: 0.025, def: 1.0, help: "Brightness of the red glow trailing nearby cars after dark." },
  { id: "brakeGlowMul", label: "BRAKE FLARE",     group: "CAR", min: 0, max: 4.5, step: 0.025, def: 1.0, help: "How hard the tail-light glow surges while a car is braking (scales the brake-heat flare on top of TAIL-LIGHT GLOW). 0 = steady tail light, no flare." },
  { id: "tailRange",    label: "TAIL-LIGHT RANGE", group: "CAR", min: 60, max: 480, step: 5, def: 160, help: "How far (m) a car's tail-light keeps working as a real light source on nearby traffic. Beyond this it's dropped from the light budget. Higher = traffic glow reaches further back down the field." },
  { id: "tailFade",     label: "TAIL-LIGHT FADE",  group: "CAR", min: 0, max: 180, step: 2.5, def: 0, help: "Distance (m) over which a car's tail-light eases out before TAIL-LIGHT RANGE, so it doesn't pop in/out abruptly as traffic drifts past the limit. 0 = hard cutoff (as-shipped), higher = softer, longer fade." },
  { id: "carSunGlint",  label: "PAINT SUN GLINT",  group: "CAR", min: 0, max: 60, step: 0.25, def: 12.0, u: "uCarSunGlint", help: "The mirror-bright pinpoint reflection of the sun off the car's clearcoat (the moving hotspot that slides over the bodywork). 12 = as-shipped, 0 = no sun hotspot on the paint, higher = a blazing HDR glint that blooms hard. Separate from PAINT SPECULAR (the broad highlight)." },
  { id: "carSparkle",   label: "METALLIC SPARKLE", group: "CAR", min: 0, max: 9, step: 0.025, def: 1.6, u: "uCarSparkle", help: "Gain of the metallic-flake glitter in the paint — the tiny individual flakes that flash as the camera moves. 1.6 = as-shipped, 0 = a flat non-metallic finish, higher = a denser, brighter sparkle field. Only affects metallic-flagged liveries." },
  // ── SKY & WEATHER ──
  { id: "cloudCover",   label: "CLOUD COVER",     group: "SKY & WEATHER", min: -1.5, max: 1.5, step: 0.005, def: 0.0, fmt: "signed", help: "Shifts cloud amount up/down from the weather default (also drives cloud shadows). 0 = as-shipped. Capped at ±1 — the underlying cloud amount is itself clamped to 0..1, so a full ±1 swing already overrides any base value to clear or overcast; more range would be a no-op." },
  { id: "cloudShadowDim", label: "CLOUD SHADOW DEPTH", group: "SKY & WEATHER", min: 0, max: 1, step: 0.01, def: 0.80, u: "uCloudShadowDim", help: "How darkly drifting clouds dim the sun where their shadow passes over the track. 0 = clouds cast no shade (flat, evenly-lit ground even under heavy cover), 0.80 = as-shipped, 1 = clouds fully block the sun in their shadow. Pairs with CLOUD COVER (which sets how much cloud there is)." },
  { id: "cloudSpeed",   label: "CLOUD SPEED",     group: "SKY & WEATHER", min: 0, max: 12, step: 0.025, def: 1.0, u: "uCloudSpeed", help: "How fast clouds drift and evolve. 0 = frozen sky, higher = fast-moving weather." },
  { id: "starBright",   label: "STAR BRIGHTNESS", group: "SKY & WEATHER", min: 0, max: 6, step: 0.005, def: 1.0, u: "uStarBright", help: "Night star intensity. 0 = washed sky, higher = vivid starfield." },
  { id: "starDensity",  label: "STAR DENSITY",    group: "SKY & WEATHER", min: 0.2, max: 9, step: 0.05, def: 1.0, u: "uStarDensity", help: "How MANY stars fill the night sky (independent of STAR BRIGHTNESS, which sets how bright each one is). 1 = as-shipped, lower = a sparse sky, higher = a dense field. Capped so a huge value can't reject every cell to a blank sky." },
  { id: "skyGrad",      label: "SKY GRADIENT",    group: "SKY & WEATHER", min: 0.15, max: 1.8, step: 0.005, def: 0.35, u: "uSkyGrad", help: "Shape of the horizon→zenith sky gradient (the exponent). 0.35 = as-shipped (deep zenith blue reaching well down the sky); lower pulls the rich colour even lower for a moodier dome; higher washes the mid-sky pale and keeps saturation to the very top." },
  { id: "daySkyBlue",   label: "DAY SKY BLUE",    group: "SKY & WEATHER", min: 0, max: 3.75, step: 0.01, def: 1.0, u: "uDaySkyBlue", help: "Strength of the deep saturated-blue band pushed into the low/mid daytime sky so the gameplay sky strip isn't a flat pale wash. 0 = off (plain gradient), 1 = as-shipped, higher = a punchy azure band. Day-only; fades out under overcast and at dawn/dusk/night." },
  { id: "mieScatter",   label: "SKY SUN GLOW",    group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, u: "uMieScatter", help: "Mie forward-scatter glow around the sun — the bright haze bloom filling the sky in the sun's direction, strongest near the horizon. 1 = as-shipped, 0 = a clean sky with no glow, higher = a big hazy sun-halo washing across the upper sky." },
  { id: "cloudSilver",  label: "CLOUD SILVER LINING", group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, u: "uCloudSilver", help: "Backlit sun-facing cloud-edge glow (the bright silver/gold rim on clouds between you and the sun). 1 = as-shipped, 0 = flat matte clouds with no rim light, higher = blazing fire-lit edges. Most intense at golden hour." },
  { id: "coronaAureole",label: "SUN AUREOLE",     group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, u: "uCoronaAureole", help: "The broad soft halo of light around the sun disc (distinct from the tight inner ring and the sun disc itself). 1 = as-shipped, 0 = a bare sun disc with no surrounding glow, higher = a large radiant aureole." },
  { id: "sunDiscSize",  label: "SUN DISC SIZE",   group: "SKY & WEATHER", min: 0.3, max: 6, step: 0.025, def: 1.0, u: "uSunDiscSize", help: "Angular size of the sun disc in the sky. 1 = as-shipped (real-sun size), larger = a bigger dramatic sun (sunset-photo look), smaller = a tighter pinpoint. Scales the disc radius without touching the corona or aureole." },
  { id: "sunCorona",    label: "SUN CORONA RING", group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, u: "uSunCorona", help: "The tight bright inner ring hugging the sun disc (distinct from the broad SUN AUREOLE and the disc itself). 1 = as-shipped, 0 = no ring (bare disc + aureole), higher = a fierce hot halo right at the disc edge." },
  { id: "sunSquash",    label: "SUN HORIZON SQUASH", group: "SKY & WEATHER", min: 0, max: 4.5, step: 0.01, def: 1.0, u: "uSunSquash", help: "How much the sun disc flattens vertically as it nears the horizon (atmospheric-refraction oval, strongest at golden hour). 1 = as-shipped, 0 = a perfectly round sun at any height, higher = a strongly squashed sunset oval." },
  { id: "starSize",     label: "STAR SIZE",       group: "SKY & WEATHER", min: 0.2, max: 6, step: 0.01, def: 1.0, u: "uStarSize", help: "Angular size of each night star point. 1 = as-shipped, lower = fine pinpricks, higher = fatter stars. Independent of STAR BRIGHTNESS (how bright) and STAR DENSITY (how many)." },
  { id: "starTwinkle",  label: "STAR TWINKLE",    group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, u: "uStarTwinkle", help: "How much the stars pulse in brightness over time (twinkle amplitude). 1 = as-shipped, 0 = rock-steady stars, higher = a lively shimmering sky. Needs CLOUD SPEED / animation running." },
  { id: "moonDiscSize", label: "MOON DISC SIZE",  group: "SKY & WEATHER", min: 0.3, max: 6, step: 0.025, def: 1.0, u: "uMoonDiscSize", help: "Angular size of the moon disc at night. 1 = as-shipped, larger = a big dramatic moon, smaller = a distant pinpoint. Scales the disc without touching the halo." },
  { id: "moonHalo",     label: "MOON HALO SPREAD", group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, u: "uMoonHalo", help: "The soft glow spreading out from the moon disc. 1 = as-shipped, 0 = a bare disc with no halo, higher = a broad hazy glow (moon through thin cloud). Scales both the halo strength and how far it reaches." },
  { id: "cityGlowReach", label: "CITY GLOW REACH", group: "SKY & WEATHER", min: 0.2, max: 6, step: 0.01, def: 1.0, u: "uCityGlowReach", help: "How high up the sky the city light-pollution dome climbs from the horizon. 1 = as-shipped (hugs the horizon), lower = the glow reaches higher up toward the zenith, higher = a thin band pinned to the horizon. Pairs with CITY SKYGLOW (strength)." },
  { id: "cloudDef",     label: "CLOUD DEFINITION", group: "SKY & WEATHER", min: 0, max: 3, step: 0.01, def: 1.0, u: "uCloudDef", help: "How sharply the billow octave carves lumpy cumulus edges onto the cloud cover. 1 = as-shipped, 0 = soft flat cloud smears, higher = harder-edged cauliflower puffs. WebGL2 renderer only (the WebGPU sky uses a simpler cloud model)." },
  { id: "skyColorSat",  label: "SKY COLOUR SATURATION", group: "SKY & WEATHER", min: 0, max: 4.5, step: 0.01, def: 1.0, help: "How colourful vs neutral-grey the sky dome is (zenith + horizon bands). 1 = as-shipped, 0 = a flat achromatic grey sky, higher = a punchy oversaturated dome (deeper blue day, more vivid dawn/dusk colour). Applied to the condition-derived sky colours; the reflected sky (glass/wet-road mirror) follows it too, so the whole scene stays coherent. Distinct from SKY GRADIENT (gradient shape) and DAY SKY BLUE (the daytime blue band)." },
  { id: "wetness",      label: "WETNESS",         group: "SKY & WEATHER", min: -0.05, max: 1, step: 0.025, def: -0.05, fmt: "auto", help: "Override the road wetness ramp (AUTO = follow weather; ramps in over a few seconds after a weather flip). Already an exact 0..1 ramp plus its AUTO sentinel — left untouched." },
  ...
  { id: "rainStreak",   label: "RAIN STREAK LEN", group: "SKY & WEATHER", min: 0.2, max: 6, step: 0.01, def: 1.0, reinitRain: true, help: "Length of rain streaks — short spits vs long driving streaks." },
  { id: "rainSpeed",    label: "RAIN FALL SPEED", group: "SKY & WEATHER", min: 0.2, max: 4.5, step: 0.01, def: 1.0, reinitRain: true, help: "How fast the rain streaks fall down the screen (base velocity of each drop before the speed-reactive shear). 1 = as-shipped, lower = a slow drifting drizzle, higher = a hard driving downpour. Drizzle (WET, no storm) still falls at 60% of this." },
  { id: "drizzleCount", label: "DRIZZLE DENSITY",  group: "SKY & WEATHER", min: 0, max: 1, step: 0.01, def: 0.30, reinitRain: true, help: "How many streaks a DRIZZLE (WET road, no storm) draws vs a full downpour. 0.30 = as-shipped (30% of RAIN INTENSITY), 0 = a damp track with no visible fall, 1 = drizzle as dense as a storm. Only affects WET (non-raining) sessions." },
  { id: "drizzleLen",   label: "DRIZZLE STREAK",   group: "SKY & WEATHER", min: 0, max: 2.25, step: 0.01, def: 0.50, reinitRain: true, help: "Length of DRIZZLE streaks vs a full storm. 0.50 = as-shipped (half-length spits), lower = fine mist dots, higher = longer streaks approaching storm rain. Only affects WET (non-raining) sessions." },
  { id: "drizzleSpeed", label: "DRIZZLE FALL SPEED", group: "SKY & WEATHER", min: 0, max: 2.25, step: 0.01, def: 0.60, reinitRain: true, help: "How fast DRIZZLE falls vs a full storm. 0.60 = as-shipped (a slow soft fall), lower = a drifting haze, higher = a brisker fall approaching storm speed. Only affects WET (non-raining) sessions." },
  { id: "rainOpacity",  label: "RAIN OPACITY",    group: "SKY & WEATHER", min: 0, max: 3, step: 0.01, def: 1.0, help: "How visible the falling rain is (alpha of the streak layer). 1 = as-shipped, 0 = invisible rain, higher = a dense, near-opaque sheet. Drizzle draws fainter than a full storm at any setting." },
  { id: "rainWind",     label: "RAIN WIND",       group: "SKY & WEATHER", min: -3, max: 3, step: 0.005, def: 0.18, fmt: "signed", help: "Horizontal wind slant on the rain (angle of the streaks). Unclamped drift term, so widened like the white-balance knobs — high settings rake the rain almost horizontal." },
  { id: "rainShearWind", label: "RAIN SPEED SLANT", group: "SKY & WEATHER", min: 0, max: 4.5, step: 0.01, def: 0.9, help: "How much the rain slants toward you as the car speeds up (apparent-wind shear added to RAIN WIND, ramping in to full by ~90 m/s). 0.9 = as-shipped, 0 = rain falls at the same angle standing still or flat-out, higher = the streaks rake hard sideways at speed." },
  { id: "rainShearLen", label: "RAIN SPEED STRETCH", group: "SKY & WEATHER", min: 0, max: 9, step: 0.025, def: 2.0, help: "How much the rain streaks stretch out as the car speeds up (motion-streak elongation, ramping to full by ~90 m/s). 2.0 = as-shipped (up to 3× length flat-out), 0 = streaks keep their standing length at any speed, higher = long driving streaks at speed." },
  { id: "lightning",    label: "LIGHTNING FREQ",  group: "SKY & WEATHER", min: 0, max: 9, step: 0.005, def: 1.0, help: "Storm lightning strike rate. 0 = off, higher = more frequent flashes." },
  { id: "lightningFlash", label: "LIGHTNING FLASH", group: "SKY & WEATHER", min: 0, max: 4.5, step: 0.01, def: 1.0, help: "How brightly a lightning strike bleaches the scene (scales the ambient spike + exposure lift together). 1 = as-shipped, 0 = thunder only with no visible flash, higher = a blinding strobe. LIGHTNING FREQ sets how often; this sets how hard." },
  { id: "lightningDecay", label: "LIGHTNING DECAY", group: "SKY & WEATHER", min: 2, max: 30, step: 0.25, def: 8.0, help: "How fast a lightning flash fades after the strike (exponential decay rate). 8 = as-shipped (~0.9 s glow), lower = a lingering sky-wide flicker, higher = a snappy instant flash." },
  { id: "weatherSunMute", label: "WEATHER SUN MUTE", group: "SKY & WEATHER", min: 0, max: 3, step: 0.01, def: 1.0, help: "How much wet/rain/overcast/fog weather dims the direct sun. 0 = weather doesn't mute the sun at all (bright, hard light through the storm); 1 = as-shipped; higher = deeper flat-grey murk. No effect in clear/dry weather." },
  // ── IMAGE & COLOUR ──
  { id: "exposureMul",  label: "EXPOSURE",        group: "IMAGE & COLOUR", min: 0.1, max: 4.5, step: 0.005,  def: 1.0,  help: "Master brightness multiplier on the tone-map input (all times of day) — applied before the tone-mapper, so this is the most direct way to darken or blow out the whole frame. Floor dropped toward near-black; ceiling raised for a hard overexposed look." },
  { id: "contrast",     label: "CONTRAST",        group: "IMAGE & COLOUR", min: 0.5, max: 4.5, step: 0.005, def: 1.12, u: "uContrast", help: "Midtone-darkening gamma. Higher = deeper, filmic shadows; lower = flatter and brighter. Ceiling pushed further than the floor since the ask is specifically for harder shadow crush." },
  { id: "blacks",     label: "BLACKS",     group: "IMAGE & COLOUR", section: "TONAL RANGE", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Exposure of the deepest detail near black. Positive reveals it; negative crushes it. 0 is neutral." },
  { id: "shadows",    label: "SHADOWS",    group: "IMAGE & COLOUR", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Exposure of dark asphalt, tyres and unlit bodywork above the black point. 0 is neutral." },
  { id: "midtones",   label: "MIDTONES",   group: "IMAGE & COLOUR", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Exposure around middle grey: most paint, grass and track detail. 0 is neutral." },
  { id: "highlights", label: "HIGHLIGHTS", group: "IMAGE & COLOUR", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Exposure of bright bodywork, clouds and lamp pools below peak white. 0 is neutral." },
  { id: "whites",     label: "WHITES",     group: "IMAGE & COLOUR", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Exposure of the brightest HDR values feeding the ACES shoulder. Negative protects peaks; positive drives them harder. 0 is neutral." },
  { id: "toe",        label: "TOE",        group: "IMAGE & COLOUR", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Shapes the transition out of black. Positive deepens the toe; negative lifts it. 0 is neutral." },
  { id: "shoulder",   label: "SHOULDER",   group: "IMAGE & COLOUR", min: -1.5, max: 1.5, step: 0.005, def: 0, fmt: "signed", help: "Shapes highlight compression before ACES. Positive protects bright detail; negative expands it. 0 is neutral." },

  { id: "liftR", label: "LIFT · RED", group: "IMAGE & COLOUR", section: "RGB LIFT / GAMMA / GAIN", min: -0.15, max: 0.15, step: 0.00125, def: 0, fmt: "signed", help: "Red offset at the black end of the RGB grade. 0 is neutral." },
  { id: "liftG", label: "LIFT · GREEN", group: "IMAGE & COLOUR", min: -0.15, max: 0.15, step: 0.00125, def: 0, fmt: "signed", help: "Green offset at the black end of the RGB grade. 0 is neutral." },
  { id: "liftB", label: "LIFT · BLUE", group: "IMAGE & COLOUR", min: -0.15, max: 0.15, step: 0.00125, def: 0, fmt: "signed", help: "Blue offset at the black end of the RGB grade. 0 is neutral." },
  { id: "gammaR", label: "GAMMA · RED", group: "IMAGE & COLOUR", min: 0.5, max: 3, step: 0.005, def: 1, help: "Red-channel midpoint control. Above 1 lifts red midtones; below 1 deepens them. 1 is neutral." },
  { id: "gammaG", label: "GAMMA · GREEN", group: "IMAGE & COLOUR", min: 0.5, max: 3, step: 0.005, def: 1, help: "Green-channel midpoint control. Above 1 lifts green midtones; below 1 deepens them. 1 is neutral." },
  { id: "gammaB", label: "GAMMA · BLUE", group: "IMAGE & COLOUR", min: 0.5, max: 3, step: 0.005, def: 1, help: "Blue-channel midpoint control. Above 1 lifts blue midtones; below 1 deepens them. 1 is neutral." },
  { id: "gainR", label: "GAIN · RED", group: "IMAGE & COLOUR", min: 0.5, max: 2.25, step: 0.005, def: 1, help: "Red scale at the white end of the RGB grade. 1 is neutral." },
  { id: "gainG", label: "GAIN · GREEN", group: "IMAGE & COLOUR", min: 0.5, max: 2.25, step: 0.005, def: 1, help: "Green scale at the white end of the RGB grade. 1 is neutral." },
  { id: "gainB", label: "GAIN · BLUE", group: "IMAGE & COLOUR", min: 0.5, max: 2.25, step: 0.005, def: 1, help: "Blue scale at the white end of the RGB grade. 1 is neutral." },

  { id: "saturation",   label: "SATURATION",      group: "IMAGE & COLOUR", section: "COLOUR", min: 0, max: 4.5, step: 0.005, def: 1.0, u: "uSaturation", help: "Overall colour intensity. 0 = greyscale, 1 = as-shipped, >1 = punchier." },
  { id: "vibrance",     label: "VIBRANCE",        group: "IMAGE & COLOUR", min: 0, max: 2.25, step: 0.005, def: 0.20, u: "uVibrance", help: "Selective saturation — lifts dull/washed pixels (hazy sky, grass, tarmac) without over-cooking neon or kerbs." },
  { id: "tint",         label: "WARM / COOL",     group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.005, def: 0.0, u: "uTint", fmt: "signed", help: "White-balance shift. + warms (amber, sunny), − cools (blue, overcast/night). Unclamped mix, widened symmetrically like the other white-balance knobs." },
  { id: "gradeStr",     label: "GRADE STRENGTH",  group: "IMAGE & COLOUR", min: 0, max: 6, step: 0.005, def: 1.0, help: "Cinematic split-tone amount (teal shadows / warm highlights). 0 = neutral, higher = stronger film look." },
  { id: "shadowHue",    label: "SHADOW TINT HUE", group: "IMAGE & COLOUR", min: -180, max: 180, step: 1, def: 0.0, fmt: "signed", help: "Rotates the split-tone SHADOW colour (default cool teal) around the hue wheel. Already a full hue wheel — only the step got finer." },
  { id: "hiHue",        label: "HIGHLIGHT TINT HUE", group: "IMAGE & COLOUR", min: -180, max: 180, step: 1, def: 0.0, fmt: "signed", help: "Rotates the split-tone HIGHLIGHT colour (default warm amber) around the hue wheel. Already a full hue wheel — only the step got finer." },
  { id: "vignette",     label: "VIGNETTE",        group: "IMAGE & COLOUR", section: "LENS & FINISH", min: 0, max: 1, step: 0.005, def: 0.80, u: "uVignette", help: "Corner darkening. 1 = none, lower = stronger frame vignette. Floor is true 0 (pure black corners) — the frame centre is mixed in separately so it never darkens no matter how low this goes. Ceiling stays 1: past that the corners would read brighter than the centre, which isn't a 'vignette' anymore." },
  { id: "vignetteSoft", label: "VIGNETTE REACH",  group: "IMAGE & COLOUR", min: 0.1, max: 0.92, step: 0.005, def: 0.35, u: "uVigSoft", help: "How far the vignette reaches in from the corners (the inner edge of the darkening ramp). 0.35 = as-shipped. Lower = broad, soft gradient creeping toward the centre; higher = a thin dark ring hugging only the extreme corners. Pairs with VIGNETTE (which sets how dark)." },
  { id: "blackLift",    label: "BLACK FLOOR",      group: "IMAGE & COLOUR", min: 0, max: 0.2, step: 0.0005, def: 0.005, u: "uBlackLift", help: "Raises the darkest blacks toward a faded film base. 0 = pure black, higher = matte shadows. Floor is already the true-black extreme so it can't go lower; only the matte ceiling was raised." },
  { id: "whitePoint",   label: "ACES WHITE SCALE", group: "IMAGE & COLOUR", min: 0.4, max: 6, step: 0.01, def: 1.0, u: "uWhitePoint", help: "Highlight roll-off knee. Lower clips highlights sooner (punchy, brighter overall), higher preserves highlight detail AND reads darker/more filmic overall (it divides the whole image before tone-mapping). Ceiling raised for a much moodier, darker top-end." },
  { id: "acesA",        label: "TONE CURVE SHOULDER", group: "IMAGE & COLOUR", min: 1, max: 6, step: 0.005, def: 2.51, u: "uAcesA", help: "ACES filmic tone-curve numerator quadratic coefficient (a in the Narkowicz fit). Governs the highlight SHOULDER — how hard bright HDR values roll off toward white. 2.51 = as-shipped; lower softens the roll-off (highlights bloom to white sooner), higher tightens it (holds more highlight detail). Advanced: reshapes the whole tone-map — pairs with the other TONE CURVE knobs." },
  { id: "acesB",        label: "TONE CURVE TOE LIFT", group: "IMAGE & COLOUR", min: 0, max: 0.3, step: 0.001, def: 0.03, u: "uAcesB", help: "ACES tone-curve numerator linear coefficient (b). Lifts the low end — brightens deep shadows/blacks slightly as they enter the curve. 0.03 = as-shipped, 0 = a touch crushier toe, higher = milkier lifted blacks. Advanced: part of the raw ACES fit (distinct from BLACK FLOOR, which is applied separately)." },
  { id: "acesC",        label: "TONE CURVE CONTRAST", group: "IMAGE & COLOUR", min: 1, max: 6, step: 0.005, def: 2.43, u: "uAcesC", help: "ACES tone-curve denominator quadratic coefficient (c). The main midtone-contrast lever of the filmic curve — trades brightness against contrast through the mids. 2.43 = as-shipped; changing it rebalances the whole S-curve. Advanced: pairs with TONE CURVE SHOULDER (both are the dominant shape terms)." },
  { id: "acesD",        label: "TONE CURVE MIDS",  group: "IMAGE & COLOUR", min: 0.1, max: 3, step: 0.005, def: 0.59, u: "uAcesD", help: "ACES tone-curve denominator linear coefficient (d). Shifts the midtone brightness of the filmic curve — where the mid-grey point sits. 0.59 = as-shipped, lower = brighter mids, higher = darker/denser mids. Advanced: part of the raw ACES fit." },
  { id: "acesE",        label: "TONE CURVE BLACK",  group: "IMAGE & COLOUR", min: 0.02, max: 0.6, step: 0.001, def: 0.14, u: "uAcesE", help: "ACES tone-curve denominator constant (e). Sets the true black floor / overall lift of the filmic curve. 0.14 = as-shipped, lower = deeper blacks and more contrast, higher = a lifted, hazier low end. Floored at 0.02 so the curve's denominator stays positive (no divide-by-zero). Advanced: part of the raw ACES fit." },
  { id: "chromAb",      label: "CHROMATIC AB.",   group: "IMAGE & COLOUR", min: 0, max: 7.5, step: 0.01, def: 0.0, u: "uChromAb", help: "Lens colour-fringing toward the frame edges — RGB split. Subtle = filmic, high = arcade." },
  { id: "grain",        label: "FILM GRAIN",      group: "IMAGE & COLOUR", min: 0, max: 0.3, step: 0.001, def: 0.0, u: "uGrain", help: "Per-pixel sensor noise. A little sells the cinematic night-camera look." },
  { id: "lensDirt",     label: "LENS DIRT",       group: "IMAGE & COLOUR", min: 0, max: 1, step: 0.005, def: 0.15, u: "uLensDirt", help: "Grime on the lens: bright light (sun, floodlights, neon) scatters into a smudgy veil and the sun flare goes blotchy where dirt catches the glare. 0 = clean lens; only shows where the frame carries bright energy." },
  { id: "flareMul",     label: "LENS FLARE",      group: "IMAGE & COLOUR", min: 0, max: 5.25, step: 0.005, def: 1.0, help: "Sun/lamp anamorphic streak + ghost strength. 0 = off, 1 = as-shipped." },
  { id: "flareStreak",  label: "FLARE STREAK",    group: "IMAGE & COLOUR", min: 2, max: 45, step: 0.25, def: 7.0, u: "uFlareStreak", help: "Width of the horizontal anamorphic sun-flare streak (its horizontal tightness). 7 = as-shipped; lower spreads it into a wide screen-spanning band of light, higher pulls it into a tight contained highlight hugging the sun. Pairs with LENS FLARE (which sets strength)." },
  { id: "flareStreak2", label: "FLARE CORE STREAK", group: "IMAGE & COLOUR", min: 0, max: 4.5, step: 0.01, def: 0.5, u: "uFlareStreak2", help: "Strength of the second thin hot-white core streak layered over the main FLARE STREAK (a tighter, brighter inner blade of the anamorphic flare). 0.5 = as-shipped, 0 = single-streak flare, higher = a piercing hot core line. Pairs with LENS FLARE (strength) and FLARE STREAK (width)." },
  { id: "sharpen",      label: "SHARPEN",         group: "IMAGE & COLOUR", min: 0, max: 3, step: 0.01, def: 0.0, u: "uSharpen", help: "Crispness recovered after FXAA — counteracts softening on kerbs, wires and distant detail." },
  { id: "speedBlur",    label: "SPEED BLUR",      group: "IMAGE & COLOUR", min: 0, max: 3, step: 0.01, def: 0.0, u: "uSpeedBlur", help: "Radial blur from screen centre that grows with car speed — a velocity cue at high speed." },

  // ── FX ── transient particle effects (js/game/particles.js pool)
  { id: "particleMul",  label: "PARTICLE FX",     group: "FX", min: 0, max: 3, step: 0.025, def: 1.0, help: "Transient particle amount — tyre smoke, collision sparks, gravel kickup and rain spray. 0 = off, 1 = as-shipped, 2 = double the emission rate." },
];
