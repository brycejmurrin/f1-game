...
const TUNE_DEFS = [
  ...
  { id: "daySkyBlue",   label: "DAY SKY BLUE",    group: "SKY & WEATHER", min: 0, max: 3, step: 0.002, def: 1.0, u: "uDaySkyBlue", help: "Strength of the deep saturated-blue band pushed into the low/mid daytime sky so the gameplay sky strip isn't a flat pale wash. 0 = off (plain gradient), 1 = as-shipped. Ceiling is 3 — the shader clamps the mix at 1, so the peak of the band saturates at 1 and extra range only punches the band edges; past 3 (bandLM < 1/3) that is dead travel too. Day-only; fades out under overcast and at dawn/dusk/night." },
  { id: "mieScatter",   label: "SKY SUN GLOW",    group: "SKY & WEATHER", min: 0, max: 4.544, step: 0.002, def: 1.0, u: "uMieScatter", help: "Mie forward-scatter glow around the sun — the bright haze bloom filling the sky in the sun's direction, strongest near the horizon. 1 = as-shipped, 0 = a clean sky with no glow. Ceiling is 4.544 — the shader mix-clamps at 1 with a 0.22 peak coefficient, so 1/0.22 is the last live notch on-sun at the horizon on a clear day." },
