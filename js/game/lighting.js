...
const TUNE_DEFS = [
  ...
  { id: "sunTemp",      label: "SUN / MOON WARMTH", group: "SUN & MOON", min: -6, max: 6, step: 0.001, def: 0.0, fmt: "signed", help: "White-balance of the direct key light (sun by day, moonlight at night). − warm sunrise/sodium, + cool overcast/moonlight. The tint math is a plain unclamped mix, so both ends scale evenly past the old ±1." },
  { id: "sunElev",      label: "SUN ELEVATION",   group: "SUN & MOON", min: -180, max: 90, step: 0.1, def: 0, fmt: "signed", help: "Sun/moon height offset from the time-of-day default (deg). − lowers it for longer raking shadows + more god-rays. 0 = as-shipped. Capped at ±60° — the renderer clamps elevation to ±88° internally, so a wider slider would just hit that ceiling once combined with the time-of-day base." },
  ...
  { id: "moonBright",   label: "MOON BRIGHTNESS", group: "SUN & MOON", min: 0, max: 9, step: 0.001, def: 1.0, help: "Moon disc/halo + its soft blue fill on the night sky." },
  { id: "grMul",        label: "SUN GOD-RAYS",    group: "SUN & MOON", min: 0, max: 12, step: 0.001, def: 1.0,  help: "Volumetric sun-shaft strength (dawn/dusk drama)." },
  ...
  { id: "godrayLowBoost", label: "GOD-RAY LOW-SUN DRAMA", group: "SUN & MOON", min: 0, max: 6, step: 0.001, def: 0.55, help: "How much STRONGER the volumetric sun-shafts get at dawn/dusk (low sun) than at midday. Base shaft strength is GOD-RAY BASE (def 0.38); this adds up to this much more as the sun drops toward the horizon. 0.55 = as-shipped, 0 = flat shaft strength all day (no low-sun drama), higher = blazing god-rays at sunrise/sunset. Scales only the low-sun boost — SUN GOD-RAYS scales the whole thing." },
  { id: "godrayBase",   label: "GOD-RAY BASE",    group: "SUN & MOON", min: 0, max: 4.5, step: 0.002, def: 0.38, help: "The flat baseline strength of the volumetric sun-shafts at ALL sun heights, before GOD-RAY LOW-SUN DRAMA adds the dawn/dusk boost on top. 0.38 = as-shipped, 0 = shafts appear only when the sun is low (pure low-sun drama), higher = strong god-rays even at midday. Rides SUN GOD-RAYS (the master) like the rest of the shaft strength." },
  { id: "sunShaftMul",  label: "SCREEN SUN-SHAFT",group: "SUN & MOON", min: 0, max: 12, step: 0.002, def: 1.0,  help: "Screen-space crepuscular rays streaming radially from the sun disc (a separate post pass from the volumetric SUN GOD-RAYS). 0 = off, 1 = as-shipped, higher = dramatic light-ray fan. Only shows when the sun is up and bright." },
  ...
  { id: "ambTemp",      label: "AMBIENT WARMTH",  group: "AMBIENT & BOUNCE", min: -6, max: 6, step: 0.001, def: 0.0, fmt: "signed", help: "White-balance of the hemisphere fill (shadow/unlit areas). − warm bounce, + cool sky fill. Unclamped mix, so widened symmetrically like the other white-balance knobs." },
  ...
  { id: "shadowBias",   label: "SHADOW BIAS",     group: "SHADOWS", min: 0, max: 0.01, step: 0.00001, def: 0.001, u: "uShadowBias", help: "Depth offset. Too low = shadow acne (self-shadow shimmer); too high = shadows detach from feet. Repair tool." },
  { id: "shadowTintAmt",label: "SHADOW COOLNESS", group: "SHADOWS", min: 0, max: 4.5, step: 0.001, def: 0.0, u: "uShadowTintAmt", help: "Tints shadowed / ambient-only areas cool blue for a sunny-day contrast look. 0 = neutral." },
  ...
  { id: "aoStr",        label: "AMBIENT OCCLUSION", group: "SHADOWS", min: 0, max: 9, step: 0.002, def: 1.0, help: "Crease/contact darkening (SSAO). Floor 0 = off (already the minimum); above 1 pushes creases toward crushed black." },
  ...
  { id: "contactStr",   label: "CONTACT SHADOW",  group: "SHADOWS", min: 0, max: 9, step: 0.002, def: 1.0, help: "Grounding shadow under the car/props where the sun map can't reach." },
  { id: "ambContactDark", label: "AMBIENT CONTACT DARK", group: "SHADOWS", min: 0, max: 9, step: 0.002, def: 1.0, u: "uAmbContactDark", help: "How much the ambient-occlusion term darkens the AMBIENT fill in creases/contact points (distinct from AMBIENT OCCLUSION, which darkens the whole shaded result). 1 = as-shipped (up to 12% ambient drop in deep creases), 0 = ambient fill is untouched by AO (flatter, brighter corners), higher = crushes contact points toward black. WebGL2 renderer only." },
  ...
  { id: "lampLevel",    label: "LAMP LEVEL",      group: "LAMPS", section: "POOLS", min: 0.001, max: 4.5, step: 0.001, def: 0.26, help: "Overall lamp brightness ceiling (on top of the twilight ramp). Applies to every track lamp — street posts and flood banks alike." },
  { id: "lampDensity",  label: "LAMP DENSITY",    group: "LAMPS", section: "POOLS", min: 0.1, max: 7.5, step: 0.005, def: 1.0, rebuild: true, help: "How densely lamps are placed along the track (1 = shipped ~22 m stride). Higher = more poles/pools per kilometre; lower = sparser. Light pools update live; pole geometry refreshes on the next track load. Distinct from LAMP COUNT (how many nearest lamps the shader keeps)." },
  ...
  { id: "poolEnergy",   label: "POOL ENERGY",     group: "LAMPS", section: "POOLS", min: 0.008, max: 6, step: 0.001, def: 0.55, rebuild: true, help: "Per-lamp pool luminance scale (physical energy per fixture). min/step chosen so the grid lands on every hundredth: at the old min 0.05 / step 0.02 the notches fell on ODD hundredths, so the max was unreachable (topped out at 1.99) and 22 shipped presets sat between notches." },
  { id: "lampRadiusMul",label: "POOL RADIUS",     group: "LAMPS", section: "POOLS", min: 0.06, max: 9, step: 0.002, def: 1.0,  rebuild: true, help: "Reach of each lamp pool. Too small and the far pool corner dies." },
  ...
  { id: "glareStr",     label: "LENS GLARE",      group: "LAMPS", section: "POOLS", min: 0, max: 4.5, step: 0.001, def: 0.12, help: "Lens-halo billboard strength at every active lamp." },
  { id: "lampTemp",     label: "LAMP TEMPERATURE",group: "LAMPS", section: "POOLS", min: -6, max: 6, step: 0.001, def: 0.0, fmt: "signed", help: "White-balance of ALL track lamps (street posts and flood banks). − warms toward sodium/amber, + cools toward LED/broadcast white. Layers over each lamp's own colour. Unclamped mix, widened symmetrically like the other white-balance knobs." },
  ...
  { id: "beamCone",     label: "BEAM CONE WIDTH", group: "LAMPS", section: "POOLS", min: 0.08, max: 6.6, step: 0.001, def: 1.0, rebuild: true, help: "Width of every lamp's illuminated cone. Wider = softer spread, narrower = tight hotspots." },
  { id: "lampWallSpill", label: "LAMP WALL SPILL", group: "LAMPS", section: "POOLS", min: 0, max: 9, step: 0.002, def: 1.0, u: "uLampWallSpill", help: "How much a lamp's light spills onto surfaces OUTSIDE its beam cone — the dim wash on walls/kerbs beside the lit pool (before the bright in-beam term takes over). 1 = as-shipped (a touch stronger on wet nights), 0 = the light is confined hard to its cone with pitch-black surroundings, higher = a broad ambient spill that lifts the whole surround. WebGL2 renderer only." },
  // Twilight ramp, warm-up, and the per-frame nearest-N cull
  { id: "twilightFloor", label: "TWILIGHT FLOOR",  group: "LAMPS", section: "BEHAVIOUR", min: 0.01, max: 1, step: 0.005, def: 0.30, help: "Minimum lamp level at bright dawn/dusk while the sun is still up. Low = lamps stay dim until the sun drops; high = pools are already strong under a lit twilight sky. Only affects dawn/dusk sessions." },
  { id: "twilightRamp",  label: "TWILIGHT RAMP",   group: "LAMPS", section: "BEHAVIOUR", min: 0.4, max: 42, step: 0.05, def: 6, help: "How fast lamps climb from the twilight floor to full as the sun sets (steepness vs sun elevation). Higher = a quicker snap to full night lighting; lower = a long, gradual build. Dawn/dusk only." },
  { id: "twilightWarm",  label: "TWILIGHT WARMTH", group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 9, step: 0.005, def: 1.0, help: "How amber the lamps glow at twilight (the 'just switched on' sodium warmth that fades toward neutral by deep night). 0 = neutral lamps at dusk, 1 = as-shipped, higher = strong orange twilight cast. Dawn/dusk only." },
  ...
  { id: "lampWarmupWarm",label: "WARM-UP WARMTH",  group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 9, step: 0.005, def: 1.0, help: "How orange a freshly-struck lamp glows before settling to its true colour (strike-warmth amount). 0 = strikes at final colour, 1 = as-shipped sodium-warm start, higher = a strong amber ignition." },
  ...
  { id: "bloomSpread",  label: "BLOOM SPREAD",    group: "NIGHT GLOW & BLOOM", min: 0.06, max: 12, step: 0.001, def: 1.0, u: "uSpread", help: "Halo WIDTH, independent of amount. Higher = wider, dreamier glow; lower = tight core." },
  ...
  // ── ATMOSPHERE ──
  { id: "fogDensityMul",label: "FOG DENSITY",     group: "ATMOSPHERE", min: 0, max: 15, step: 0.001, def: 1.0, u: "uFogDensity", help: "Scales atmospheric haze depth — how fast distance fades into fog. 1 = as-shipped." },
  { id: "fogHeight",    label: "FOG HEIGHT FALLOFF", group: "ATMOSPHERE", min: 0, max: 0.2, step: 0.0001, def: 0.018, u: "uFogHeight", help: "How fast fog thins with altitude. 0 = uniform wall; higher = fog pools low and clears overhead." },
  { id: "fogTint",      label: "FOG WARM / COOL", group: "ATMOSPHERE", min: -6, max: 6, step: 0.001, def: 0.0, u: "uFogTint", fmt: "signed", help: "White-balance of the distance haze. + warm (amber/dusty), − cool (blue/overcast). Unclamped mix in-shader, so widened like the other white-balance knobs." },
  { id: "fogColorSat",  label: "FOG COLOUR SATURATION", group: "ATMOSPHERE", min: 0, max: 9, step: 0.002, def: 1.0, help: "How colourful vs neutral-grey the base fog/haze colour is. 1 = as-shipped, 0 = a flat achromatic grey haze, higher = a vivid coloured murk (e.g. the amber dawn / blue overcast fog reads much stronger). Distinct from FOG WARM / COOL (which shifts the hue) — this sets how far the fog sits from grey. Applied to the condition-derived fog colour before the warm/cool balance." },
  { id: "mistDensity",  label: "GROUND MIST",     group: "ATMOSPHERE", min: 0, max: 12, step: 0.001, def: 1.0, u: "uGroundMist", help: "Amount of low-lying drifting ground mist (dawn/humid/fog). 0 = none, higher = thick rolling bank." },
  { id: "mistHeight",   label: "MIST HEIGHT BAND",group: "ATMOSPHERE", min: 0.008, max: 3.6, step: 0.001, def: 0.30, u: "uMistHeight", help: "How tall the ground-mist layer stands. Low = ankle fog, high = deep bank up to the eyeline." },
  ...
  { id: "lampFogHaze",  label: "FOG GLOW HAZE",   group: "ATMOSPHERE", min: 0, max: 7.5, step: 0.001, def: 0.6,  help: "Extra lamp-fog glow added as ground mist / fog weather thickens." },
  ...
  { id: "hazeWetShare", label: "WET HAZE SHARE",  group: "ATMOSPHERE", min: 0, max: 4.5, step: 0.001, def: 0.22, help: "How much a WET road feeds the volumetric haze that god-rays and lamp-beams scatter through (spray/humidity hanging in the air). 0.22 = as-shipped, 0 = wet roads add no airborne haze (shafts only respond to mist/cloud), higher = rain sessions fill with thick light-catching spray. Separate from GROUND MIST and the on-road wetness sheen." },
  { id: "hazeCloudShare", label: "CLOUD HAZE SHARE", group: "ATMOSPHERE", min: 0, max: 4.5, step: 0.001, def: 0.12, help: "How much CLOUD COVER feeds the volumetric haze that god-rays and lamp-beams scatter through (an overcast sky's diffuse murk). 0.12 = as-shipped, 0 = cloud adds no airborne haze, higher = heavy overcast fills the air with light-catching haze. Separate from cloud SHADOWS and the sky dome cover." },
  { id: "fogClip",      label: "FOG GLOW CLIP",   group: "ATMOSPHERE", min: 0, max: 7.5, step: 0.002, def: 0.7,  u: "uLampFogClip", help: "Soft shoulder stopping lamp clusters pushing the fog wall to white." },
  { id: "fogSunCore",   label: "FOG SUN CORE",    group: "ATMOSPHERE", min: 0, max: 9, step: 0.002, def: 0.6, u: "uFogSunCore", help: "The tight hot bloom of the sun burning through haze — a concentrated bright core right at the sun's position in the fog (on top of the broad sun-glow scatter). 0.6 = as-shipped, 0 = no hot core (just the wide glow), higher = a fierce sun burning through the murk. Best seen at low dawn/dusk sun through fog." },
  { id: "overcastFogMul", label: "OVERCAST FOG BOOST", group: "ATMOSPHERE", min: 0.2, max: 18, step: 0.005, def: 1.7, help: "How much thicker the atmospheric haze gets in OVERCAST weather (multiplies the base fog density). 1.7 = as-shipped, 1 = overcast adds no extra haze (clear-day fog depth), higher = a soupier grey murk. Only affects overcast sessions; FOG BOOST does the same for fog weather." },
  { id: "fogWxMul",     label: "FOG BOOST",       group: "ATMOSPHERE", min: 0.2, max: 24, step: 0.005, def: 3.0, help: "How much thicker the atmospheric haze gets in FOG weather (multiplies the base fog density). 3.0 = as-shipped, 1 = fog weather adds no extra haze, higher = near-zero-visibility pea-soup. Only affects fog sessions; OVERCAST FOG BOOST does the same for overcast." },
  { id: "lampVolBase",  label: "BEAMS (CLEAR)",   group: "ATMOSPHERE", min: 0, max: 0.8, step: 0.0005, def: 0.05, help: "Volumetric lamp-beam strength in clear night air. Mobile tier renders no beams regardless (the volumetric pass is shed for GPU headroom)." },
  { id: "lampVolHaze",  label: "BEAMS (HAZE)",    group: "ATMOSPHERE", min: 0, max: 7.5, step: 0.005, def: 0.65, help: "How much haze/rain swells the lamp beams." },
  { id: "lampVolCap",   label: "BEAM CEILING",    group: "ATMOSPHERE", min: 0, max: 2.25, step: 0.001, def: 0.70, help: "Hard cap on volumetric beam strength." },
  { id: "renderDistMul", label: "RENDER DISTANCE", group: "ATMOSPHERE", min: 0.1, max: 6, step: 0.005, def: 1.0, help: "Scales how far the camera can see — the far clipping plane AND the scenery draw-distance cull move together. 1 = as-shipped (900 m). Higher reveals more distant track/scenery at a real GPU cost (more chunks drawn every frame); lower saves performance on weak devices. Fog still fades distant geometry out visually regardless of this setting." },
  // ── REFLECTIONS ──
  { id: "ssrWetMul",    label: "WET MIRROR",      group: "ROAD & REFLECTIONS", min: 0, max: 7.5, step: 0.001, def: 1.0,  help: "Wet-road scene-mirror strength (scales the wetness ramp)." },
  { id: "ssrDryNight",  label: "DRY NIGHT SHEEN", group: "ROAD & REFLECTIONS", min: 0, max: 1, step: 0.0005, def: 0.08, help: "Dry tarmac lamp/neon sheen at night." },
  { id: "ssrDryDay",    label: "DRY DAY SHEEN",   group: "ROAD & REFLECTIONS", min: 0, max: 0.6, step: 0.0005, def: 0.07, help: "Faint tower-and-sky mirror on dry day tarmac." },
  { id: "roadRough",    label: "TARMAC ROUGHNESS",group: "ROAD & REFLECTIONS", min: 0.01, max: 1.8, step: 0.001, def: 1.0, help: "Scales dry-tarmac roughness — lower = glossier asphalt with a tighter sun streak. Bounds now track the shader's actual roughness clamp (0.85×value clamped to 0.04..1): below ~0.05 or above ~1.2 the old wider slider was just dead range that did nothing extra." },
  { id: "surfDetail",   label: "SURFACE DETAIL",  group: "ROAD & REFLECTIONS", min: 0, max: 10.5, step: 0.002, def: 1.0, help: "Road/terrain procedural grain + micro-normal relief (aggregate, patches, cracks). 0 = flat." },
  ...
  { id: "ssrThick",     label: "SSR THICKNESS",   group: "ROAD & REFLECTIONS", min: 0.004, max: 15, step: 0.002, def: 0.20, u: "uSsrThick", help: "Depth tolerance for a wet-road reflection hit. Lower = crisper but more gaps; higher = fewer holes, more smear. Ceiling raised to 5 — the hit-test's own far-plane reject means values above that would be a no-op." },
  { id: "wetDark",      label: "WET ROAD DARKEN", group: "ROAD & REFLECTIONS", min: 0, max: 6, step: 0.001, def: 1.0, u: "uWetDark", help: "How much darker wet asphalt reads (water absorption). Independent of the wetness amount." },
  { id: "windowSunFlash", label: "WINDOW SUN FLASH", group: "ROAD & REFLECTIONS", min: 0, max: 9, step: 0.002, def: 1.0, u: "uWindowSunFlash", help: "The mirror-bright flash of the sun off building glass — the hot specular pinpoint that slides across reflective façades as you pass. 1 = as-shipped, 0 = flat matte glass with no sun hotspot, higher = a blazing glare off every window. Day-only; scales with how directly the glass faces the sun. WebGL2 renderer only." },
  { id: "skyRimGlow",   label: "SKY RIM GLOW",    group: "ROAD & REFLECTIONS", min: 0, max: 9, step: 0.002, def: 1.0, u: "uSkyRimGlow", help: "Fresnel rim light along the grazing edges of reflective surfaces (glass, wet bodywork) picking up the sky — the bright sheen where a surface curves away from you. 1 = as-shipped, 0 = no edge sheen (matte silhouettes), higher = a strong glassy rim halo. Strongest on smooth surfaces at grazing angles. WebGL2 renderer only." },
  // ── CAR ──
  { id: "carReflect",   label: "CAR REFLECTION",  group: "CAR", min: 0, max: 7.5, step: 0.001, def: 0.05, u: "uCarReflect", help: "How strongly the world (track, sky, lights) mirrors on the car bodywork." },
  ...
  { id: "carGloss",     label: "PAINT GLOSS",     group: "CAR", min: 0, max: 2.4, step: 0.001, def: 1.0,  u: "uCarGloss", help: "Sharpness of the paint's highlights & reflections. Higher = glassier (lower roughness). Range now tracks the SSR-streak formula's own clamp window (roughly 0..1.4): below 0 or above ~1.4 the old slider was pushing a value the shader already saturates on." },
  { id: "carSpecular",  label: "PAINT SPECULAR",  group: "CAR", min: 0, max: 10.5, step: 0.002, def: 1.0,  help: "Brightness of the specular highlight rolling over the bodywork." },
  { id: "carClearcoat", label: "CLEARCOAT",       group: "CAR", min: 0, max: 10.5, step: 0.001, def: 0.05, help: "Lacquer coat that catches crisp sun / lamp glints over the base colour." },
  { id: "carMetal",     label: "PAINT METALNESS", group: "CAR", min: 0, max: 15, step: 0.002, def: 1.0,  help: "How metallic the paint reads — reflection tint and grazing falloff." },
  { id: "carGlow",      label: "BODY GLOW",       group: "CAR", min: 0, max: 15, step: 0.002, def: 1.0,  help: "Self-lit body glow after dark (only the night / wet liveries carry it)." },
  { id: "tailLightMul", label: "TAIL-LIGHT GLOW", group: "CAR", min: 0, max: 15, step: 0.005, def: 1.0, help: "Brightness of the red glow trailing nearby cars after dark." },
  { id: "brakeGlowMul", label: "BRAKE FLARE",     group: "CAR", min: 0, max: 9, step: 0.005, def: 1.0, help: "How hard the tail-light glow surges while a car is braking (scales the brake-heat flare on top of TAIL-LIGHT GLOW). 0 = steady tail light, no flare." },
  ...
  { id: "carSunGlint",  label: "PAINT SUN GLINT",  group: "CAR", min: 0, max: 120, step: 0.05, def: 12.0, u: "uCarSunGlint", help: "The mirror-bright pinpoint reflection of the sun off the car's clearcoat (the moving hotspot that slides over the bodywork). 12 = as-shipped, 0 = no sun hotspot on the paint, higher = a blazing HDR glint that blooms hard. Separate from PAINT SPECULAR (the broad highlight)." },
  { id: "carSparkle",   label: "METALLIC SPARKLE", group: "CAR", min: 0, max: 18, step: 0.005, def: 1.6, u: "uCarSparkle", help: "Gain of the metallic-flake glitter in the paint — the tiny individual flakes that flash as the camera moves. 1.6 = as-shipped, 0 = a flat non-metallic finish, higher = a denser, brighter sparkle field. Only affects metallic-flagged liveries." },
  // ── SKY & WEATHER ──
  { id: "cloudCover",   label: "CLOUD COVER",     group: "SKY & WEATHER", min: -3, max: 1.5, step: 0.001, def: 0.0, fmt: "signed", help: "Shifts cloud amount up/down from the weather default (also drives cloud shadows). 0 = as-shipped. Capped at ±1 — the underlying cloud amount is itself clamped to 0..1, so a full ±1 swing already overrides any base value to clear or overcast; more range would be a no-op." },
  ...
  { id: "cloudSpeed",   label: "CLOUD SPEED",     group: "SKY & WEATHER", min: 0, max: 24, step: 0.005, def: 1.0, u: "uCloudSpeed", help: "How fast clouds drift and evolve. 0 = frozen sky, higher = fast-moving weather." },
  { id: "starBright",   label: "STAR BRIGHTNESS", group: "SKY & WEATHER", min: 0, max: 12, step: 0.001, def: 1.0, u: "uStarBright", help: "Night star intensity. 0 = washed sky, higher = vivid starfield." },
  { id: "starDensity",  label: "STAR DENSITY",    group: "SKY & WEATHER", min: 0.04, max: 9, step: 0.01, def: 1.0, u: "uStarDensity", help: "How MANY stars fill the night sky (independent of STAR BRIGHTNESS, which sets how bright each one is). 1 = as-shipped, lower = a sparse sky, higher = a dense field. Capped so a huge value can't reject every cell to a blank sky." },
  { id: "skyGrad",      label: "SKY GRADIENT",    group: "SKY & WEATHER", min: 0.03, max: 3.6, step: 0.001, def: 0.35, u: "uSkyGrad", help: "Shape of the horizon→zenith sky gradient (the exponent). 0.35 = as-shipped (deep zenith blue reaching well down the sky); lower pulls the rich colour even lower for a moodier dome; higher washes the mid-sky pale and keeps saturation to the very top." },
  { id: "daySkyBlue",   label: "DAY SKY BLUE",    group: "SKY & WEATHER", min: 0, max: 7.5, step: 0.002, def: 1.0, u: "uDaySkyBlue", help: "Strength of the deep saturated-blue band pushed into the low/mid daytime sky so the gameplay sky strip isn't a flat pale wash. 0 = off (plain gradient), 1 = as-shipped, higher = a punchy azure band. Day-only; fades out under overcast and at dawn/dusk/night." },
  { id: "mieScatter",   label: "SKY SUN GLOW",    group: "SKY & WEATHER", min: 0, max: 12, step: 0.002, def: 1.0, u: "uMieScatter", help: "Mie forward-scatter glow around the sun — the bright haze bloom filling the sky in the sun's direction, strongest near the horizon. 1 = as-shipped, 0 = a clean sky with no glow, higher = a big hazy sun-halo washing across the upper sky." },
  { id: "cloudSilver",  label: "CLOUD SILVER LINING", group: "SKY & WEATHER", min: 0, max: 12, step: 0.002, def: 1.0, u: "uCloudSilver", help: "Backlit sun-facing cloud-edge glow (the bright silver/gold rim on clouds between you and the sun). 1 = as-shipped, 0 = flat matte clouds with no rim light, higher = blazing fire-lit edges. Most intense at golden hour." },
  { id: "coronaAureole",label: "SUN AUREOLE",     group: "SKY & WEATHER", min: 0, max: 12, step: 0.002, def: 1.0, u: "uCoronaAureole", help: "The broad soft halo of light around the sun disc (distinct from the tight inner ring and the sun disc itself). 1 = as-shipped, 0 = a bare sun disc with no surrounding glow, higher = a large radiant aureole." },
  { id: "sunDiscSize",  label: "SUN DISC SIZE",   group: "SKY & WEATHER", min: 0.06, max: 12, step: 0.005, def: 1.0, u: "uSunDiscSize", help: "Angular size of the sun disc in the sky. 1 = as-shipped (real-sun size), larger = a bigger dramatic sun (sunset-photo look), smaller = a tighter pinpoint. Scales the disc radius without touching the corona or aureole." },
  { id: "sunCorona",    label: "SUN CORONA RING", group: "SKY & WEATHER", min: 0, max: 12, step: 0.002, def: 1.0, u: "uSunCorona", help: "The tight bright inner ring hugging the sun disc (distinct from the broad SUN AUREOLE and the disc itself). 1 = as-shipped, 0 = no ring (bare disc + aureole), higher = a fierce hot halo right at the disc edge." },
  { id: "sunSquash",    label: "SUN HORIZON SQUASH", group: "SKY & WEATHER", min: 0, max: 9, step: 0.002, def: 1.0, u: "uSunSquash", help: "How much the sun disc flattens vertically as it nears the horizon (atmospheric-refraction oval, strongest at golden hour). 1 = as-shipped, 0 = a perfectly round sun at any height, higher = a strongly squashed sunset oval." },
  { id: "starSize",     label: "STAR SIZE",       group: "SKY & WEATHER", min: 0.04, max: 12, step: 0.002, def: 1.0, u: "uStarSize", help: "Angular size of each night star point. 1 = as-shipped, lower = fine pinpricks, higher = fatter stars. Independent of STAR BRIGHTNESS (how bright) and STAR DENSITY (how many)." },
  { id: "starTwinkle",  label: "STAR TWINKLE",    group: "SKY & WEATHER", min: 0, max: 12, step: 0.002, def: 1.0, u: "uStarTwinkle", help: "How much the stars pulse in brightness over time (twinkle amplitude). 1 = as-shipped, 0 = rock-steady stars, higher = a lively shimmering sky. Needs CLOUD SPEED / animation running." },
  ...
  { id: "wetness",      label: "WETNESS",         group: "SKY & WEATHER", min: -0.1, max: 1, step: 0.005, def: -0.05, fmt: "auto", help: "Override the road wetness ramp (AUTO = follow weather; ramps in over a few seconds after a weather flip). Already an exact 0..1 ramp plus its AUTO sentinel — left untouched." },
  ...
  { id: "rainStreak",   label: "RAIN STREAK LEN", group: "SKY & WEATHER", min: 0.04, max: 12, step: 0.002, def: 1.0, reinitRain: true, help: "Length of rain streaks — short spits vs long driving streaks." },
  { id: "rainSpeed",    label: "RAIN FALL SPEED", group: "SKY & WEATHER", min: 0.04, max: 9, step: 0.002, def: 1.0, reinitRain: true, help: "How fast the rain streaks fall down the screen (base velocity of each drop before the speed-reactive shear). 1 = as-shipped, lower = a slow drifting drizzle, higher = a hard driving downpour. Drizzle (WET, no storm) still falls at 60% of this." },
  ...
  { id: "drizzleLen",   label: "DRIZZLE STREAK",   group: "SKY & WEATHER", min: 0, max: 4.5, step: 0.002, def: 0.50, reinitRain: true, help: "Length of DRIZZLE streaks vs a full storm. 0.50 = as-shipped (half-length spits), lower = fine mist dots, higher = longer streaks approaching storm rain. Only affects WET (non-raining) sessions." },
  { id: "drizzleSpeed", label: "DRIZZLE FALL SPEED", group: "SKY & WEATHER", min: 0, max: 4.5, step: 0.002, def: 0.60, reinitRain: true, help: "How fast DRIZZLE falls vs a full storm. 0.60 = as-shipped (a slow soft fall), lower = a drifting haze, higher = a brisker fall approaching storm speed. Only affects WET (non-raining) sessions." },
  { id: "rainOpacity",  label: "RAIN OPACITY",    group: "SKY & WEATHER", min: 0, max: 6, step: 0.002, def: 1.0, help: "How visible the falling rain is (alpha of the streak layer). 1 = as-shipped, 0 = invisible rain, higher = a dense, near-opaque sheet. Drizzle draws fainter than a full storm at any setting." },
  { id: "rainWind",     label: "RAIN WIND",       group: "SKY & WEATHER", min: -6, max: 6, step: 0.001, def: 0.18, fmt: "signed", help: "Horizontal wind slant on the rain (angle of the streaks). Unclamped drift term, so widened like the white-balance knobs — high settings rake the rain almost horizontal." },
  { id: "rainShearWind", label: "RAIN SPEED SLANT", group: "SKY & WEATHER", min: 0, max: 9, step: 0.002, def: 0.9, help: "How much the rain slants toward you as the car speeds up (apparent-wind shear added to RAIN WIND, ramping in to full by ~90 m/s). 0.9 = as-shipped, 0 = rain falls at the same angle standing still or flat-out, higher = the streaks rake hard sideways at speed." },
  { id: "rainShearLen", label: "RAIN SPEED STRETCH", group: "SKY & WEATHER", min: 0, max: 18, step: 0.005, def: 2.0, help: "How much the rain streaks stretch out as the car speeds up (motion-streak elongation, ramping to full by ~90 m/s). 2.0 = as-shipped (up to 3× length flat-out), 0 = streaks keep their standing length at any speed, higher = long driving streaks at speed." },
  { id: "lightning",    label: "LIGHTNING FREQ",  group: "SKY & WEATHER", min: 0, max: 18, step: 0.001, def: 1.0, help: "Storm lightning strike rate. 0 = off, higher = more frequent flashes." },
  { id: "lightningFlash", label: "LIGHTNING FLASH", group: "SKY & WEATHER", min: 0, max: 9, step: 0.002, def: 1.0, help: "How brightly a lightning strike bleaches the scene (scales the ambient spike + exposure lift together). 1 = as-shipped, 0 = thunder only with no visible flash, higher = a blinding strobe. LIGHTNING FREQ sets how often; this sets how hard." },
  { id: "lightningDecay", label: "LIGHTNING DECAY", group: "SKY & WEATHER", min: 0.4, max: 60, step: 0.05, def: 8.0, help: "How fast a lightning flash fades after the strike (exponential decay rate). 8 = as-shipped (~0.9 s glow), lower = a lingering sky-wide flicker, higher = a snappy instant flash." },
  { id: "weatherSunMute", label: "WEATHER SUN MUTE", group: "SKY & WEATHER", min: 0, max: 6, step: 0.002, def: 1.0, help: "How much wet/rain/overcast/fog weather dims the direct sun. 0 = weather doesn't mute the sun at all (bright, hard light through the storm); 1 = as-shipped; higher = deeper flat-grey murk. No effect in clear/dry weather." },
  // ── IMAGE & COLOUR ──
  ...
  { id: "contrast",     label: "CONTRAST",        group: "IMAGE & COLOUR", min: 0.1, max: 9, step: 0.001, def: 1.12, u: "uContrast", help: "Midtone-darkening gamma. Higher = deeper, filmic shadows; lower = flatter and brighter. Ceiling pushed further than the floor since the ask is specifically for harder shadow crush." },
  { id: "blacks",     label: "BLACKS",     group: "IMAGE & COLOUR", section: "TONAL RANGE", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Exposure of the deepest detail near black. Positive reveals it; negative crushes it. 0 is neutral." },
  { id: "shadows",    label: "SHADOWS",    group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Exposure of dark asphalt, tyres and unlit bodywork above the black point. 0 is neutral." },
  { id: "midtones",   label: "MIDTONES",   group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Exposure around middle grey: most paint, grass and track detail. 0 is neutral." },
  { id: "highlights", label: "HIGHLIGHTS", group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Exposure of bright bodywork, clouds and lamp pools below peak white. 0 is neutral." },
  { id: "whites",     label: "WHITES",     group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Exposure of the brightest HDR values feeding the ACES shoulder. Negative protects peaks; positive drives them harder. 0 is neutral." },
  { id: "toe",        label: "TOE",        group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Shapes the transition out of black. Positive deepens the toe; negative lifts it. 0 is neutral." },
  { id: "shoulder",   label: "SHOULDER",   group: "IMAGE & COLOUR", min: -3, max: 3, step: 0.001, def: 0, fmt: "signed", help: "Shapes highlight compression before ACES. Positive protects bright detail; negative expands it. 0 is neutral." },

  { id: "liftR", label: "LIFT · RED", group: "IMAGE & COLOUR", section: "RGB LIFT / GAMMA / GAIN", min: -0.3, max: 0.15, step: 0.00025, def: 0, fmt: "signed", help: "Red offset at the black end of the RGB grade. 0 is neutral." },
  { id: "liftG", label: "LIFT · GREEN", group: "IMAGE & COLOUR", min: -0.3, max: 0.15, step: 0.00025, def: 0, fmt: "signed", help: "Green offset at the black end of the RGB grade. 0 is neutral." },
  { id: "liftB", label: "LIFT · BLUE", group: "IMAGE & COLOUR", min: -0.3, max: 0.15, step: 0.00025, def: 0, fmt: "signed", help: "Blue offset at the black end of the RGB grade. 0 is neutral." },
  { id: "gammaR", label: "GAMMA · RED", group: "IMAGE & COLOUR", min: 0.1, max: 6, step: 0.001, def: 1, help: "Red-channel midpoint control. Above 1 lifts red midtones; below 1 deepens them. 1 is neutral." },
  { id: "gammaG", label: "GAMMA · GREEN", group: "IMAGE & COLOUR", min: 0.1, max: 6, step: 0.001, def: 1, help: "Green-channel midpoint control. Above 1 lifts green midtones; below 1 deepens them. 1 is neutral." },
  { id: "gammaB", label: "GAMMA · BLUE", group: "IMAGE & COLOUR", min: 0.1, max: 6, step: 0.001, def: 1, help: "Blue-channel midpoint control. Above 1 lifts blue midtones; below 1 deepens them. 1 is neutral." },
  { id: "gainR", label: "GAIN · RED", group: "IMAGE & COLOUR", min: 0.1, max: 4.5, step: 0.001, def: 1, help: "Red scale at the white end of the RGB grade. 1 is neutral." },
  { id: "gainG", label: "GAIN · GREEN", group: "IMAGE & COLOUR", min: 0.1, max: 4.5, step: 0.001, def: 1, help: "Green scale at the white end of the RGB grade. 1 is neutral." },
  { id: "gainB", label: "GAIN · BLUE", group: "IMAGE & COLOUR", min: 0.1, max: 4.5, step: 0.001, def: 1, help: "Blue scale at the white end of the RGB grade. 1 is neutral." },

  { id: "saturation",   label: "SATURATION",      group: "IMAGE & COLOUR", section: "COLOUR", min: 0, max: 9, step: 0.001, def: 1.0, u: "uSaturation", help: "Overall colour intensity. 0 = greyscale, 1 = as-shipped, >1 = punchier." },
  { id: "vibrance",     label: "VIBRANCE",        group: "IMAGE & COLOUR", min: 0, max: 4.5, step: 0.001, def: 0.20, u: "uVibrance", help: "Selective saturation — lifts dull/washed pixels (hazy sky, grass, tarmac) without over-cooking neon or kerbs." },
  { id: "tint",         label: "WARM / COOL",     group: "IMAGE & COLOUR", min: -6, max: 6, step: 0.001, def: 0.0, u: "uTint", fmt: "signed", help: "White-balance shift. + warms (amber, sunny), − cools (blue, overcast/night). Unclamped mix, widened symmetrically like the other white-balance knobs." },
  { id: "gradeStr",     label: "GRADE STRENGTH",  group: "IMAGE & COLOUR", min: 0, max: 12, step: 0.001, def: 1.0, help: "Cinematic split-tone amount (teal shadows / warm highlights). 0 = neutral, higher = stronger film look." },
  ...
  { id: "flareMul",     label: "LENS FLARE",      group: "IMAGE & COLOUR", min: 0, max: 10.5, step: 0.001, def: 1.0, help: "Sun/lamp anamorphic streak + ghost strength. 0 = off, 1 = as-shipped." },
  { id: "flareStreak",  label: "FLARE STREAK",    group: "IMAGE & COLOUR", min: 0.4, max: 90, step: 0.05, def: 7.0, u: "uFlareStreak", help: "Width of the horizontal anamorphic sun-flare streak (its horizontal tightness). 7 = as-shipped; lower spreads it into a wide screen-spanning band of light, higher pulls it into a tight contained highlight hugging the sun. Pairs with LENS FLARE (which sets strength)." },
  { id: "flareStreak2", label: "FLARE CORE STREAK", group: "IMAGE & COLOUR", min: 0, max: 9, step: 0.002, def: 0.5, u: "uFlareStreak2", help: "Strength of the second thin hot-white core streak layered over the main FLARE STREAK (a tighter, brighter inner blade of the anamorphic flare). 0.5 = as-shipped, 0 = single-streak flare, higher = a piercing hot core line. Pairs with LENS FLARE (strength) and FLARE STREAK (width)." },
  { id: "sharpen",      label: "SHARPEN",         group: "IMAGE & COLOUR", min: 0, max: 6, step: 0.002, def: 0.0, u: "uSharpen", help: "Crispness recovered after FXAA — counteracts softening on kerbs, wires and distant detail." },
  { id: "speedBlur",    label: "SPEED BLUR",      group: "IMAGE & COLOUR", min: 0, max: 6, step: 0.002, def: 0.0, u: "uSpeedBlur", help: "Radial blur from screen centre that grows with car speed — a velocity cue at high speed." },

  // ── FX ── transient particle effects (js/game/particles.js pool)
  { id: "particleMul",  label: "PARTICLE FX",     group: "FX", min: 0, max: 6, step: 0.005, def: 1.0, help: "Transient particle amount — tyre smoke, collision sparks, gravel kickup and rain spray. 0 = off, 1 = as-shipped, 2 = double the emission rate." },
];
