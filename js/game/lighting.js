...
const TUNE_DEFS = [
  // ── SUN & MOON ──
  { id: "keyMul",       label: "KEY LIGHT (SUN)", group: "SUN & MOON", min: 0,   max: 4,  step: 0.01,  def: 1.0,  u: "uKeyMul", help: "Direct sun/moon intensity — diffuse + speculars + shadows. Ambient, fog and sky reflection are untouched, so the scene stays coherent when dimmed. Floor stays 0 (already fully off); headroom is on the bright end." },
  { id: "sunTemp",      label: "SUN / MOON WARMTH", group: "SUN & MOON", min: -2, max: 2, step: 0.01, def: 0.0, fmt: "signed", help: "White-balance of the direct key light (sun by day, moonlight at night). − warm sunrise/sodium, + cool overcast/moonlight. The tint math is a plain unclamped mix, so both ends scale evenly past the old ±1." },
  ...
  { id: "moonBright",   label: "MOON BRIGHTNESS", group: "SUN & MOON", min: 0, max: 3, step: 0.01, def: 1.0, help: "Moon disc/halo + its soft blue fill on the night sky." },
  { id: "grMul",        label: "SUN GOD-RAYS",    group: "SUN & MOON", min: 0, max: 4, step: 0.01, def: 1.0,  help: "Volumetric sun-shaft strength (dawn/dusk drama)." },
  ...
  // ── AMBIENT & BOUNCE ──
  { id: "ambientMul",   label: "AMBIENT FILL",    group: "AMBIENT & BOUNCE", min: 0, max: 4,    step: 0.01,  def: 1.0,  help: "Hemisphere ambient multiplier — the shadow/unlit fill and night readability floor. Floor is true 0 now (was 0.3): with no direct light in a shadow, 0 ambient means that shadow reads pure black — the actual 'crushed shadow' extreme, not just 'dim'." },
  { id: "ambTemp",      label: "AMBIENT WARMTH",  group: "AMBIENT & BOUNCE", min: -2, max: 2, step: 0.01, def: 0.0, fmt: "signed", help: "White-balance of the hemisphere fill (shadow/unlit areas). − warm bounce, + cool sky fill. Unclamped mix, so widened symmetrically like the other white-balance knobs." },
  ...
  { id: "bleedMul",     label: "VALLEY BLEED",    group: "LAMPS", section: "POOLS", min: 0,    max: 5,   step: 0.05,  def: 1.0,  rebuild: true, help: "Out-of-beam light floor — lifts the dark valleys between pools. Floor stays 0 (pitch-dark valleys already the minimum); headroom is on the bright side." },
  { id: "glareStr",     label: "LENS GLARE",      group: "LAMPS", section: "POOLS", min: 0,    max: 1.5, step: 0.01, def: 0.12, help: "Lens-halo billboard strength at every active lamp." },
  { id: "lampTemp",     label: "LAMP TEMPERATURE",group: "LAMPS", section: "POOLS", min: -2, max: 2, step: 0.01, def: 0.0, fmt: "signed", help: "White-balance of ALL track lamps (street posts and flood banks). − warms toward sodium/amber, + cools toward LED/broadcast white. Layers over each lamp's own colour. Unclamped mix, widened symmetrically like the other white-balance knobs." },
  { id: "lampFlicker",  label: "LAMP FLICKER",    group: "LAMPS", section: "POOLS", min: 0, max: 0.6, step: 0.005, def: 0.10, help: "How much aging lamps pulse. 0 = rock-steady, higher = strong buzz on the odd tube." },
  { id: "beamCone",     label: "BEAM CONE WIDTH", group: "LAMPS", section: "POOLS", min: 0.4, max: 2.2, step: 0.01, def: 1.0, rebuild: true, help: "Width of every lamp's illuminated cone. Wider = softer spread, narrower = tight hotspots." },
  { id: "lampWallSpill", label: "LAMP WALL SPILL", group: "LAMPS", section: "POOLS", min: 0, max: 3, step: 0.02, def: 1.0, u: "uLampWallSpill", help: "How much a lamp's light spills onto surfaces OUTSIDE its beam cone — the dim wash on walls/kerbs beside the lit pool (before the bright in-beam term takes over). 1 = as-shipped (a touch stronger on wet nights), 0 = the light is confined hard to its cone with pitch-black surroundings, higher = a broad ambient spill that lifts the whole surround. WebGL2 renderer only." },
  // Twilight ramp, warm-up, and the per-frame nearest-N cull
  { id: "twilightFloor", label: "TWILIGHT FLOOR",  group: "LAMPS", section: "BEHAVIOUR", min: 0.05, max: 1, step: 0.05, def: 0.30, help: "Minimum lamp level at bright dawn/dusk while the sun is still up. Low = lamps stay dim until the sun drops; high = pools are already strong under a lit twilight sky. Only affects dawn/dusk sessions." },
  { id: "twilightRamp",  label: "TWILIGHT RAMP",   group: "LAMPS", section: "BEHAVIOUR", min: 2, max: 14, step: 0.5, def: 6, help: "How fast lamps climb from the twilight floor to full as the sun sets (steepness vs sun elevation). Higher = a quicker snap to full night lighting; lower = a long, gradual build. Dawn/dusk only." },
  { id: "twilightWarm",  label: "TWILIGHT WARMTH", group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 3, step: 0.05, def: 1.0, help: "How amber the lamps glow at twilight (the 'just switched on' sodium warmth that fades toward neutral by deep night). 0 = neutral lamps at dusk, 1 = as-shipped, higher = strong orange twilight cast. Dawn/dusk only." },
  { id: "lampWarmup",    label: "LAMP WARM-UP",    group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 4, step: 0.1, def: 1.0, help: "How long lamps take to ramp from a dim sodium-orange strike to full cool brightness when they switch on (scales the ~4-8 s warm-up). 0 = instant full brightness, 1 = as-shipped, higher = a long, staggered warm-up glow." },
  { id: "lampWarmupDim", label: "WARM-UP DIP",     group: "LAMPS", section: "BEHAVIOUR", min: 0, max: 0.9, step: 0.05, def: 0.30, help: "How dim a freshly-struck lamp starts before warming to full (depth of the strike dip). 0 = lamps strike at full brightness, 0.30 = as-shipped (start at 70%), higher = a deeper cold start." },
  ...
  { id: "glowAmp",      label: "EMISSIVE GLOW",   group: "NIGHT GLOW & BLOOM", min: 0.2,  max: 6,    step: 0.01,   def: 2.3,  u: "uGlowAmp", help: "HDR push for windows / lenses / neon — roughly half the night frame energy." },
  ...
  { id: "cityGlowMul",  label: "CITY SKYGLOW",    group: "NIGHT GLOW & BLOOM", min: 0, max: 5,   step: 0.01,  def: 1.0, help: "Light-pollution dome hugging the horizon over lit circuits/cities." },
  ...
  { id: "bloomMul",     label: "BLOOM AMOUNT",    group: "NIGHT GLOW & BLOOM", min: 0,    max: 4,    step: 0.01,  def: 1.0,  help: "Halo strength around bright HDR sources (lamps, neon, windows)." },
  { id: "bloomSpread",  label: "BLOOM SPREAD",    group: "NIGHT GLOW & BLOOM", min: 0.3, max: 4, step: 0.01, def: 1.0, u: "uSpread", help: "Halo WIDTH, independent of amount. Higher = wider, dreamier glow; lower = tight core." },
  ...
  // ── ATMOSPHERE ──
  { id: "fogDensityMul",label: "FOG DENSITY",     group: "ATMOSPHERE", min: 0, max: 5, step: 0.01, def: 1.0, u: "uFogDensity", help: "Scales atmospheric haze depth — how fast distance fades into fog. 1 = as-shipped." },
  ...
  { id: "fogTint",      label: "FOG WARM / COOL", group: "ATMOSPHERE", min: -2, max: 2, step: 0.01, def: 0.0, u: "uFogTint", fmt: "signed", help: "White-balance of the distance haze. + warm (amber/dusty), − cool (blue/overcast). Unclamped mix in-shader, so widened like the other white-balance knobs." },
  ...
  { id: "mistDensity",  label: "GROUND MIST",     group: "ATMOSPHERE", min: 0, max: 4, step: 0.01, def: 1.0, u: "uGroundMist", help: "Amount of low-lying drifting ground mist (dawn/humid/fog). 0 = none, higher = thick rolling bank." },
  ...
  { id: "lampFogBase",  label: "FOG GLOW BASE",   group: "ATMOSPHERE", min: 0, max: 0.9,   step: 0.01, def: 0.45, help: "How strongly lamps tint the distant fog wall on a clear night. Tops out at 0.9 because its only consumer sums it with FOG GLOW HAZE under a Math.min(0.9, ...) ceiling (js/game.js) — the slider used to run to 1.5, so its top 40% moved nothing at all." },
  { id: "lampFogHaze",  label: "FOG GLOW HAZE",   group: "ATMOSPHERE", min: 0, max: 2.5, step: 0.01, def: 0.6,  help: "Extra lamp-fog glow added as ground mist / fog weather thickens." },
  ...
  { id: "carGloss",     label: "PAINT GLOSS",     group: "CAR", min: 0, max: 1.6, step: 0.01, def: 1.0,  u: "uCarGloss", help: "Sharpness of the paint's highlights & reflections. Higher = glassier (lower roughness). Range now tracks the SSR-streak formula's own clamp window (roughly 0..1.4): below 0 or above ~1.4 the old slider was pushing a value the shader already saturates on." },
  ...
  { id: "starBright",   label: "STAR BRIGHTNESS", group: "SKY & WEATHER", min: 0, max: 4, step: 0.01, def: 1.0, u: "uStarBright", help: "Night star intensity. 0 = washed sky, higher = vivid starfield." },
  ...
  { id: "rainWind",     label: "RAIN WIND",       group: "SKY & WEATHER", min: -2, max: 2, step: 0.01, def: 0.18, fmt: "signed", help: "Horizontal wind slant on the rain (angle of the streaks). Unclamped drift term, so widened like the white-balance knobs — high settings rake the rain almost horizontal." },
  ...
  { id: "lightning",    label: "LIGHTNING FREQ",  group: "SKY & WEATHER", min: 0, max: 6, step: 0.01, def: 1.0, help: "Storm lightning strike rate. 0 = off, higher = more frequent flashes." },
  ...
  // ── IMAGE & COLOUR ──
  { id: "exposureMul",  label: "EXPOSURE",        group: "IMAGE & COLOUR", min: 0.1,  max: 3,  step: 0.01,  def: 1.0,  help: "Master brightness multiplier on the tone-map input (all times of day) — applied before the tone-mapper, so this is the most direct way to darken or blow out the whole frame. Floor dropped toward near-black; ceiling raised for a hard overexposed look." },
  { id: "contrast",     label: "CONTRAST",        group: "IMAGE & COLOUR", min: 0.5, max: 3, step: 0.01, def: 1.12, u: "uContrast", help: "Midtone-darkening gamma. Higher = deeper, filmic shadows; lower = flatter and brighter. Ceiling pushed further than the floor since the ask is specifically for harder shadow crush." },
  ...
  { id: "saturation",   label: "SATURATION",      group: "IMAGE & COLOUR", section: "COLOUR", min: 0, max: 3,   step: 0.01, def: 1.0, u: "uSaturation", help: "Overall colour intensity. 0 = greyscale, 1 = as-shipped, >1 = punchier." },
  ...
  { id: "tint",         label: "WARM / COOL",     group: "IMAGE & COLOUR", min: -2, max: 2, step: 0.01, def: 0.0, u: "uTint", fmt: "signed", help: "White-balance shift. + warms (amber, sunny), − cools (blue, overcast/night). Unclamped mix, widened symmetrically like the other white-balance knobs." },
  { id: "gradeStr",     label: "GRADE STRENGTH",  group: "IMAGE & COLOUR", min: 0, max: 4, step: 0.01, def: 1.0, help: "Cinematic split-tone amount (teal shadows / warm highlights). 0 = neutral, higher = stronger film look." },
  ...
  { id: "flareMul",     label: "LENS FLARE",      group: "IMAGE & COLOUR", min: 0, max: 3.5, step: 0.01, def: 1.0, help: "Sun/lamp anamorphic streak + ghost strength. 0 = off, 1 = as-shipped." },
