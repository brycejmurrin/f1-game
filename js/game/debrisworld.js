/* Apex 26 — Rapier side-world for render-only debris (adoption phases R0+R1,
   ...
   Loading: opt-in via localStorage apex26.debris = "1" or __apex.debris(true).
   On first enable the vendored @dimforge/rapier3d-compat 0.19.3 module
...
function create(ctx) {
  G = ctx;
  let opt = "0";
  try { opt = localStorage.getItem("apex26.debris") || "0"; } catch (e) {}
  if (opt === "1") setEnabled(true);   // opt-in flag: async load, never blocks boot
