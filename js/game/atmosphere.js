/* Apex 26 — session atmosphere for js/game.js: applyRaceSettings(), the
   ...
   table, plus the live weather switch and weather-arc (setWeatherLive /
   startWeatherArc / tickWeatherArc). Extracted verbatim; live state
   (frame, frameSky, track, weather, cloud/lightning timers, weatherArc)
   is reached through the ctx façade G handed to Atmosphere.create(G);
   stable helpers (clamp, satAdjust, weather predicates, applyLightTune)
   arrive by destructure. Consumes globals LightTune, V3, Tracks,
   Particles, GameAudio. Must load BEFORE js/game.js (see index.html). */
...
function setWeatherLive(w) {
  ...
  if (G.soundOn) { if (isRaining()) GameAudio.startRain(); else GameAudio.stopRain(); }
...
function startWeatherArc(from, to, dur) {
  ...
  G.weatherArc = { from, to, t: 0, dur: Math.max(1, dur || 60), seq: weatherArcSeq(from, to) };
  if (G.raceWeather !== from) setWeatherLive(from);
  return G.weatherArc;
}
function tickWeatherArc(dt) {
  if (!G.weatherArc) return;
  G.weatherArc.t += dt;
  const f = Math.min(1, G.weatherArc.t / G.weatherArc.dur);
  const seq = G.weatherArc.seq;
  const want = seq[Math.min(seq.length - 1, Math.floor(f * seq.length))];
  if (G.raceWeather !== want) setWeatherLive(want);
  if (f >= 1) {
    if (G.raceWeather !== G.weatherArc.to) setWeatherLive(G.weatherArc.to);
    G.weatherArc = null;   // arc complete — weather stays at `to`
  }
}

return { applyRaceSettings, trackAtmoBias: _trackAtmoBias,
         setWeatherLive, startWeatherArc, tickWeatherArc };
}
