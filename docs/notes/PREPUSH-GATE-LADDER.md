...
| `npm run test:tooling-fast` | 238 of 317 | 79 | the documented edit-loop check |
| `node tools/ci/deploy.mjs --gate-only` | 302 of 317 | 15 | the whole gate; what a deploy runs |
...
The remaining 15 are the per-circuit geometry sweeps (the table above is generated by `node tools/gen/gen-ladder-figures.mjs` from `tests/groups.json` and the files on disk; `tests/unit/docs-integrity.test.mjs` cross-checks the `N of M unit files` phrasing). `ci.yml` gives
...
the 79 files it leaves out cost about 7.5 minutes serial on this box
