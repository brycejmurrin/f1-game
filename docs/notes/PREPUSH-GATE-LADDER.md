...
| `npm run test:tooling-fast` | 261 of 343 | 82 | the documented edit-loop check |
| `node tools/ci/deploy.mjs --gate-only` | 326 of 343 | 17 | the whole gate; what a deploy runs |
...
The remaining 17 are the per-circuit geometry sweeps (the table above is generated by `node tools/gen/gen-ladder-figures.mjs` from `tests/groups.json` and the files on disk; `tests/unit/docs-integrity.test.mjs` cross-checks the `N of M unit files` phrasing). `ci.yml` gives
...
the 82 files it leaves out cost about 7.5 minutes serial on this box
...
`skipped`. A pin in one of those 17 files can therefore sit red for a while. If
