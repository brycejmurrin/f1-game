...
- Audi dark `ridgeTint` on dark cover: removed (factory uses `cap`). Derived `ridgeFill` (no tint set) still `pickOn`s against the cover so catalog team×`ridge` cannot vanish; an explicit `ridgeTint` is a pick and is never re-derived.
- Rake / shoulder: explicit `sideTint` / clearing `saddleTint` are picks; only *derived* fills re-pick against flank backgrounds so a wrap shelf cannot land 1:1 on the saddle block.

## Gates

```bash
# 59/59 pass (2026-09-10)
