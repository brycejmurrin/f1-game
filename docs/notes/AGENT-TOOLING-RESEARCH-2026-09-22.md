...
| `deploy.mjs --pr` auto-merge, CORRECTED: the first version armed it over GraphQL, which `api.github.com/graphql` REFUSES from a Claude Code session — and the refusal is a plain `{"message": …}` with no `errors` array, so the code read it as success. PRs #182 and #184 were both told auto-merge was armed; neither timeline ever carried an `auto_merge_enabled` event and both were merged by hand. Now armed through the session's CCR route (`PUT /repos/{o}/{r}/pulls/{n}/ccr/auto_merge`) and CONFIRMED by reading the PR back, with the note saying "WATCH this PR and merge it yourself" whenever it is not. The lesson is the general one: absence of an error is not evidence of an effect. The first honest run (PR #186) then gave the REAL answer — "Pull request Branch does not have required protected branch rules": the deploy branch has no protection, so GitHub will not arm auto-merge on it at all, and no PR has ever auto-merged here (#178 included). A session should expect the "NOT armed" note and merge its own PR once CI is green; turning it on is a train-latency decision (required checks on the deploy branch), not a tooling one | yes | `deploy-pr-rest.test.mjs` (GraphQL now fails the fake curl outright; one case replays the 200-shaped refusal and asserts `autoMerge: false`) |
