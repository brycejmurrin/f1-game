...
| B5 | `node-suites` is a matrix; the first run measured `vm` at 9.4 min alone (`fast` 1.3, `slow` 1.9), so `game-vm` is now two partitions (`test:game-vm-a` / `-b`, heavy files two a side) on two runners: slices `vm-a` / `vm-b` / `slow` / `fast`. The step text still lists every script for `deploy.mjs` and `twinned-specs` | 11.3 min → 9.4 (one slice) → ≈ 5 expected with the split |
| B6 | `smoke.spec.js` reordered so the [3,3,2,2] chunking pairs heavy with light, and `bootRace` now quiets the renderer for the build the way `goToRace` does | on SwiftShader (run 35062467814) shard walls 4.8 / 3.1 / 1.8 / ≤ 3 min against a 7.4 median, 14.5 p90 before; on llvmpipe (C2) 1.1–1.6 |
