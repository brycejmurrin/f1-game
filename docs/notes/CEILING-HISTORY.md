...
`js/render/glx/glx.js` lines 2615 -> **2643** (+28). DECLARE, DON'T BRACKET: every draw path set the state it wanted then restored the default, so the sequence was 1,0,1,0 and the cull/alpha cache — measured once and retired as "collapses zero" — never had two consecutive values to collapse. The pattern was the cost. Each path now declares on entry and does not restore, which moved the invariant, so the five paths that used to inherit the default (drawSky/drawShadow/drawMark/drawSkidBatch/drawDrivingLine, plus alpha on drawInstanced) declare explicitly. glx-call-census, vegas night, ONE car near the player — the floor, not the ceiling: CULL_FACE toggles 19.3 -> 5.3/frame, colorMask 15.3 -> 5.3, depthMask 7.5 -> 5.5, every draw count byte-identical.

`js/game.js` lines 10385 -> **10390** (+5). `FIELD_WHEEL_CACHE_MAX` 12 -> 16 and its justification corrected: it claimed the grid shares factory parts so live combos are "far fewer" than 22, but each of the 11 FACTORY_PRESETS has its own tyre and rim signature, so live keys equal the team count — 11, or 12 with MY TEAM, against a cap of 12. Zero margin on false reasoning, at ~205 KB per entry.
