...
| Bucket | Files | Bytes |
|---|---|---|
| `js/render/webgpu/*` → `js/render/webgpu/` | 4 | 512,738 |
| `js/render/three/*` → `js/render/three/` | 9 | 447,907 |
| `vendor/three-0.185.1/*` → `vendor/three-0.185.1/` | 6 | 1,097,747 |
| GPU-only tools → `tools/gfx/` | 13 | 123,778 |
...
| `tests/unit/tools-runnable.test.mjs:255,259,502-509` | Runs `pick-tests.mjs js/render/three/tlx.js` and `wgx-validate.mjs --static` as smoke tests, and asserts `wgx-gallery.mjs`/`wgx-shot.mjs` forwarding. All three tools/paths move. | Delete these three test cases (or point them at `tools/gfx/...` if the move window decides the spike keeps its OWN smoke-tested tools — out of scope for this inventory, a move-window decision). |
