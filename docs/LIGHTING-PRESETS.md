...
3. Parent merges with `merge-proposals.mjs`, flips the row to ✅, then
   `node tools/gen/gen-shell.mjs --check` (no cache bump is needed (tags read `?v=dev`; `pages.yml` stamps the hashes at deploy) — after a `tools/manifest.cjs` change run `node tools/gen/gen-shell.mjs`) after the last js edit.
