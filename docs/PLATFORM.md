...
Safari still double-tap-zooms a `pan-y` pane even with `touch-action: manipulation` on `*` (it ignores the viewport zoom cap for a11y). The shell therefore ships an early `touchend`/`gesture*` `preventDefault` in `index.html` that cancels a second tap on the same spot within 350 ms. Rapid taps on *different* controls, and two-thumb driving, are left alone. The viewport meta carries **no zoom cap at all** — neither `user-scalable=no` nor `maximum-scale` — and that is two decisions taken for one reason. `user-scalable=no` went first: Safari ignored it, so it bought nothing on iOS, while Android Chrome honours it and it blocked pinch-zoom for low-vision players (WCAG 1.4.4). `maximum-scale=1` was the same bug and was simply missed at the time: [W3C ACT rule b4f0c3](https://www.w3.org/WAI/standards-guidelines/act/rules/b4f0c3/) fails any `maximum-scale` under 2 against that same SC, iOS has ignored it since iOS 10, and Chrome Android honours it unless the player has dug out the "Force enable zoom" accessibility opt-in — so keeping one cap while dropping the other for an a11y reason only ever cost Android users their pinch. Nothing about the driving surface relaxed: `#game` and `.touchbtn` keep `touch-action: none`, and the touchend canceller is what does the double-tap work on every platform.
