...
| [research/PIT-OPEN-ITEMS-PLAN-2026-09.md](research/PIT-OPEN-ITEMS-PLAN-2026-09.md) | Plan (not built): the loose ends after the walls batch — the far PIT ENTRY board placed by a mass-hash walk instead of a number (Nürburgring's trunk), planned AI stops arming on the approach instead of at the line (the lap spent armed that every Bahrain hunt counted), crest panels facing the track as well as the lane, one real-GPU night frame for the neon, a painted hatch on the entry road's track side, and the post-exit excursions handed to the AI/tyre side. Facts at `892f2cd`, tests and costs per item. | `js/track/scenery/pits.js`, `js/race/pit-lane.js`, `js/track/core/mesh.js`, `.github/workflows/gpu-census.yml` |
