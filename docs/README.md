...
| [notes/AGENT-TOOLING-RESEARCH-2026-09-22.md](notes/AGENT-TOOLING-RESEARCH-2026-09-22.md) | Fourth pass, as research: the agent surface (AGENTS.md, skills, subagents, hooks, MCP, CI, branching) checked against the 2026 guidance by nine web-research subagents and four repo reviewers — what already matches, the three branching models for a shared deploy branch, the ranked gap list, and §6: what landed the same day (hook holes closed, agent tiers, `ci-red-triage`, two new hooks, `who-is-on-it`), what was withdrawn on evidence, and the settings file (deny list, allowlist, `worktree.baseRef`, the new hook rows) that landed with auto mode off. |
| [notes/SELF-IMPROVING-SURFACE-2026-09-22.md](notes/SELF-IMPROVING-SURFACE-2026-09-22.md) | Should each session write what it learned back into the rules? Measured answer: mostly no. Agentic context files grow +226% over their lifetime and old instructions get HARDER to delete (hazard -0.032/commit); a 288-run ablation finds context files do not move correctness (p=1.00) but do move token cost; an agent scoring its own edits put 15 of 35 runs below a random baseline while every one self-reported >=0.70. What the evidence backs instead: expiry, a rationale requirement, a budget on the always-on surface, and promotion of a lesson into a hook or a test rather than into prose. Includes what this tree already does better than the literature, and the CVSS 7.8 reason an instruction file must stay review-gated. |
...
| [research/RENDERER-PERF-AUDIT-2026-09-02.md](research/RENDERER-PERF-AUDIT-2026-09-02.md) | Three read-only renderer perf audits (GLX / WGX / TLX), ranked findings with proposed patches; landed items marked; the temporary WGX/TLX spike-out was later reversed. | `spike/backends/README.md`, `docs/archive/moves/spike-backends.json` |
...
| [research/PIT-GUIDANCE-STRATEGY-PLAN-2026-09.md](research/PIT-GUIDANCE-STRATEGY-PLAN-2026-09.md) | Plan (not built): the pit entrance on the minimap at the peel-off with a stateful marker and a distance arc, and the cue promoted in the MINIMAL profile; the cue ladder extended to the exit road (`fitted`, `served`, `merge`, `out`), a distance bar, one voice between the cue and the engineer, a stop summary and a first-stop teach; and strategy — a reference plan for the player from the same `stintPlan`, a HUD plan line, a pre-race STRATEGY panel with the circuit's pit loss, engineer calls that name a lap (undercut, rain, caution), rivals' windows on the tower, per-lap re-planning. Facts at `8acd3d4`, tests and costs per section. | `js/race/pit-lane.js`, `js/race/engineer.js`, `js/ui/hud.js`, `js/physics/ai-drive.js`, `index.html` |
| [research/PIT-OPEN-ITEMS-PLAN-2026-09.md](research/PIT-OPEN-ITEMS-PLAN-2026-09.md) | Plan (not built): the loose ends after the walls batch — the far PIT ENTRY board placed by a mass-hash walk instead of a number (Nürburgring's trunk), why a planned AI stop arming at the line and circulating a lap armed is by design and costs nothing (the hunt counter that said otherwise), crest panels facing the track as well as the lane, one real-GPU night frame for the neon, a painted hatch on the entry road's track side, and the post-exit excursions handed to the AI/tyre side. Facts at `892f2cd`, tests and costs per item. | `js/track/scenery/pits.js`, `js/race/pit-lane.js`, `js/track/core/mesh.js`, `.github/workflows/gpu-census.yml` |
| [research/PIT-ENTRY-LAMPS-PLAN-2026-09.md](research/PIT-ENTRY-LAMPS-PLAN-2026-09.md) | Built 2026-09-16 (the far post where the peel has its full width, not 6 m in): two coloured lamps flanking the pit entrance — one post each side of the entry road's mouth (the exit signal's own geometry), a light record each through the pit-lamp registrar (white pool, coloured head; or a `signal_green` kind), the aspect green at build time and RED under a red flag live through the pit-signs decal (which also closes the lighting plan's open question on a live exit signal). Facts at `8d3604596`, tests, costs, order. | `js/track/scenery/pits.js`, `js/garage/pit-signs.js`, `js/track/core/pit.js`, `tests/unit/pit-complex.test.mjs` |
| [research/PIT-NEXT-STEPS-2026-09.md](research/PIT-NEXT-STEPS-2026-09.md) | The ranked list after the guidance round at `f86dd678a`: the verification owed first (CI 4007 and its pages run, the two unrun specs `hud-layout` / `pit-lane` with `#announce` added to the measured set, one real-GPU night frame), then strategy (§4 of the guidance plan, broken into seven commits with the verified facts), then the entrance lamps, then the loose ends. | `docs/research/PIT-GUIDANCE-STRATEGY-PLAN-2026-09.md`, `docs/research/PIT-ENTRY-LAMPS-PLAN-2026-09.md`, `tests/specs/hud-layout.spec.js` |
