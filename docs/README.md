...
| [`research/RENDERER-PERF-AUDIT-2026-09-02.md`](research/RENDERER-PERF-AUDIT-2026-09-02.md) | Three read-only renderer perf audits (GLX / WGX / TLX), ranked findings with proposed patches; landed items marked |
| [`research/BROWSER-GRAPHICS-2026.md`](research/BROWSER-GRAPHICS-2026.md) | Outside knowledge, gathered 2026-09-02 and graded by how directly it was read: three.js draws `scene.backgroundNode` FIRST with the depth test off (read from upstream source — the mechanism behind the TLX sky item), Chrome 144 halving `writeBuffer` cost, immediates and transient attachments shipping, render-bundle state rules, and Khronos' measured Mali load/store and MSAA-resolve bandwidth numbers. Every row is CONFIRMED / CONFIRMED\* / REPORTED because the container's proxy blocks most canonical hosts. |
| [`research/BUG-HUNT-2026-09-02.md`](research/BUG-HUNT-2026-09-02.md) | Two rounds of read-only bug hunts (UI, GLX, WGX+TLX, memory, race-flow; then track engine + data, physics/AI, input/audio, net + data layer): every CONFIRMED row, what each fix batch landed, what was left and why |
