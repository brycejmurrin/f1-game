...
| [research/APEX-TOOLS-MCP.md](research/APEX-TOOLS-MCP.md) | Hosted `apex-tools` MCP wrapping committed CLIs against the local tree; deploy stays TinyFish. Week-1 + week-2 catalog in `tools/apex-tools-mcp.mjs` (browser tools take the lock; CI is mock/`dryRun`). |
