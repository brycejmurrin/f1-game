...
| [research/PIT-ENTRY-LAMPS-PLAN-2026-09.md](research/PIT-ENTRY-LAMPS-PLAN-2026-09.md) | Plan (not built): two coloured lamps flanking the pit entrance — one post each side of the entry road's mouth (the exit signal's own geometry), a light record each through the pit-lamp registrar (white pool, coloured head; or a `signal_green` kind), the aspect green at build time and RED under a red flag live through the pit-signs decal (which also closes the lighting plan's open question on a live exit signal). Facts at `8d3604596`, tests, costs, order. | `js/track/scenery/pits.js`, `js/garage/pit-signs.js`, `js/track/core/pit.js`, `tests/unit/pit-complex.test.mjs` |
