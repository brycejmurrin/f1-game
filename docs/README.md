...
| [research/F1-2026-RULES-FIDELITY.md](research/F1-2026-RULES-FIDELITY.md) | What the 2026 regulations actually say versus what we model — the retired X/Z mode names, Overtake missing its activation zone, and a deploy taper sitting well below the real one. |
...
| [research/PHASE-C-SLIDER-DESIGN.md](research/PHASE-C-SLIDER-DESIGN.md) | The handling-slider recalibration as arithmetic: RACE PACE's 5x resolution asymmetry and its geometric replacement, SPEED STEER's 1.9-point spread at racing speed, RESPONSE spending its top half below any real wheelbase, SMOOTHING mapped in Hz where players feel lag — with the migration error table and which numbers still need the sweep. |
