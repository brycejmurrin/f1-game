...
| `js/render/shared/driving-line.js` | `build` (via game.js `drivingLineApi`) | **surface** | the DRIVING LINE ribbon: reads the static curvature LUT once per circuit to place the line and shade its braking zones; a picture on the road, no car reads it. Same lateral formula as the assist-gated `lineX` so the two agree |
