# Architecture review journal (2026-08) — ARCHIVED

> **Dated record, kept verbatim as provenance.** This is the session journal
> the 2026-08 architecture review produced: the full defect register, the fix
> narratives, and the measurements behind them, exactly as written. The
> **standing review** — current defect statuses, the guard table, the backlog —
> lives at `docs/ARCHITECTURE-REVIEW.md`; read that first.
>
> Statuses in this journal were live at the time of writing and are **not
> maintained**. Several "open" items below have since been fixed and several
> "Fix pending" notes are superseded by later entries **in this same file**
> (e.g. A18's "Fix pending" vs CI-7's confirmation of the fix).
>
> Known internal quirks, preserved rather than repaired: the defect ID **A13
> is used twice** for two unrelated defects (a silent-save-failure fix in
> Tier A, and the Safari zoom/rect audit in Tier C); the second Tier A table
> ("fourteen failures the CI gate made visible") was renumbered **CI-*** after
> its original A-numbers collided with a concurrent pass; and some entries
> correct earlier entries in place by design — the wrongness was kept as the
> lesson.
...
| Guard | What it holds |
|---|---|
| `tests/unit/load-order.test.mjs` | `index.html` matches `tools/manifest.cjs` exactly, including eval-time `HARD_EDGES` |
| `tests/unit/scenery-api-contract.test.mjs` | the 107-member `scenery(api)` surface every circuit file was written against |
| `tests/unit/test-groups.test.mjs` | the test taxonomy: every group real, every source dir routed, `docs/TESTING.md` in step |
| `tests/unit/docs-integrity.test.mjs` | live docs reference only files that exist; CLAUDE.md's counts match the repo |
...
exact figure is the one `tests/unit/module-size.test.mjs` ratchets, which is the only
...
`tests/unit/module-size.test.mjs` is for, and it is the reason the number lives in a
...
`tests/unit/backend-surface-parity.test.mjs` rather than remembered.
...
pre-existing failures in `tests/specs/new-hooks.spec.js` that nobody had seen, because
...
`tests/specs/hud-layout.spec.js` reported the throttle and brake buttons overlapping
the pause button (`btn-throttle+pausebtn`, `btn-brake+pausebtn`) in BUTTONS steer
mode, landscape only — four tests. `tests/specs/ui-scale.spec.js` failed five more, on
...
`tests/specs/menu-baseline.spec.js-snapshots/garage-phone-landscape.png` showed a
...
`tests/specs/audit.spec.js`, both of which fail without the fix. Original entry: `lap` increments on a forward line crossing and there
...
`tests/unit/docs-integrity.test.mjs` fails if a season literal comes back. Not
...
- ~~340 `catch` blocks in `js/`; 59 `Log` call sites. The great majority of
  ...
  API, a `localStorage` read that may be blocked). `tests/unit/silent-catch.test.mjs`
...
game.js is "~6,100 lines" against a file that has never been near that since (the current figure lives in `tests/unit/module-size.test.mjs`, not here). `docs/DEBUG-HOOKS.md` and
...
The variance was in the REPORTING. `tests/helpers/live-reporter.js:84` kept the first 4
...
alarm this hook exists to rule out") and in `tests/specs/terrain-over-road.spec.js:162`,
