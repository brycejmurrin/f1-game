...
`js/glx.js:2946-2977`) and **no MRT / no instancing**. I would ship the plan's
...
- **The seam already exists.** The plan itself says so (Migration Step 3: "the
  interface *is* today's `GLX` return object", `js/glx.js:3693-3769`). `game.js`
...
- **The real maintainability lever is inside `glx.js`, not at its boundary.**
  ...
  (`draw()`, `js/glx.js:2982-3024`) — neither of which Phase 0 touches. So I
...
**maintaining two textual copies of `LIT_FS` (~850 lines, `js/glx.js:39-889`)
and `COMPOSITE_FS` (~390 lines, `js/glx.js:1594-1987`) in two shading languages
...
(`js/glx.js:133`) and then **re-defined verbatim as `vnoise2` in `SKY_FS`**
(`js/glx.js:926`) because the two shaders are independent strings that cannot
share a function. `hash21` (`js/glx.js:128`), `cloudFBM` (`js/glx.js:323`),
`D_GGX` (`js/glx.js:107`), `acesTonemap` (`js/glx.js:1644`), `colourGrade`
(`js/glx.js:1652`) are each authored once but are *structurally* the kind of
...
1. **The crash-prone devices get nothing from WebGPU (plan R2).** Correct and
   ...
   (`js/glx.js` mobile caps; `createTargets` goes out of its way to
   bind-before-delete to avoid transient double-resident surfaces,
   `js/glx.js:2276-2296`) are pre-iOS-26 and cannot run WebGPU at all. Spending
...
2. **WebGL2 already has the one feature that would actually help every device:
   ...
   `castShadowChunked` at `js/glx.js:3707`) frustum-culls per chunk but still
...
un-DRY across passes (`vnoise` vs `vnoise2`, `js/glx.js:133` / `926`). Any fix
...
```js
// js/shaders/chunks.js  (new IIFE global: ShaderChunks)
...
// js/glx.js  (unchanged authoring style — still template strings):
...
- **What:** Section B option (i). Extract `hash21`/`vnoise`/`cloudFBM`
  (`js/glx.js:128/133/323`), the GGX trio around `D_GGX` (`js/glx.js:107`),
  `acesTonemap`/`colourGrade` (`js/glx.js:1644/1652`) into named string
  constants in a new `js/shaders/chunks.js`; concatenate into the program
  sources. Delete `vnoise2` (`js/glx.js:926`) in favour of the shared chunk.
...
- **What:** Physically split the 3,770-line file into `js/glx/shaders.js`
  ...
  (`createTargets`, `initPost`, the render-target set, `js/glx.js:2200-2453`),
  `js/glx/passes.js` (shadow/env/post pass orchestration), and keep
  `js/glx.js` as the thin IIFE that wires them and **returns the identical ~35-
  method object** (`js/glx.js:3693-3769`). No public surface change.
...
- **What:** Write down the contract that the `GLX` return object
  (`js/glx.js:3693-3769`) already *is*: for each method, its signature, the
...
- **What:** The stride-15 light layout is decoded inline in `begin()`
  (`js/glx.js:2952-2977`) into six scratch `Float32Array`s and uploaded with
...
- **Why (today):** the magic numbers `o+0..o+14` and the parallel
  ...
  (god-ray, `drawGlow` at `js/glx.js:3281`) with the offsets duplicated there.
...
- **What:** `draw()` imperatively toggles `setDepthMask`, `setBlend`,
  ...
  polygon-offset for shadows/marks (`js/glx.js:3006-3023`). Enumerate the actual
...
- **Why (today):** the "each draw declares full state, no restores" convention
  (`js/glx.js:3006`) is clever but means the legal state space is implicit and
...
- **What:** `createTargets` (`js/glx.js:2261+`) allocates ~14 targets
  (scene/depth/MSAA/bloom mips/SSAO/god-ray/LDR/shadow/blocker/env) with the
  careful bind-before-delete discipline (`js/glx.js:2276-2296`). Represent the
...
- **Why (today):** the bind-before-delete jetsam-avoidance rule
  (`js/glx.js:2276`) must be repeated correctly for *every* target; a
...
| 2 | **Shader chunk system**: extract shared noise/BRDF/tonemap to `js/shaders/chunks.js`; delete `vnoise2`; add `#line` fix. | C.1 / B | S-M | **Do anyway** | `test:visual` shows **zero** delta (concatenation is byte-identical GLSL). |
...
- **Verdict on the plan:** sound and honest; adopt its recommendation ("Phase 0,
  ...
  return object (`js/glx.js:3693-3769`). The real maintenance cost lives *inside*
  ...
  that duplication problem *already exists today* (`vnoise` at `js/glx.js:133` is
  re-inlined as `vnoise2` at `js/glx.js:926`).
...
- **Top 3 "do-anyway" refactors:** (1) shader chunk system for shared
  ...
  (3) extract the stride-15 light-upload layout (`js/glx.js:2952-2977`) into one
