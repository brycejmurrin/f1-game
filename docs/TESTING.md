...
| `jsconfig-globals.test.mjs` | `jsconfig.json` is a script-mode editor project over `js/` only (`checkJs` off, no `moduleDetection:force`, no `types/globals.d.ts`) so IIFE `const X` names resolve to the writer; `tsc -p jsconfig.json` stays clean |
