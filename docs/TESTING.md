...
47 root Playwright spec files (`tests/*.spec.js`), plus 24-file
...
`playwright.config.js`, so a bare `npx playwright test` runs the 47 root specs
...
The 47 root specs are split into two Playwright projects (see
