...
| `title-fx.test.mjs` | MENU ANIMATIONS + TITLE INTRO + MENU WASH + MENU LAYOUT + TITLE ART as player settings (`js/ui/title-fx.js`) under SETTINGS › APPEARANCE (UI SIZE also lives there via `js/ui/scale.js`): unset motion is ON and an OS asking for reduced motion always wins; TITLE INTRO is FULL / QUICK / OFF; MENU WASH is FULL / SOFT / OFF; MENU LAYOUT is AUTO / STACK / COMPACT via `data-menu-layout`; TITLE ART is ON / SOFT / OFF; five SettingRows + REPLAY INTRO round-trip; intro one-shot / replay / pause / vibrate contracts; CSS and settings-export list the appearance keys including `uiScale` |
