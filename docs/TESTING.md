...
| When | Run |
|---|---|
| in the edit loop | `npm run test:tooling-fast` (structural, no browser; 246 of 326 unit files — ~5 min one at a time, ~2 min via `node tools/ci/tooling-fast.mjs --jobs=3`, which is what `verify-change` runs on a quiet box; MEASURED 2026-09-16) |
...
| `lamp-bake.test.mjs` | `LampBake` bakes the lit shaders' diffuse pool exactly: two aimed lamps checked texel-by-texel against an independent evaluation of the shader term (windowed 1/d² with LAMP NEAR CLAMP, the cone smoothstep, N·L for N=+Y) within 0.2% after half-float encoding; ground height feeds the bake; the cache rebakes on a new light-set array or near clamp; the map stays under its texel budget; `toHalf` round-trips |
...
| `appearance-opts.test.mjs` | Settings › APPEARANCE (`js/ui/appearance-opts.js`): the shell carries the door, panel and three setting rows; defaults stamp `data-ui-theme` / `data-menu-accent` / `data-hud-accent` at EVAL; theme and named menu presets persist; CUSTOM menu accent writes inline `--red`; HUD BRAND points `--accent` at `var(--red)`; DOM ready wires the three SettingRow ids |
