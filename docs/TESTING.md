...
| `apex-tools-mcp.test.mjs` | `apex-tools` MCP — `serverInfo.name === apex-tools-mcp`, tools are all `apex_*` (zero chrome/tinyfish), week-1 pins (`--fast`/`--json`, never `--wait`/`--bg`/`--apply`), week-2 frozen `apex_ui_survey` recipe + lock/occupancy refuses (`lock_held` / stale steal / `playwright_live` / `chrome_daemon_up`), `target=deploy` → `tree_only`/`local_only`, `github.io` → `github_io_blocked`, loopback url on browser tools → `url_not_supported`, `isError` preserved, stdout JSON-RPC only (mock/`dryRun`, no Chromium) |
