...
| When | Run |
|---|---|
| in the edit loop | `npm run test:tooling-fast` (structural, no browser; 237 of 317 unit files — ~5 min one at a time, ~2 min via `node tools/ci/tooling-fast.mjs --jobs=3`, which is what `verify-change` runs on a quiet box; MEASURED 2026-09-16) |
...
| `title-fx.test.mjs` | MENU ANIMATIONS as a player setting (`js/ui/title-fx.js`): unset follows prefers-reduced-motion, a stored `on`/`reduce` beats the OS both ways and garbage reads as unset; `<html data-motion>` lands at eval; the built row sits under UI SIZE and round-trips `apex26.motion`; wiring waits for DOMContentLoaded while deferred scripts run; showing `#overlay` replays `data-intro` with a forced reflow; a hidden tab sets `data-paused`; a title `.bigbtn` click taps once through `Input.vibrate` (disabled/non-button clicks do not); and the CSS in `css/responsive.css` keys off those attributes |
