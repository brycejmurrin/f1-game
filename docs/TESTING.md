...
| `replay-determinism-vm.test.mjs` | "same seed + same inputs => same result" (~5 s, one boot): three `run(42)` episodes in one VM must digest byte-identically, and two seeds must deal different grids. A deliberate TWIN of the first test in `agent-determinism.spec.js` — the invariant has been broken four times (drivetrain, `_prevS`, `accSm`, `lane`), always by per-car scratch a re-grid does not restore, and the browser spec is too expensive to catch a fifth in the edit loop. Run 1 is always the odd one out |
