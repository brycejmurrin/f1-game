...
116 root Playwright spec files (`tests/specs/*.spec.js`) + 210 `node --test` unit suites
...
| `menu-traversal.spec.js` | EVERY control of a menu layer is reachable with the arrow keys alone and with a controller's D-pad alone — a breadth-first walk of focus states compared against `MenuNav.items(layer)`, plus Escape and the pad's B leaving each layer. Five defects shipped in exactly this gap and are pinned here: a closed `<details>` kept its contents' layout boxes so `step()` chose one and `focus()` silently refused (from BACK on DISPLAY no arrow moved at all); a `<select>` owned all four arrows so every ‹ value › row was an island needing Tab, which a pad has not; the across-penalty let a distant full-width slider beat the next row's chevrons; the ‹ › chevrons formed a focusable column nothing entered; and the pad's synthetic key went to `document`, where an element's own `onkeydown` (both tab rails) is not in its path. Both walks run in-page on synthetic keydowns — what the pad sends anyway, and ~100x the round-trip speed |
