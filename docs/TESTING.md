...
112 root Playwright spec files (`tests/specs/*.spec.js`) + 109 `node --test` unit suites
...
| `mcp-smoke.test.mjs` | four-server shell probe — `--dry-run` lists apex-tools / probe / chrome-devtools / tinyfish, never `verify` / `deploy-check` / `test-bg`, `apex-tools-mcp.sh smoke` delegates, no Chromium |
| `agent-surface.test.mjs` | wrap map lockstep — `docs/AGENT-SURFACE.md` names every `apex_*` in `tools/apex-tools-mcp.json`, each CLI/skill exists, never-wrap lists `test-bg` / `--apply` / github.io, indexes point at the map, catalog descriptions start Tree / Browser (lock first) |
