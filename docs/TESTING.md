...
| `career.spec.js` | career mode: the flow/session axes, the three save SLOTS + the single-save migration into slot 0, the isolation guarantees (career development must not reach a Grand Prix, and must reach the career), the hub flow, a settled round, driver ratings, the R&D garage, MY TEAM, objectives/contracts/rollover, reliability, the CAREER MODES screen, the settlement panel, EXTRA FUNDS (and that it never raises the fitted cap), the research facility, the hire's contract, and sponsors |
| `quali.spec.js` | one-lap qualifying: the simulated field and its spread, the sheet's two states, that the race grid IS the qualifying order car-for-car (not the hardcoded P12), that EVERY championship round qualifies rather than only the first, and that a classification never leaks into the Grand Prix that follows it |
