...
| When | Run |
|---|---|
| in the edit loop | `npm run test:tooling-fast` (structural, no browser; 228 of 302 unit files — ~5 min one at a time, ~2 min via `node tools/ci/tooling-fast.mjs --jobs=3`, which is what `verify-change` runs on a quiet box; MEASURED 2026-09-16) |
...
| `circuit-vocab.test.mjs` | the same question one level down — a def may name a `furniture.tree` species or a `standSet` grandstand livery the engine has no entry for, and every lookup has a fallback, so the circuit renders as something else in silence (five circuits grew broadleaf trees from `tree: "pine"`; Dijon's `"stone"` livery was the steel default). The valid sets are read OUT of `js/track/tracks.js` and `js/track/scenery/data.js` rather than restated, so adding a species does not turn the gate into a liar |
...
| `reject-lint.test.mjs` | its sibling over `js/`: a `try`/`catch` cannot swallow a promise REJECTION, and `index.html` paints a full-screen overlay on any unhandled one. A ZERO ratchet — the tree is clean and the fix is one line per site — over the APIs whose promise-return is unambiguous, with `close`/`play`/`pause`/`resume` disambiguated by RECEIVER so the rule does not cry wolf at thirty synchronous WebSocket and IndexedDB teardowns and get switched off |
