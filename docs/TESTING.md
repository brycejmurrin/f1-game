...
| `player-dynamics-vm.test.mjs` | The SHAPE of the player's driving model, via `tools/check/player-dyn.mjs`'s pinned bench (the car is re-pinned to a straight each step, position only, so skidpad / step-steer / technique tests run without walls): the lateral force peaks and FALLS past its peak (2026-09-16: `tanh` never fell, so a flick at full lock cost nothing); the throttle charges the rear axle and never the front, more at low speed than high; planting the throttle at the limit steps the rear out; lift-off and trail braking rotate the car; full lock at 45 m/s washes wide and never spins; lateral g is monotone in lock up to the front's limit. Relative assertions only — a retune may move every magnitude. Evidence: `docs/notes/PLAYER-PHYSICS-RESEARCH-2026-09.md` |
