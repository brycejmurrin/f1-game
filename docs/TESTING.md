...
| `title-fx.test.mjs` | MENU ANIMATIONS as a player setting (`js/ui/title-fx.js`): unset is ON and an OS asking for reduced motion always wins (a stored `reduce` adds reduction, garbage reads as unset); `<html data-motion>` lands at eval; the built row sits under UI SIZE and round-trips `apex26.motion`; wiring waits for DOMContentLoaded while deferred scripts run; the shell's `#overlay[data-intro]` is held for ONE pass then removed (never replayed on return, dropped at once under reduced motion, deferred to the first show when booted past the title), and `replay()` restarts on the next frame with no forced layout; a hidden tab sets `data-paused`; a trusted title `.bigbtn` click taps once through `Input.vibrate` (disabled, non-button and scripted clicks do not); and the CSS in `css/responsive.css` keys off those attributes |
