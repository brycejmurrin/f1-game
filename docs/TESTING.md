...
| `track-line.test.mjs` | `TrackLine.bake` on synthetic tracks: a straight has no line (weight 0), one corner turns in outside / apexes inside / exits outside and stays on the road, a tighter corner turns in later, a chicane is straight-lined through the middle, sub-threshold wiggles are not corners, a corner across the lap seam closes continuously, and the attack zones rate a corner after a long straight on a wide road as prime and a narrow or short-fed one as poor. The live check (approach/apex positions on monza) is in `ai-racecraft-vm` |
