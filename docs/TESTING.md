...
113 root Playwright spec files (`tests/specs/*.spec.js`) + 125 `node --test` unit suites
...
| `skill-progressive.test.mjs` | mcp-probe SKILL.md stays a thin index (≤120 lines) with traps/recipes in `references/`; previously-fat skills stay split (index ≤180 + the named reference file); slim-bloat / bloat-auditor / `bloat-scan.mjs` stay the Claude-simplify analog (thin index, readonly auditor, JSON scan) |
