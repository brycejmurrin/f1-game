...
| `title-fx.test.mjs` | MENU ANIMATIONS + TITLE ART as player settings (`js/ui/title-fx.js`) under SETTINGS › APPEARANCE: unset motion is ON and an OS asking for reduced motion always wins (a stored `reduce` adds reduction, garbage reads as unset); TITLE ART is ON / SOFT / OFF via `data-title-art` (garbage → on); `<html data-motion>` / `data-title-art` land at eval and in the boot script; both SettingRows + REPLAY INTRO are static shell DOM and round-trip store keys; the shell's `#overlay[data-intro]` is held for ONE pass then removed (never replayed on return, dropped at once under reduced motion, deferred to the first show when booted past the title), and `replay()` / the button restart on the next frame with no forced layout; a hidden tab sets `data-paused`; a trusted title `.bigbtn` click taps once through `Input.vibrate`; CSS in `responsive.css` / `menus.css` keys off those attributes; both keys export under the appearance group |
