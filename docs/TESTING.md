...
| `ghost-share.test.mjs` | the share codec contract: track-tagged ghost round trips preserve lap time and sample lengths, malformed payloads return `corrupt` without throwing, and payloads naming tracks outside the circuit registry return `unknown-track` |
