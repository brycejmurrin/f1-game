...
history: the last ten samples for each key, each carrying a run-start timestamp,
its env bucket (`swiftshader` / `llvmpipe` / `local`) and its wall time in
seconds. `tools/ci/spec-timings.mjs` merges into it from the two sources a run
already produces — the junit XML at `artifacts/test-results-*/junit.xml` and
`tests/helpers/live-reporter.js`'s own per-test durations, which it writes at
`onEnd` only under `APEX_SPEC_TIMINGS=1` (off by default, so an ordinary local
run neither loads the tool nor dirties the tree). A sample is keyed by (bucket,
run-start second), so merging the same junit twice, or a run's junit alongside
...
### Coverage is opt-in, and measured across both halves of the suite

`APEX_JS_COVERAGE=1` makes a browser run collect raw V8 script coverage —
`tests/helpers/js-coverage.js` starts `page.coverage` on every page the fixtures
open (once per worker under `sharedTest`) and writes one JSON list per test to
`artifacts/coverage-<port>/`. The node half uses Node's own dump:
`NODE_V8_COVERAGE=artifacts/coverage-node npm run test:game-vm-b`. The two are
the same format, so `tools/ci/coverage-merge.mjs` folds both into one lcov +
html report under `artifacts/coverage-report/` (the live reporter calls it at
`onEnd` of a flagged run; it is also a standalone command). Both flags are off
by default and the helper is inert without them — an ordinary run makes no CDP
call and writes nothing under `artifacts/coverage*`.

Two things the numbers depend on. The VM harnesses (`tools/lib/game-vm.cjs`,
`tools/track/verify-track.cjs`) name every script by ABSOLUTE path, because V8
silently drops a relative one: before 2026-09-22 every twin reported 0 % for
the game sources it ran. And the same harnesses rewrite `^const` → `var`, so a
node entry's offsets index the rewritten text; the merge attaches whichever
text V8's top-level range length fits and drops an entry that fits neither.
The html shows `var` where the file says `const` on those lines.

First measurement (2026-09-22, this box): one twin, `active-aero-vm.test.mjs`,
alone reaches 230 sources, 49.1 % of lines / 29.1 % of functions / 21.2 % of
branches under `js/` — a boot plus one race on one circuit. That is the
floor a browser group adds to, not a suite figure; there is no CI job for it
yet (SwiftShader cost), so a number worth quoting is one you measured with the
group named beside it.
...
| When | Run |
|---|---|
| in the edit loop | `npm run test:tooling-fast` (structural, no browser; 265 of 348 unit files — ~5 min one at a time, ~2 min via `node tools/ci/tooling-fast.mjs --jobs=3`, which is what `verify-change` runs on a quiet box; MEASURED 2026-09-16) |
...
coverage audit noticed later, or nobody did. It is one edit now. The gate-ladder
figures ("N of M unit files" in AGENTS.md rule 3, `docs/notes/PREPUSH-GATE-LADDER.md`
...
| `tooling-fast` | the structural half — 265 files, ~5 min one at a time (317 s measured 2026-09-16), ~2 min at `--jobs=3` — via `tools/ci/tooling-fast.mjs` (`--test-concurrency=1` inside each file; `--jobs=N` files at once, LONGEST FIRST by `tests/data/tooling-fast-timings.json` — refresh it with `--record` — each child under `--test-timeout` plus a per-file wall that fails a hung file by name) with START/PASS/FAIL + `not ok` names on stdout and `artifacts/logs/tooling-fast-suite.log`. Load order, docs integrity, test groups, api contracts, css layer discipline, graph, validators. The full-fleet sweeps dominate `tooling`; this is everything else, for the edit loop |
...
| `game-vm-b` | every other `game-vm` file (`vm-b`): agent-view 27 s, ai-racecraft 33 s, mechanics-integration 15 s, the rest seconds each — ~2 min on a runner. Partition of `game-vm` |
| `vm-page` | the browser specs that run **as themselves** in node under `tests/helpers/vm-page.js` (`tools/ci/twinned-specs.mjs` `ADAPTED`): `tests/unit/adapted-specs.test.mjs` spawns one `APEX_VM_PAGE=1 node --test <spec>` child per spec, two at a time, and fails a child whose TAP summary cannot be read or that declares no tests. Its own CI runner (`node-suites` slice `vm-page`), in the Pages gate unconditionally, which is what lets `partitionArgs` drop the browser copies locally and `select-specs` skip them on the blocking gate. Each entry was RUN green under the adapter before it was listed — `vm-portable` says eligible, only a run says adapted |
...
| `docs-guards` | the unit files that READ the prose trees ci.yml's `paths-ignore` skips (`docs/**`, `**/*.md`, `.claude/**`, `.cursor/**`, `.codex/**`): docs-integrity, generated-docs, agent-config, skill-progressive, hooks-documented and the rest of `tests/groups.json`'s list. A PR touching only those trees starts no `ci.yml` run, so `.github/workflows/docs-guards.yml` runs this group on exactly that path list (`ci-coverage.test.mjs` pins the two lists equal). Every file is also in `tooling-fast`; this is a CI entry point, not new coverage |
| `node-slow` | the raster/spawn-heavy car files (`cockpit-pale-surfaces`, `crest-marks`, `slider-effect`; 152 s of the old 315 s loop) plus `flyby-fleet` (the flyby's twelve-circuit audit, ~1 min) and `foundation-core-vm` (~26 s, builds all sixteen foundation circuits) — CI guards always, locally when pick-tests names it |
| `sweeps` | the full-fleet geometry audits (19 files — `package.json` `test:sweeps` IS the list; the enumeration that used to stand here had drifted four files behind it, so read the script). Most rebuild every circuit through `tools/lib/track-build-vm.cjs`; a few build no circuit at all (driving-line's synthetic stadium, driving-line-opts' store round-trip, track-build-vm-release's two-circuit harness check) and ride here beside the geometry they serve. `coplanar-faces` is the z-fighting ratchet that `clip-audit` structurally cannot see. Runs `--test-concurrency=1` **on purpose** — see below |
...
`--portable`, `--blocked`): **35 of 119 specs portable today**, 53 after a
...
| `agent-view.spec.js` | the agent world view: `world`, `trackInfo`, `scene`, `describe`, `query`, typed errors — 116 tests, every one replayed by `agent-view-vm.test.mjs` on the node gate (twinned; the frustum-chunk probe lives in `webgl-probes.spec.js`) |
...
| `wake-lock.spec.js` | the screen wake lock held for the duration of a race: requested on start, released on finish and on a mid-race quit (no results screen), released on hide and re-acquired on return, and degrades silently when the API is missing or its request rejects — twinned by `wake-lock-vm.test.mjs` on the node gate; the browser copy runs on the nightly |
...
| `pit-signs.spec.js` | the pit bay signs, live on the renderer: `__apex.pitSigns()` reports twelve cells with the atlas and the texMesh uploaded, the decal draws every frame from the grid (inside the row's 350 m gate), and `meshToggle({pitSigns:true})` stops it. The numbers are `tests/unit/pit-signs.test.mjs`'s; this is the boot evidence |
...
| `throttle-latch.test.mjs` | THROTTLE = LATCH in `js/input/input.js`, driven through the pedal's own pointer listeners in the same VM harness: the UP edge does not undo the DOWN edge (or a tap would be a no-op), a latched pedal reports FULL travel rather than wherever the thumb last slid it to, the latch drops on the everything-off path (window blur, `Input.reset`) so it cannot accelerate the car while the player is not looking at it, leaving LATCH mode clears a live one, and the pedal SHOWS it — `:active` follows the thumb, `.on` plus `aria-pressed` outlive it |
| `steer-presets.test.mjs` | the two tables in `js/input/steer-tuning.js` that must agree — every PRESET bundle (RELAX / STANDARD / PRO) lands exactly ON its FEEL level (easy / normal / sim), because `matchSteerLevel()` reads the live sliders back through the levels and a bundle landing BETWEEN two of them lights up CUSTOM; STANDARD equals the store fallbacks so a fresh install does not read CUSTOM either; the ladder stays monotonic on what it actually differentiates (LOCK and the speed taper over one calm rack, with SIM the single step that quickens it); and RELAX never ends up with a quicker rack than STANDARD, which inverts the bundle. Stated in two comments and checked by nothing until the 2026-09-08 re-centring moved two of the three and left RELAX behind for a week |
| `key-binds.test.mjs` | the desktop key-rebinding table and its controller twin in `js/input/input.js`, in the same VM harness: the defaults are the keys and buttons the game always had, a rebind moves the action and the old key stops answering, a key another action held is taken from it and the caller is told which, reserved menu keys (Escape, Enter, Tab, P) and malformed codes are refused, either Shift is one key, a saved `apex26.keys` map round-trips with per-action fallback for garbage, a held pedal is released when its key changes meaning, and key names read like key caps; for the pad (a fake standard-mapping gamepad polled through `Input.poll`) a rebound button drives and the old one stops, triggers stay analog, an edge action fires once per press, Start and the d-pad steer buttons are refused, an armed `padCapture` takes the first rising edge and the frame does nothing else, a saved `apex26.pad` map round-trips, button names switch to the PlayStation family when the pad says so; and the CONTROLS page on a touch device (the UI module against a stub DOM): both tables hide behind one hint until a physical key (`Input.keyboardSeen`, never latched by typing in a field) or a pad reveals its table, and a desktop shows both with no hint |
| `settings-export.test.mjs` | the two FILES (`js/ui/settings-export.js`) in a VM with a scripted store and a real-shaped localStorage. SETTINGS: a fresh store exports no changes and ALL lists every allowlisted key at its default, a changed key carries its value, the default it replaced and the source file that owns it, a tuner that saves its whole table reports only the fields moved, the binding tables count as changed by `Input`'s word, a store poisoned with garage / save / account / latch keys leaks nothing into either mode, the device picks the graphics default, and every literal default in the allowlist agrees with the `store.get` read in its source file. GARAGE: the prefix allowlist carries parts, liveries, setups and an invented team and nothing else out of the same poisoned namespace, and an empty store still yields a valid file. LOADING: the settings loader writes only SPEC keys (never the garage, never a migration version, `null` on the raw lane meaning UNSET), refuses a file of the wrong format whole, skips a value of the wrong type while applying the rest; the garage loader writes only garage-shaped keys, so a career save or a token in a hand-edited file is dropped; and both files round-trip into a fresh store |
| `settings-defaults.test.mjs` | the SHIPPED default lives in one file and three consumers agree about it (`js/data/settings-defaults.js`). Every key in the file is a SPEC key and none is device-adaptive; `GameStore.get`/`raw` answer from the file rather than the call-site literal, while a value the player stored still outranks it and an UNLISTED key is untouched — including `raw()` still returning `null` for never-set, which call sites read as meaningful; `settings-export.js`'s `defaultOf` reports from the file, so an exported CHANGED list cannot start naming keys the player never touched; the file loads before the store that reads it; and a REVIEW key (muting the game, lifting the career budget) is never promoted to a shipped default without being named |
...
| `props-over-road.spec.js` | all-circuit audit: no PROP triangle sits on/above the racing line, in 3D, 0.2-5 m above the road. Per-track `BASELINE` caps document justified overheads (Miami's beach canopy, Mexico's Foro Sol, gantries). **Runs on no push** — it declares 1500 s, over the change-aware gate's 180 s per-test cap, so `select-specs` excludes it on every `js/track` / `js/circuits` diff and its only schedule is the nightly rota, one night in eleven. It is currently RED on monza, mosport and zandvoort (`docs/notes/DEFECT-LEDGER.md`). `props-over-road.test.mjs` is the blocking guard on the same axis, but it audits the VM build, which is NOT this spec's geometry — see its row |
| `props-over-road.test.mjs` | THE BLOCKING copy of that audit, in node: no prop or glass triangle on or above the racing surface, 0.2-5 m up, on all 52 circuits. The fourth scenery axis — `buildProps`'s on-road rejection is footprint-based and the crowd emitters bypass it entirely with `RAW.addBox`, so a prop anchored legally off-track can still hang over the tarmac; it has shipped twice (a Vegas barrier, a Buenos Aires tree). Not a replacement for the spec, and the gap is measured: same sample ladder, tolerance band and barycentric test with the same 0.01 projected-area floor (1e-9 lets a near-vertical face of a long diagonal box report a box-top height at any distance — two 15 m-clear grandstands read as 4.65 m), and `nodeAt()`'s three-decimal rounding reproduced so the sample points are identical to the millimetre. Verified against the two readings the spec wrote down: jeddah's grey `[0.46,0.47,0.5]` at 1.07 m lateral -6.35, and mont_tremblant's 4.74. What changed is cost — one uniform XZ grid replaces the two linear scans, so the fleet takes 90 s against the spec's 1500 s budget, verdicts byte-identical circuit by circuit. It found the spec had been RED on mosport and zandvoort since the walled-exit pit redesign (same pit wall, same colour, unbaselined because nobody reads the nightly), and it does NOT see monza's browser-only 0.75 m face, because the VM build has no prop vertex within 5 m of it — the VM's geometry is not the browser's, which bounds every fleet sweep, not just this one. One fleet pass for all three tests, verdict kept and build dropped (peak 799 MB); the third test is anti-vacuity, pinning that the known wall and crown are still found so a silently-blind audit cannot pass |
| `baked-model-road-guard.test.mjs` | A BAKED MODEL MAY NOT STAND ON THE ROAD. `js/track/tracks.js` guards every prop emitter against the racing surface — `addBox`, `addCyl`, `addCone`, `addFrustum`, `addPrism`, `addPyramid` all go through `GUARDED`, which rejects a primitive whose footprint is on the tarmac — and `addMesh` was not in that set, so `bakedModel()` called it raw. The one emitter that stamps whole buildings was the one with no guard, and monza's `kenney_ind_building-d` was anchored 60 m off node 17 at the Variante del Rettifilo, where the track turns about 90 degrees: 60 m "outward" landed back ON the road two corners along, putting a 2.22 × 0.20 m window band 0.75 m over the racing line. This pins the `rejBox` guard that now keeps it off. **It carries its own pack loader**, because the node harness has none: `js/render/shared/assets.js` is in the manifest's `FULL` list and not `TRACK_VM`, so `Assets` is undefined inside `tools/lib/track-build-vm.cjs` and every fleet sweep is blind to all 36 baked models (`docs/notes/DEFECT-LEDGER.md`, open). Three tests: the mirrored `_parseModel` must agree with the manifest's own vertex and triangle counts on every model, so a format change cannot leave it silently decoding the wrong geometry; monza must GAIN vertices when `Assets` is supplied, which is the anti-vacuity check — without it the road assertion would pass while testing an empty pack, the state every sweep runs in today; and monza must read ≤ 0.20 m over its racing line. Measured: 15 of 52 circuits over tolerance without the guard, 14 with it, the remaining 14 being the overhang class a footprint test cannot reject |
...
| `sheetshape-keyboard.test.mjs` | SheetShape's `--kb` keyboard band is a subtraction over TWO viewports (`innerHeight` minus `visualViewport.height`/`offsetTop`) and watched only the visual one, so a rotation out of portrait-with-keyboard latched a band computed from a mismatched pair — ~454px of `.screen` padding-bottom held in a 390px-tall landscape viewport until reload. Drives the rotation sequence in a VM on `mini-dom` with a scriptable visualViewport, and keeps the pinch / URL-bar guards honest on the new window path. `ui-resize.spec.js` pins only the CONSUMPTION half and says so |
| `apca-timing.test.mjs` | the sector strip encodes state in colour — purple session best, green personal best, yellow slower than your own — read at 14px in a corner at 300 km/h, and they were not equally readable: `--faster` Lc 79.4 and `--sec-slow` Lc 80.4 against `--sec-best` **Lc 50.4**, so the BEST lap you can set was the hardest to read. Judged in APCA rather than the 4.5:1 used elsewhere because WCAG 2.x "overstates contrast for dark colors to the point that 4.5:1 can be functionally unreadable" and ignores font weight. Asserts a BAND, not a threshold: `.sec-val` carries a dark `text-shadow` halo that lifts effective contrast, so every Lc is a floor — but all three share that halo, so the SPREAD between them is the robust quantity and the 30 Lc gap was the defect. Carries the APCA implementation checked against both published anchors (#000/#FFF = 106.04, #FFF/#000 = -107.88) so a wrong constant fails before any palette assertion runs |
| `font-digits.test.mjs` | the ~36 `font-variant-numeric: tabular-nums` declarations on `--font-hud` are INERT and this pins that as a fact rather than leaving it to be rediscovered. Rajdhani's full TTF carries 13 OpenType features, ALL under script `deva` with no `latn` entry, so Google's subsetter serves a Latin face with a literally empty feature list; its digits are proportional (0.208em spread = 7.07px of jitter at `#hud-speed`). CSS cannot synthesise a missing feature and says nothing when it fails, and `@supports` tests the PROPERTY not the FONT. Asserts against `tests/data/font-digits.json`, the fontTools-measured ledger `tools/check/font-digits.py` writes — and the anti-vacuity property IS the ledger: a font added without re-measuring has no entry and fails here, which matters because this bug class is exactly "a check that reads as working and is not". Also pins that `3ch` was NOT the culprit (1.1% short of `"888"` — the jitter is the string sliding inside a box that barely moves) and that the 36 declarations stay put, since they become correct the day the face gains real tabular figures |
| `css-faces.test.mjs` | an unterminated CSS comment ate EVERY self-hosted webfont for ~85 minutes on the deploy branch (2026-09-14, `ebde6cd`..`58bc33c3`): four `@font-face` blocks went, and the terminator closing the comment above them went with them, so the comment ran to the next terminator further down and swallowed the three Rajdhani faces too — measured at that revision, **zero live `@font-face` blocks**, the game in system fonts. Only two BROWSER jobs went red (golden menu screenshots; a `ui-redesign` CLOSE button 39.2px inside a 36.2px head), each costing a group and a bisect to read, while the whole no-browser suite stayed green — a `grep` for the woff2 filenames still matched all three Rajdhani lines, because they were commented out rather than removed. A brace-counter would NOT have caught it either: checked against the real revision, every `/*` there was closed. So this parses instead — comments stripped with a state machine, then every woff2 on disk must be named by a LIVE `@font-face` src, and no font reference may exist only inside a comment. Anti-vacuity is exercised, not claimed: the last case feeds the check a brace-balanced fixture of `ebde6cd`'s shape and fails if it reads healthy |
| `steel-role.test.mjs` | `d02d19bd4` standardised section chrome on `--steel` ("leftover idle ink and leftover-red headlines move onto `--text`/`--steel`") — it picked the token, it did not measure it. `#8a8a96` is APCA **Lc 39.8** on `--bg` (39.3 on `--surf-1`, 37.7 on `--surf-3`), carried at `--fs-micro`/`--fs-2` on settings labels, section headings and `#howtoplay dt`, under a comment claiming "at readable contrast". It was not given a better literal, because there is no room for one: `--dim` does the same job at Lc 48.0 and the two are the SAME INK — identical OKLab hue angle (−74.1°), chroma 0.0178 vs 0.0203, differing only in lightness (0.637 vs 0.691) — so sweeping lightness with a/b fixed, every value clearing a defensible floor on the worst ground is at or past `--dim`'s own. The step goes and the name stays: `--steel: var(--dim)`, verified in Chromium because the declaration sits ABOVE `--dim` and a forward `var()` looks wrong (it resolves — substitution reads the referenced property's computed value, not source order). Asserts the APCA anchors; that the alias holds and the ink measures 48.0/47.6/45.9 on the three grounds, each better than the `#8a8a96` it replaced; that the finding itself still holds (the replaced value vs `--dim`, since asserting the alias against its own target would be vacuous); and that ~30 call sites still name `--steel`, so nobody "simplifies" the role away |
| `scale-defaults.test.mjs` | the `(pointer: coarse)` defaults, and the two floors they serve. css/tokens.css owns FIRST paint and js/ui/scale.js owns every write after it — two copies of the same three numbers with nothing holding them together, until both were moved to one person's phone settings (`--hud-scale` 1.24, buttons 1.4536x that = 1.80) and the driving controls landed on the readouts on every phone: `#btn-brake` over 119x53px of `#minimap`, `#btn-boost` over 59x84px of `#hud-sectors`, red in `hud-layout.spec.js` for six days unnoticed because test:ui is not in the fast tier. Pins the lockstep, and the ARGUMENT: interactive TARGET size has a physical floor (~15mm phone per XAG 107, 0.96cm per GAG, 48dp per Android) while READOUT size has a separate px-at-DPI legibility floor (XAG 101, whose mobile figures are a DPI correction, not a touch bump) — so the button ratio must carry the physical floor and `--hud-scale` must NOT carry a blanket one. Bounds the pedals both ways (past ~28mm the column reaches the minimap) and asserts the old 1.80 stays reachable on the slider, because the default is a floor, not a cap |
| `nontext-contrast.test.mjs` | WCAG 1.4.11 over the tokens, scoped to what 1.4.11 actually asks: the FOCUS RING (pure non-text information — white, swept at 3:1 against every surface it can land on, including the red primary button and the selected plate), the SELECTED boundary (`--red` vs the ground outside and its own `--plate-on` fill inside), and the standing claim that `var(--accent)` is DECORATION — racingbulls `#1634cb` 2.21:1 and williams `#0f3cc9` 2.33:1 over `--bg` are legal only while every consumer is ornament, so the consumer list is frozen and the team-skin blocks may re-point `--accent` and nothing else. Deliberately NOT a blanket hairline sweep: `--plate-line` at 1.55:1 is asserted to STAY under 3:1, because 1.4.11 exempts a boundary around a control its own text label identifies, and a sweep would have 'fixed' the whole UI's control edge. Carries an OKLab `color-mix` resolver checked against Chromium's computed values (agrees to ~5dp, alpha-scaling and premultiplied-`transparent` cases included) |
| `road-under-floor.test.mjs` | no visible road surface may sit below the flat floor plane |
| `coplanar-faces.test.mjs` | ratchet: SAME-FACING coplanar faces — the pairs that z-fight at every distance, which `clip-audit` structurally cannot see. The default and `--overhead` sweeps come from ONE build per circuit (`coplanar-audit --all --json --both`), each half identical to its own run |
| `props-tri-ratchet.test.mjs` | ratchet: per-circuit props TRIANGLES after the hidden-face strip, against the exact counts in `tools/track/props-tris-baseline.json` — fails on > 0.5 % growth (or a circuit missing from the baseline) and on a stale cap (measured < 99 %: lower it). The same `tools/track/props-tris.cjs --all` sweep proves the build's vertex compaction (`TrackHiddenFaces.compact`) render-identical: every triangle corner keeps its position, normal, colour and material. `test:sweeps` |
| `scenery-ground-audit.test.mjs` | ratchet over `tools/track/ground-audit.cjs`, the small-scale vertical checks the other scenery gates are too coarse for: BURIED (a prop whose whole top lies > 2 cm under `Tracks.terrainY` — invisible paint, decals, slabs at the wrong height), UNSUPPORTED (a prop whose bottom stands > 0.15 m above terrain/road/water and that no chain of touching prims connects to the ground — `float-audit` only flags bases > 1.2 m up within 6 m, so a lamp head 0.5 m off its yoke passed) and FLAT COPLANAR (`coplanar-audit --flat`: same-facing HORIZONTAL faces, which `coplanar-faces` never counts). The fleet baseline is `tests/data/scenery-audit-baseline.json`, a ratchet (`ground-audit.cjs --all --update` only lowers; `--all --gate` is the whole-fleet check, ~3 s a circuit); this file samples monaco/monza/spa so it stays under a minute: over a cap fails, a cap above the measured count fails as slack, and an anti-vacuity test needs every check to find something on the sample |
...
| `lamp-bake.test.mjs` | `LampBake` bakes the lit shaders' diffuse pool exactly: two aimed lamps checked texel-by-texel against an independent evaluation of the shader term (windowed 1/d² with LAMP NEAR CLAMP, the cone smoothstep, N·L for N=+Y) within 0.2% after half-float encoding; read through the sparse tile atlas the way the shaders do; empty tiles are empty in the indirection, slot gutters equal their neighbours (no bilinear bleed), the atlas stays under its texel budget; ground height feeds the bake; the cache rebakes on a new light-set array or near clamp; `toHalf` round-trips |
| `frame-lights-shed.test.mjs` | A governor step does not pop the lamp set: lighting follows the tier only after it holds 1.5 s, the slot cap then slides at most one slot per 16 ms frame to the tier-2 budget, a step reverted inside the hold never touches the lamps, halos fade (not cut) at a held tier 3, and a device booting at tier 2 starts shed |
...
| `garage-sign-occlusion.test.mjs` | no wall sign in the bay has opaque geometry mounted within half a metre in front of it. Builds the scene in the VM harness with a canvas and LiveryTex that accept anything, rasterises every wall-mounted dress and live quad, and fails when more than 5% of one sits behind near geometry — the band where a bracket, shelf, bezel or rail hides a sign from every eye in the bay. Found five on the shipped tree: the live telemetry traces drawn inside the monitor bank, the door fascia wordmark drawn inside its lit fascia, two shutter signs under the guide rail, and the budget board behind two tyre stacks. |
| `front-wing-decal.test.mjs` | the front-wing endplate decal lands ON the plate at every aero recipe. The plate's height, outboard kick, thickness and taper all move with the aero level and the recipe's `plate` pick (four profiles), so a decal placed from literals floats — the first cut sat 641 mm inboard, having used the plate's half-thickness without its centreline. Builds the real quads (`CarMesh.carDecalData`) and the real car (`Car3D.build`) and measures every endplate decal vertex to the nearest car triangle, over four teams crossed with ten recipes covering plate profiles 0-3. `Car3D.frontPlate` is the one placement both the mesh and the decal read. |
| `livery-contrast.test.mjs` | no design paints a LARGE AREA it cannot be seen on. Every visibility failure found so far is one shape — a colour that owns a big piece of a panel and does not separate from what it covers — and liverytex answers that question at a dozen sites with nine different thresholds, so a new design is only as safe as whoever wrote it remembering to ask. This asks once, knowing nothing about which code path painted what: the real `buildAtlas` into crest-sweep's recording 2D context, the ops that own >= 15 % of a region, each against the paint beneath it (halo passes and the crest's own redraws excluded, or every glyph and every crest reports 1.00 against itself). The slice here is the crown designs that repaint the cover flank crossed with the flank designs that stand on it, over every team; `tools/car/livery-contrast.mjs` sweeps all combinations of `SPINE_LOGO_IDS` × `SPINE_SIDE_IDS`. Caught: the wrap's sun at 1.09:1 on Ferrari's own cover, and (since culled) the old split/bars/slash painting the saddle's colour onto itself at 1.00:1; flank wordmark/duo under saddle now clamp onto the panel with a forced halo so they no longer span the rake onto bare cover. |
| `livery-decal-surfaces.test.mjs` | every OTHER livery decal lands on the surface it is painted for, measured vertex-to-SURFACE (`tests/helpers/mesh-distance.mjs`) rather than vertex-to-box — the substitution that hid all six of these. The rear-wing sponsor band sits on the flap's POSED skin at every aero level (the recipe's design chord is 19.5 degrees off where `drawAeroFlaps` hangs it, so the band floated ~90 mm over a parked car) and on the topmost element, moving to the BAKED plane with a DRS package. The fin panel and badge sit on the blade at every `spineHeight` crossed with three engine cover heights, above the cover skin at each vertex's OWN station — and where a crown tops the regulation fin line, the decal is DROPPED rather than drawn inside the bodywork. `Car3D.COVER_STACK` orders bodywork under the livery, and the tail strip clears the heat shield and the spine vent while the flank band clears the accent pinstripe and the service hatches. The cockpit nose decal lists its corners front-first, like the race quad: reversing them flips v alone, which is a reflection, and the driver number rendered backwards. |
...
| `glx-occlusion.test.mjs` | the FAIL-SAFE contract of GLX occlusion culling (`apex26.occlusionCull`, ships off): hardware `ANY_SAMPLES_PASSED_CONSERVATIVE` queries skip chunks the depth buffer says contribute no pixel, and the saving is real (a census measured 94 % of candidate chunks skipped at vegas on Apple/Metal, gpuErrors 0, meanLuma unchanged at 27.3). What this file tests is the other half, the half a wrong answer makes catastrophic: a hole in the world is worse than a slow frame, so EVERY way of not knowing must resolve to VISIBLE. Against a stub GL that can refuse to compile, refuse to link, withhold a result or answer hidden: off leaves the draw path byte-identical; a chunk is culled only after a query has actually answered; a pending result, an unlinked program and an uncompilable shader each keep it drawn; and toggling the flag either way drops the visibility state, because stale flags re-applied on a later enable would hide chunks in a scene that has moved before a single query re-tested them |
| `glx-multidraw.test.mjs` | the EQUIVALENCE proof for `WEBGL_multi_draw` (`apex26.multiDraw`, ships off): collapsing the per-chunk `drawElements` into one `multiDrawElementsWEBGL` per light-set group must change how many calls carry the ranges and nothing else. So both paths run the same fixture and the same camera against a stub GL that records every (offset, count) it is handed, and the two must cover the identical set of index bytes — merged or split, one call or many. The risk it guards is specific: `drawElements` can merge only CONTIGUOUS chunks while a multi-draw group survives gaps, and a group left open across a CULLED chunk would undo the cull silently, in exactly the case (most chunks culled) where the feature is meant to pay. Also pins the fallback — a missing extension must report unsupported, must not latch the flag on, and must submit the same indices, because Firefox is at 1.4 % support — and that toggling off returns the path to `drawElements`. Deliberately NOT a timing test: run 141 measured the timing harness's own noise floor at 18-45 %, so counting is exact here and timing is not |
| `vertex-pack.test.mjs` | the packed world vertex format is a contract between files that cannot see each other — `js/render/shared/vertex-pack.js` writes the bytes and `LIT_VS` decodes them in GLSL — and nothing at runtime checks the pair agrees: a drifted scale draws a world that is merely the wrong brightness, or picks the wrong procedural material, and no SwiftShader render would call either a failure. So the encode scales are parsed out of the JS and the `#define`s out of the GLSL and compared; both are asserted to be exact divisors of 65535, which is WHY every material id (0-16 track, 20-32 car surface) decodes bit-for-bit — the `int(aMat + 0.5)` the FLAG cloth-wave and every car-surface branch key on; and the round trip is measured against the values actually in the tree (colour to the fleet maximum 3.4, the FLAG wave weight, normals under 0.01°), with out-of-range inputs asserted to SATURATE rather than wrap. Also pins the stride at 28/40 bytes so the 30 % VBO saving cannot be quietly given back, and checks every `vertexAttribPointer` offset is a whole multiple of its component size — a misalignment WebGL rejects outright. Also covers the shared half-float encoder WGX writes its `float16x4` colour lane with (TLX has its own, and three exports none): whole numbers exact to 2048, the mean signed bias asserted at ~0 because a TRUNCATING mantissa — which the copy in `tlx-chunked.js` uses — comes back systematically dark, and overflow asserted to saturate to Infinity rather than wrap to a small number. Finally it pins TLX's SECOND half-float encoder (`tlx-chunked.js` twiddles bits where this one does arithmetic, and TLX is DEFERRED so it cannot call the shared module at eval time) against this one value-for-value across the colour range — two encoders is a standing invitation to drift, and the TLX copy shipped a truncating mantissa, i.e. a darkening bias on every emissive colour and material id on both legs, until it was measured |
| `tex-census.test.mjs` | the texture-byte instrument, tested against the REAL `glx.js` through `tests/helpers/glx-mock.mjs`. It exists because the biggest texture number in the game was ARITHMETIC — ~147 MB of livery atlases, ten times the packed world geometry, with no hook able to confirm or refute it. Asserts the EXACT mip-chain sum rather than the `w*h*4*4/3` rule of thumb (that rule assumes a square texture and the livery atlas is 1024×1280, so the test also asserts the rule-of-thumb answer is NOT what the census returns); that a freed texture leaves the ledger, so the number is RESIDENT rather than cumulative; that a lost context reports nothing resident instead of the last value it held; and that an empty census reports 0 and names its `excludes`, never `undefined` — §2i and §2j of the perf ledger are both about instruments that could not say "I measured nothing" |
| `livery-tier.test.mjs` | which resolution a car's livery atlas uploads at — the biggest single memory decision in the renderer. `texCensus()` measured 146.67 MB of livery atlases on a full grid against 13.8 MB for a mean circuit's whole packed world VBO, so the desktop tier now sends AI cars to 512×640 and keeps the player's car authored-size (147 → 42 MB measured). Rasterising a livery needs a browser (the boundary `tools/car/parts-sweep.mjs` draws), but the tier DECISION is arithmetic, so `atlasDiv()` is pure and pinned here headlessly: the exact divisors per tier, mobile asserted UNCHANGED, the grid saving asserted to exceed 90 MB, and an ordering guard so a swapped ternary cannot pass by rewriting the exact values together. Also pins the close-up escape hatch — photo mode must request the full tier for EVERY car, and must not fold that into `usePlayerSetup`, which selects the player's SETUP and means something else |
| `material-shimmer.spec.js` | does baked tarmac CRAWL when the car moves |
| `render-boot.spec.js` | the BLOCKING renderer gate: GLX and TLX each boot, install (or deliberately do not install) their backend marker, report a live `gpuErrors` reader at 0, and present a frame. Deliberately cheap — no `test.slow`, no `test.setTimeout`, three tests — because every other `test:gfx` spec declares 240-540 s against the change-aware gate's 180 s cap and is therefore excluded, which left `js/render/` routed to nothing that can fail a push. The bare WebGL2 path is identified by the ABSENCE of a marker, so it is paired with a live `getContext("webgl2")` (a canvas WGX has claimed returns null). WGX is not here: the selected gate runs under llvmpipe, which has no `navigator.gpu` |
...
| `wgsl-bindings.test.mjs` | Every WGSL program in `js/render/webgpu/wgsl-chunks.js` may only name the resource bindings IT declares. The file holds several complete, independent programs that do NOT share a scope: LIT binds `F : FrameU` at `@group(0) @binding(0)` and SKY binds `U : SkyU` at the same slot. `9f2c6dc83` added the painted pit lane and wrote `U.pitLane` inside LIT, where `U` does not exist — a plain compile error (`unresolved value 'U'`), fatal to the whole WebGPU lit shader on every device. Nothing caught it: WGX is opt-in with no `<script>` tag, so the normal gate never builds that shader, and it took gpu-census run 113 on a macOS runner to report it. The check is member access (`Name.`) on an identifier another program declares as a binding and this one does not, so it cannot fire on a same-named local. Comments are stripped first and that is load-bearing, not tidy: SHADOW legitimately carries `// match GLX uInstanced / LIT D.mat2.y` in prose. Run against the pre-fix file it reports LIT/`U` at line 1028 — the exact line Dawn named |
...
| `model-pack-format.test.mjs` | the AX26 **v2** packed model layout. Every model in `assets/pack/models` is fetched at boot (`Assets.loadModels()` from `js/game.js`), so their total IS first-load latency; v2 packs a vertex into 22 bytes against v1's 40 and an index into 2 against 4, taking the shipped catalogue from 2.95 MB to 1.60 MB. The packing is only safe because of what the models contain, so the suite checks that rather than the arithmetic: every shipped model is really v2 (a silent fall back to v1 gives the bytes back and nothing else notices); positions, indices and material ids come back BIT-EXACT; colour stays within half an 8-bit display step and normals within 0.01°; a mesh v2 cannot hold — emissive colour past 1.0, a fractional or >255 material id — FALLS BACK to v1 rather than wrapping; v1 still parses, so an older pack is not bricked; and the manifest md5s match the bytes on disk. Reads everything back through the game's own reader via `tests/helpers/ax26.mjs` — the two suites that used to hand-roll the layout now do the same, because their copies would have kept passing against bytes the game could no longer read |
...
| `browser-ui-hunt.spec.js` | focused regressions for Race Settings Duel help ownership, Time Trial Daily selection chrome, Data Hub Results name wrapping, Live scroll ownership, Export readiness copy, and Telemetry comparison guidance |
...
| `audio-tune.test.mjs` | the player ENGINE TONE layer keeps the engine's timbre contract: pitch stays monotonic in rev and gear 1 below gear 4 at redline under EVERY profile and at both ends of every slider, a non-finite stored value cannot reach `playbackRate`, a profile replaces rather than merges, muted layers go silent, the CAMERA MIX (chase neutral, TV pulls your car back and opens the reverb, onboard closer), the downshift rev flare, the CarSfx scrub / lock-up / surface / pit-limiter levels and the wheel-gun edges — plus the static seam between `js/audio/panel.js`'s id/key tables and the shell |
| `radio-voice.test.mjs` | the radio spoken aloud: plan() as a pure policy function and every refusal reason it can carry; that create() reads the voice list ZERO times, so a speech API can never reach the boot path; that the SPEAKERS map stays total over js/game.js's ANN_PRI (scraped, so a new kind fails here rather than speaking in the wrong voice); that no shipped announce() line can still be speaking after its card has gone, asserted against the real strings and durations; the cancel-speak-RESUME triple that works around Bugzilla 1522074, without which the PENALTY that interrupts is the line that goes missing; and that an empty voice list still speaks, which is the Safari path |
| `rival-audio.test.mjs` | the field around you in the player's TRACK frame: left/right lateral sign, ahead/behind arc sign ACROSS the start-finish wrap, closing-vs-opening for a car ahead and behind, nearest-four selection, retired/self exclusion, each rival's own power-unit voice following its car through a re-sort, and the reused-row contract that keeps it allocation-free in the hot path |
| `voice-pack.test.mjs` | the RECORDED radio voice and the spotter: the clip composer takes the longest key and refuses any line it cannot cover whole, the committed `assets/voice/george` pack is contiguous, Apache-2.0 and under 3 MB, every `eng.*` line without a lap time composes for every driver on the grid, RadioVoice hands a covered line to the pack after the courtesy figure (music ducked) and falls back to synthesis otherwise, race control never recorded; the spotter's debounce, one clear per call, one still-there, side by lateral sign across the wrap, pit cars ignored, pack-only |
...
| `data-schedule-tz.test.mjs` | the SCHEDULE tab with `TZ=America/Los_Angeles`: a timed entry's date comes from the same instant as its start time in the viewer's zone (Las Vegas 2026-11-22 04:00Z reads 21 Nov), a date-only entry stays a UTC calendar day, and NEXT is decided against the clock (the race instant plus its running time), not today's UTC date |
| `fin-design.test.mjs` | the tail DESIGN picks a livery can make. Geometry: `Car3D.FIN_SHAPES` — an absent `finShape` builds byte-identically to before, `sharkFinBadge()` with no arguments still lands on z −1.235/−1.465, every shape's badge and panel sit inside that shape's own blade, and `none` removes the blade AND its decal quads so no graphic hangs in the air. Atlas: `finStyle: none` paints nothing into the fin or cover regions, `spineLogo: none` leaves the crest off the spine while the fin badge keeps it, the crown carries no tail wash at all so the fin motif stops at the fin, and every other SPINE TOP design (saddle, panel, stripe, twin, wordmark, carbon, number) paints the bare crown with text only where it carries a name or number, the band designs alone run down the tail strip, and the photo-derived side designs (`SPINE_SIDE_IDS`: plate, wordmark, duo, ribbon, lockup, title, emblem, band, sash — culled slash/bars/split/chevron paint nothing) carry number/name/colour fills only, every side design paints BOTH flank regions (the left in the atlas's extra rows) and both are bare by default, `finBadge: number` puts the race number on the fin, and `check` is a real motif Plus the body DETAILS the same editor rows carry: the stepped fin adds its rear block behind an unchanged front-blade outline, cover vents add geometry while the T-cam pick changes only colour (auto reads car 1 black / car 2 yellow off the driver slot), the driver-code badge lands the three letters on the fin, the mirror lamp anchors follow the team's mirror style and are cached, and the 2026 ERS light code flashes once per second at full deploy, rapidly when a full battery clips under braking, and stands aside otherwise; spineHeight lifts the cover crown's top line by its declared rise, tapering to the tail with the floor fixed, shares the cached anchors at "standard", and never moves the fin top; spineSide leaves the flank band bare by default, paints the number / code / crest there on pick, and moves the service panels out from under it |
| `parts-distinct-mesh.test.mjs` | Every one of the 297 catalog options must build a car DISTINCT from the others in its category, and none may build the team's default — a part you pay for and cannot see is indistinguishable from one that is not wired up. Hashes `Car3D.build`'s positions, vertex colours and material ids (pure geometry, no canvas), which answers in 10 s what `tools/car/audit-parts.mjs` needs ~2 hours of SwiftShader and 300 PNGs to only half-answer. Each option is built against a team it is LEGAL for: SIGNATURE parts are team-locked and manufacturer parts supplier-locked, so asking for one on the wrong team correctly resolves to the default — the first version of this check read that as 100+ dead options. |
| `team-livery.test.mjs` | ONE `livery:` key per team record, and every field it names is one `Liveries.forTeam` actually copies (plus `spineHeight`/`finShape` against the real `Car3D` id lists). Two sessions each added a `livery:` block to the same team objects; git merged both without a conflict, JavaScript kept the LAST duplicate key, and Ferrari's and Mercedes' whole engine-cover design went silently dead. A merge hazard rather than a typo, so the guard is on the file's shape. |
| `body-split.test.mjs` | `liv.bodySplit === "lr"` recolours chassis→livery verts by `sign(x)` so Cadillac's left body is `c1` and right body is `c2`; absent `bodySplit` keeps a single body colour on both flanks. Pins the full-body L/R path (not atlas-only) Cadillac ships. |
| `cover-legibility.test.mjs` | Every shipped default's crown and flank design must CONTRAST the cover it sits on (floor 2.0, measured by replaying the atlas and sampling the dominant paint). Four covers shipped invisible: Haas's dark weave and Audi's mark on near-black cars, then a white saddle and teal stripes on the pale covers another session gave Ferrari and Mercedes. The first two were caught by eye on track; this file caught the other two in seconds. |
| `fill-gating.test.mjs` | The garage greys a design-fill row (BAND, SADDLE, FLANK FILL, SUN, 2ND BAND, PLATE) when no design can spend it, and `garage-angles --plan` warns that such a flag is inert — both from ONE table declared beside the painters (`LiveryTex.FILL_SURFACES` / `liveFills`). This proves that table against the RASTERISED atlas: every id the map calls live lets a probe colour land on some car, and every id it calls inert changes no cover pixel. Its first run caught `rake` reading SADDLE only under a saddle-wrap bind, and the `ridge` / `fade` crowns re-picking an authored BAND. |
| `helmets.test.mjs` | one hand-made helmet design per race number in `js/car/helmets.js`, and — the reason the module exists — none of them the colour of the car it sits in (the helmet used to BE the team's paint: a papaya dome in a papaya McLaren, identical for both team-mates). Also: every design's `alt` is a real escape when a custom team's paint matches its base, every zone kind resolves and every colour is an rgb triple in range, the painter lands at least three colours on the shell so no design is a plain ball, the visor is an aperture over the eyes rather than a band round the head, `shell()` marks it glass, a number off the grid gets a stable distinct design, and car3d paints through `Helmets.shell` with no accent table of its own |
...
| `ghost-share.test.mjs` | APXG1 compressed/plain codec round-trips, corrupt and unknown-track refusal, 14 KiB fragment cap with file fallback, session-only guest replay/interpolation, selective `#ghost=` consumption, results export controls, and Phase 1 boot/HUD/world wiring |
...
| `tyre-model.test.mjs` | the tyre wear model in a VM — OFF is a true no-op through every multiplier the driving model reads (so the characterization baseline holds), compound life is a fraction of the SCHEDULED distance rather than a lap count, the stint floor keeps a 3-lap blast off the cliff, severity orders and clamps, the grip curve is linear then a cliff with traction taking a smaller share, and the catalog's `life` ladder trends against grip, stays inside the clamp and spans a real strategy range (the one guard on it — `tools/car/parts-ladder.mjs` deliberately does not score `life`). Plus the thermal layer: a fresh set comes out of blankets BELOW its window (the out-lap), driving brings it in and holds it there, a softer compound switches on faster, the bulk lags the surface both ways, graining accumulates cold-and-sliding and HEALS while blistering accumulates from a cooked core and never does, colder weather means a colder tyre, and OFF still costs nothing. Plus per-axle: the two shares always average to exactly 1 (so `c.tyreWear` is untouched), braking wears the front and traction the rear, brake bias moves the braking half only, and the axle split is a RATIO against `gripMul` so game.js cannot count wear twice |
| `body-attitude.test.mjs` | `js/physics/body-attitude.js`, the cosmetic pitch/roll/heave springs, as pure numbers in a VM (the 2026-09 coverage census's one zero-reference module). Sign parity — a human car (speed·yawRate) and an AI car (−speed²·curvature) lean the SAME way through the same corner, both directions; the closed-form spring is critically damped (monotone approach, no overshoot, settled within 2 % at 0.75 s); pitch and roll caps hold and heave saturates through the tanh knee below its ±5 cm rail; a 10 s frame is clamped to `MAX_DT`; full downforce heaves less than none; disabled returns zeros, settles every car and persists under `apex26.bodyAttitude` |
| `pit-lane.test.mjs` | the pit lane's pure geometry on every circuit without a browser (and the note that a planned AI stop arms when the lap counter turns at the line, inside the window, and is served at the end of that lap: `h_armedAtLine` in the hunt, not a missed stop) — the window WRAPS the start/finish line (a naive range test is wrong for every lane in the game), the lane is measured in METRES so a long lap does not get a long pit lane, a short circuit is not swallowed by its own lane, street circuits take the lower limit, and the box window is long enough to stop in without being a trap. Plus the CUE that makes the gesture findable — it stays quiet on a fresh set (a permanent PIT sign is wallpaper), counts the entry down in metres once the set is used, says ENTRY only while the entry road lasts, follows the car through armed/lane/box and then shuts up, speaks only to the local player, and always has a SIDE to point at — and `worthStopping` IS its gate: the minimap's "P" (js/ui/hud.js) and the cue read one function, so across every (wear, tread, caution) triple the cue speaks exactly when the marker lights, and the map's distance arc starts where the cue's countdown does (`cueM`, 550 m); the EXIT ROAD speaks (GO GO GO for `servedS` after the release, MERGE naming a car closing inside `mergeS` on the racing surface — a car in the lane is no threat — else the metres to the end of the road, with a bar fraction); the release announces what the stop cost (the hold, the place, the places — and no "GO GO GO" at the START of the hold); the armed cue names the compound the crew will fit and shows whatever the wear gate thinks; and the first stop is taught in three lines, each once. Plus WHEN the cue asks for the working lane and WHAT it takes to be stopped in a box: inside `BOX_CUE_M` (90 m) it COUNTS the box down while the car holds the fast lane — reported as a defect, "it's wrongly telling me to stay right before it's my time to pull over", because the ask used to ride the whole 90 m at a driver doing the right thing — and it says KEEP RIGHT/LEFT only inside `MOVE_M` (36 m, a second and a half at the limit); and the latch is `boxSquare`, not merely "past the working lane's line": the car must be within `BOX_SQUARE_LAT` of its OWN box's centre and, where a live heading exists (the local player's, from game.js's bicycle model — an AI's `c.head` is written at resets only), within `BOX_SQUARE_RAD` of the lane's tangent, so a car parked 2.4 m wide of its bay gets no crew and is told SQUARE IT UP rather than STOP HERE. Plus the PLAYER'S REFERENCE PLAN: a human's plan never arms a stop (`think` is the AI's — the same car as an AI fires it), `planInfo` reads it for the HUD (the stops, the next box lap, soon / now / served, a called stop carries no urgency), `windowOf` names a rival's window for the gap chips (P<lap> within three laps, IN while stopping, nothing for a missed one), and `lossS` is the lane's net cost that `estimate` agrees with off a caution. Plus the COMMITMENT that replaced the pit button: no pit control survives anywhere in the input layer or the shell, holding the line into the pits calls the stop while a drift / a transient run-wide / the wrong side does not, a spun, beached, reversing or parked car commits to nothing, you can commit anywhere up to your own box but never past it (the grid sits INSIDE the window, so an entry-only rule made a lap-1 stop impossible), only the LOCAL player steers itself in, and the commitment test is the one place the module samples the track |
| `pit-complex.test.mjs` | the pit complex is ONE MODEL (`TrackPit`, js/track/core/pit.js) and everything reads it, on real track builds. The lane used to be described five times — ribbon fit, stop row, painted boxes, scenery hulls, the GARAGE screen's frontage — with three bay pitches and four team-colour tables between them. This holds: every circuit carries a model (street circuits the STREET set built between their walls, no engine street barrier left on the lane, Jeddah's compressed row painted without bays; a `pit.mode: "narrow"` opt-out is the PAINTED lane that keeps nothing out); the bands are the regulation's (≥ 12 m wall-to-garage, fast lane ≤ 3.5 m, ≥ 1 m corridor) and the WORKING lane rides `b`, not `w`, so it exists only where the bays do — the entry road, the exit road and the stretches between carry the fast lane alone (the lane used to run full width for 100-238 m with no garage beside it), the driving boundary and the outer wall follow that same edge through `TrackPit.outerAt`, and the complex's flattened FOOTPRINT stays the full width so the apron can pave it; `hw` never moves; the entry road eases the lane off the road and the exit road eases it back, with the wall growing only once the lane has left the road; `pitLaneAt`/`inPitLane` read the model; the driving boundary is opened across the complex on the pit side only, to 0.9 m short of the garage line where the OUTER WALL stands; the exit road is at least twice ROAD_MIN and only the wall's fade must keep to the straight; the painted boxes, the stop positions and the garage doors are the SAME twelve arc positions; the entry HATCH (`TrackMesh.pitHatch`) is paint inside the road edge from the road's start to where the wall's fade begins, a stripe every 2 m (eleven teams plus MY TEAM, one bay pitch apart — and MY TEAM seated ONCE with the roster the game has, where the career module has already pushed it into `Teams.LIST`; a 13-bay row put its head through Yas Marina's hotel leg); no source keeps its own team table; a def's `pit.side` is honoured; the terrain is level under the complex; and the garages are `GarageScene.buildStatic`, the setup screen's own bay — the trackside `props: "lite"` tier bounded in vertices, with its two tyre stacks standing past the door plane on the apron. Two more rules ride here: the entrance carries a GATE of two lamps at the ENTRY LINE, one on each wall corner — the platform wall's nose on the track side, the outer wall opposite — at one arc with the lane's middle between them (the only arc where a wall stands on both sides: down the entry road the peel is a wedge off the road edge with tarmac on its track side), each a RED head on a short post off its own wall top, each aimed at the fast lane's middle a car's length in with a radius that is the throw and no more (a flat 24 m pool on a post 8.5 m off the centreline washed 16 m of racing line white), each registered as an `entry` light so the canopy's six are counted `pit && !entry`, and each with a GREEN decal quad in `track.pitSignal` on the atlas's signal cell facing the oncoming driver; and the MOUTH stands on the straight — for every circuit whose straight back from the line can hold `ENTRY_MIN + ENTRY_ROAD + MOUTH_RUN`, no node from `sA − MOUTH_RUN` to `sIn` corners (`|k| ≤ PIT_K`) and the entry road is its full length, so Monza keeps the 260 m window while Abu Dhabi and Sochi drop to 174 (the mouth used to sit inside the last corner on 21 of 52 circuits, Abu Dhabi's on a 23 m radius) |
| `pit-signs.test.mjs` | each team's identity on the OUTSIDE of its bay, on real track builds: `SceneryPits` lays out twelve 5.2 × 0.7 m fascia quads (`track.pitSigns`) in row order, a centimetre proud of the lintel, facing the lane, not mirrored on a left-hand pit, UVs inside their `TrackPit.SIGN` cell, none on a painted lane, nothing added to the props buffer, plus one to three BOARD quads after them ("PIT ENTRY" on the approach, "PIT LANE <limit>" at the entry line) facing the oncoming driver, on the atlas's seventh row, each an anchor for the distance gate, then twenty-four PANEL quads — each bay's crest cell again at 0.46 scale, standing on the pit wall opposite its bay, one face toward the lane and one toward the track, not mirrored — the two approach boards walk up to 24 m back past any single-object prop on the verge (`track.props`; Nürburgring's trunk) and stand with a decimetre to the barrier — then twelve CREST quads (the crest alone on the team's colour, `SIGN.crests` in the atlas's lower half) as 1.8 m plaques on each door's approach-side pier; the row carries each team's code; and the painter (`js/garage/pit-signs.js`) reports unsupported and uploads nothing in a bare VM, while with a stub canvas and `LiveryTex` it paints twelve cells and the lettered boards, uploads one texture and one texMesh of every quad, draws once inside 350 m of the row or a board and never beyond them or when hidden; the entrance lamps' green aspects ride the SAME atlas as a SECOND texMesh (`track.pitSignal`), uploaded beside the first and drawn only when `draw` is told this car is called in (red is what the props mesh paints, and the props mesh is built once) |
| `track-build-vm-release.test.mjs` | the sweeps harness's two MEMORY contracts, which every fleet suite depends on and none of them asserts. `tools/lib/track-build-vm.cjs` shares one VM context across all 52 circuits on purpose, so nothing is reclaimed unless the caller asks: `trim(from)` drops the primitive records (each one pins the whole mesh buffer its emitter wrote into), and `release(track)` drops a finished build's own vertex buffers — the `*Geo` arrays and the per-mesh `__cap` capture — which `trim` cannot reach because they hang off the returned track rather than the context. Both were being skipped: `pit-complex` and `pit-signs` cache one build per circuit to avoid rebuilding and so held all 52 circuits' geometry at once, peaking at 6173 MB and 5563 MB against the 4088 MB that has already OOM-killed an audit child on CI. Asserted on SLOTS REACHABLE from a track, never on RSS, so the guard is deterministic: an unreleased build must pin > 3 M slots (a FLOOR — if a build stops holding the geometry, `release()` is obsolete and this file should go, rather than passing vacuously), `release()` must free nine tenths of it, every field the two suites read must survive it unchanged (the centreline, `pit`, `graph`, `surface`, a mesh's `verts`, `propsGeo.uv` staying undefined), and `trim(from)` must truncate to the mark and clear the live-buffer set. Two circuit builds, ~6 s; it is in `test:sweeps` beside the suites it protects and in `toolingFast` so the edit loop catches it |
| `ai-strategy.test.mjs` | the field's race strategy in a VM — `stintPlan` scales stop count with the distance (a 3-lap blast never stops, a full distance always does) and its stints cover the race exactly, a 20-car grid produces at least three distinct plans (biasing only the stop COUNT put all 20 on one, which is a procession), a full tank costs tyre life so strategies MIX rather than running one compound all race, the plan is a pure function of its inputs, and `pitNow`'s reactive rules fire in the right order — weather over the free stop over a spent set over the plan; and the PLAYER's pins — a pinned stop count returns that count with the laps inside the race (a pin above the cap is the cap), a pinned start is the first compound (a wet start falls back to the classes), and `firstLife` shortens the FIRST stint only, so a re-plan runs the set that is on the car |
| `engineer.test.mjs` | the race engineer's ladder in a VM — it ADVISES and never arms a stop or touches a car's pit state (the AI has a plan, the player has an engineer, which is §11 decision 1 made good), it speaks only to the local player, the wrong tread and a free stop under a caution outrank every complaint about the tyre, graining and blistering say DIFFERENT things because one heals if you ease off and the other does not, the axle call names an end of the car and what to do about it but sits BELOW the defects ("brake earlier" is the wrong answer to graining), the wear ladder counts DOWN, it does not nag — one line at a time, no line twice, and a threshold crossed while the banner was busy is not silently spent — and it is ONE VOICE with the pit cue: while `PitLane.lastCue` is giving a direction (`DIRECTIONAL`: enter, keep, stop, merge) the engineer waits and the line it owes is said once the cue lets go; and the PLAN LINES sit between the tread and the tyre complaints, each naming a lap or a compound — a caution that fits the plan with margin ("STOP NOW LOSES NOTHING"), a rival's undercut (a car behind inside pit loss + 2 s whose stop count rose, said once), rain arriving before the planned stop (names the lap), BOX BOX BOX / BOX NEXT LAP with the compound (this lap beats the wear ladder, a tread call still beats next lap), and a called stop silences all of it |
| `race-radio.test.mjs` | the race radio (js/race/race-radio.js, race-facts.js, radio-lines.js) in a VM — every phrasebook line, filled with its longest plausible values, can be SPOKEN inside its own card (RadioVoice.plan); a pool deals like a deck (every variant once before any twice, never back to back); a half-filled template is skipped; the gap is the timing loop's time between two cars at the same line (a car braking does not move it); a pass counts only once the order HOLDS, and a car in the pit lane is never "overtaken"; a burst of lost places is one net call; the brain calls a gained place by name, holds every line below tier 5 while the driver brakes or is loaded in a corner, drops a defend call whose threat has gone rather than saying it late, goes silent at chatter OFF, sends a safety car at tier 5 over a busy card, keeps commentary to TV cameras unless set to ALWAYS, calls the result after the flag, and answers RADIO CHECK with the position and both gaps; and the post-merge regressions — a second race starts with a fresh radio, the result follows the order the flag fell in (finPos is 0 until endRace), a retirement ahead is told once and never as a pass, a red-flag restart is not read as a car closing, and a RADIO CHECK does not swallow a safety car on the same tick |
...
| `badges.test.mjs` | LICENCE BADGES (`js/career/badges.js`) in a VM over the real `GameStore` and a fake localStorage — the unlock rules as a pure function of the player's classified result (a win is also a podium, a retirement earns nothing, a clean sheet needs zero track-limits cuts and no penalty, fastest lap needs a finish, a win at a 2026 venue is that venue and all 23 unlock 2026 WORLD TOUR), the 7-day streak threshold, persistence under `apex26.badges`, missing / corrupt / wrong-shape storage reads as no badges without throwing, and an unlock never repeats (no second write, no second toast) |
...
| `duel.test.mjs` | the duel format (`js/race/duel.js`): `pick()` takes the fastest non-player car, the BUMP lifts craft/awareness harder than pace (a rival that is only faster reads as a cheat), and `asLegend()` hands the rival the legend's OWN five axes intact rather than bumping whoever was there — compared against a SNAPSHOT of the ratings, because an implementation that bumped them in place would otherwise be measured against its own mutation (two mutants of that shape survived the first version of this test) |
| `duel-row.test.mjs` | the DUEL row's rules (`RaceSettings.duelOpts`/`duelValue`): OFF and ON stay first and keep their old meaning, every legend reaches the row labelled by name, no legend id shadows `off`/`on`, every option round-trips through the two setters (a legend must set BOTH `duelMode` and `duelLegend` — one alone silently races the ordinary duel), a stale saved legend id still reads as ON rather than OFF, and with `js/data/legends.js` absent the row degrades to the OFF/ON it always was |
| `legends-team.test.mjs` | the LEGENDS team as a grid entry, separate from MY TEAM: its own `legends` id (sharing `custom` would overwrite the player's decals and parts sheet), the whole roster in `drivers` so the select screen's ordinary DRIVER picker doubles as the legend picker, `seatOf()` resolving by id and code, the team's tier/colour/stats following the legend it fields rather than just its name, and the ONE-SEAT rule — `drivers.length` is 12, so anything counting seats by it grids twelve legends (measured: 34 cars). Out-of-range and junk picks still grid exactly one car with a real driver |
| `legend-parts-seed.test.mjs` | the PERIOD CAR reaching the garage SHEET, not just the team record. A team's `factory` build renders an AI car, so a legend duel RIVAL already arrived in the right machine; the player's own seat reads `parts.legends`, and an empty sheet resolves to Parts DEFAULTS — so picking Fangio gave you a 2026 chassis in Silver Arrow paint while duelling him produced the 1954 car. Twelve legends share ONE `legends` id, so the WRITE POLICY is the behaviour and all four cases are pinned: seed an empty sheet, reseed on a real switch (you asked for the MP4/4, not Fangio's build in Senna's paint), and leave it alone on a boot or a same-seat re-sync — either would wipe a build the player spent credits on |
...
| `frame-hitch-analyse.test.mjs` | The hitch instrument's own arithmetic (`tools/gfx/frame-hitch.mjs`), on synthetic series whose answer is known. Exists because three performance findings in a row were retracted here and not one was a reasoning error — each was an instrument nobody had checked against a known case (`window.GLX` that never resolved so a whole memory column read as "flat" when it was never sampled; a `--steer-hz` driver that was itself a frame and moved the median deciding what "wide" meant; a "+23.8 MB/min leak" measured over a window that opened while the working set was still filling). `analyse()` must separate a periodic spike train from a flat run. `analyseHeap()` — the function the GC hypothesis rests on — is held both ways: it must NAME a spike train whose collections land on the spikes, and CLEAR one whose spikes sit elsewhere at the same allocation rate, which is the check that a naive "there are collections and there are spikes" verdict fails. And an all-zero heap series must report as unsampled, never as 0 MB/s. `analyseAlloc()` — the V8 sampling-profile reader that answers WHO allocates — must fold one site reached from two parents into one row (a split would rank a 60% allocator below a 25% one), must REFUSE to name a winner on a flat profile (a ranked list always has a first row, and calling it "the allocator" at an eighth of the bytes is how a round gets spent fixing 2% of a problem), and must report an absent or empty profile as absent. `analyseCpu()` — the CDP `Profiler` reader that answers WHERE THE CPU TIME GOES over the race start, the instrument that is honest for CPU cost where the heap sampler was not for garbage — must compute self time from `timeDeltas` (unequal sample spacing respected, not a hit count), fold one function reached from two parents into one row, fold by file, and report an absent profile as absent. Pure arithmetic, no browser (~0.2 s) |
| `tlx-mat-key-memo.test.mjs` | TLX's material cache key must be memoised without ever going stale. `matCache.get(key)` needs the string before the lookup, so `materialFor()` built ~15 intermediate strings on EVERY call, hit or miss, at 150-400 draws a frame; every caller passes a long-lived scratch object it mutates in place, so the same object produces the same string frame after frame. The memo skips the rebuild behind a field-by-field compare — and the failure mode it introduces is the dangerous one here: a stale key returns the WRONG material (a matte car, a car-paint road, an alpha-written decal), which no counter flags and only the screen shows. So this lifts `buildMatKey`/`matKeyFor` out of `js/render/three/tlx.js` and evaluates the REAL source (a re-implementation would test the test), then asserts the memoised key always equals a fresh build across every single-field mutation on one object identity — plus `depthBias` compared by value not identity, the chunked/instanced flags in the compare and not only in the key, and `Object.is` on a repeat call, which is the only way from outside to see that nothing was rebuilt. No browser (~0.1 s) |
| `tlx-inst-cells.test.mjs` | TLX's instanced-prop cull must reuse its resident pack by the surviving CELL SET, as GLX/WGX do (`InstCells`, PERF-FINDINGS §2c): the old key was the exact six frustum planes, which never repeat while the camera moves, so every prop batch re-packed n×16 floats and re-uploaded n×64 bytes every frame. Lifts the REAL `cullInstances`/`updateInstances` out of `js/render/three/tlx.js` against the real `InstCells` + `Frustum` and a counting stand-in for the InstancedMesh write: a moved camera over the same cells neither repacks nor uploads and does not stamp `_cullPlanes`; a changed set repacks; the shadow cull (`upload:false`) never consults or records the key; `updateInstances` invalidates it; `apex26.instCellCache=0` turns it off. No browser (~0.1 s) |
| `shadow-pass-depth.test.mjs` | the SUN shadow map's depth span must hold a 250 m caster (Singapore/Baku/Vegas/Jeddah towers): the light eye sat 150 m up the sun axis with near 1, so anything more than ~149 m toward the sun was near-clipped and cast no shadow. Drives the real `ShadowPass.sunPass` (with the real `M4`) on a stub renderer, captures the light VP handed to `shadowBegin`, and projects a 250 m prop top (and a receiver 165 m down-sun) through it at five sun elevations. No browser (~0.2 s) |
| `material-variants.test.mjs` | Draw options are TLX's MATERIAL KEY (`buildMatKey`: emissive and alpha quantised to 1/32, then the surface fields and flags), and a new key is a new NodeMaterial — a new program, its codegen, its pipeline, its first-draw costs. gpu-census 212, the first driven window on real Metal, counted twenty material misses in twenty seconds, one a second, the cadence of the "lags every few seconds" report: three draw sites fed the key a continuous per-frame value (the rear light's brightness followed ERS energy, the flame's alpha followed the pop, the aero-bar blink was a sine on alpha). Pins each to a few fixed levels and scans the draw modules for a sine or clamp multiplied straight into an alpha or emissive option. No browser (~0.1 s) |
| `coach-clone.test.mjs` | DrivingCoach's practice rewind captured three fields per car with `JSON.parse(JSON.stringify(x))` — 20 cars at 2 Hz, 120 serialisations a minute whose only product is a copy. `jsonClone()` walks the value instead, and the risk is not speed but SEMANTICS: `restore()` reads back exactly what `capture()` wrote, so anywhere the walk disagrees with the round trip it replaces is a rewind into a state the car was never in. The disagreements are all in the corners — `JSON.stringify` writes `null` for NaN and Infinity, drops an undefined property from an object but writes `null` for one in an array, treats functions the same two ways, normalises `-0` to `0`, and lets a Date through `toJSON` rather than copying its empty own properties. So this does not test against a DESCRIPTION of JSON; it lifts the real `jsonClone` from `js/race/driving-coach.js` and asserts `deepEqual` against `JSON.parse(JSON.stringify())` over every one of those shapes, plus the tyre/tyreLog/pitNext shapes the field actually carries, plus a no-aliasing check (a snapshot that shared the live car would be overwritten by the driving it exists to undo). No browser (~0.1 s) |
...
| `deploy-tool.test.mjs` | `tools/ci/deploy.mjs` offline: same deploy branch as pick-tests, `--help`, the circuit-touch detector, preflight refusals, and a real clean-but-stale merge (both sides add a unit file) ending with the ladder figures regenerated |
| `tooling-fast-runner.test.mjs` | `tools/ci/tooling-fast.mjs` itself: longest-first scheduling from `tests/data/tooling-fast-timings.json` (local runs override it), a hung FILE failing by name at the per-file wall with its process group killed, a hung TEST failing via `--test-timeout` |
...
| `physics-baseline-provenance.test.mjs` | `tests/data/physics-baseline.json` must say where it came from. The writer (the browser spec under `APEX_UPDATE_BASELINE=1`) stamps `_blessed: { sha, reason, at, hash }` and refuses an empty `APEX_BASELINE_REASON`; this holds the stamp on the fast gate: a non-empty reason, a data hash that still matches (a hand-edited number no longer does — the edit has to be a regen, and the diff has to be read), a full commit sha that is an ancestor of HEAD when the checkout can see it (a shallow CI clone skips only that half), and that neither reader of the file walks its keys. `tests/helpers/baseline-blessing.mjs` is the one hash both sides use |
...
| `pit-lane-vm.test.mjs` | the pit stop against the built complex, driven in the VM on Bahrain (~40 s): THE DIRECTIONS, FOLLOWED, MAKE THE STOP — a car that obeys every cue is measured against the model's own geometry at each phase (the countdown opens at `cueM` with the metres to the line to the metre, HOLD THE LANE from the peel-off, STAY IN LANE · BOX — <compound> once the lane's tarmac has been held for the dwell and still on the entry road, STAY IN LANE · <limit> from the line with the limiter holding, KEEP <side> at `BOX_CUE_M` from THIS car's box while the car is in the fast lane, PULL IN · <m> once it is in the working lane, STOP HERE inside `BOX_TOL`, the stop latched and served, GO GO GO for `servedS`, EXIT <m> to the end of the exit road, silence past it on a fresh set); a car on the racing surface never commits — not from the grid, not holding the pit-side third of the road inside the window (the old painted line sat 3.2 m inside the edge and armed the limiter on the racing line) — and a car on the lane's own entry road, held, does; a commitment made on the entry road can be aborted there by holding the road again (un-armed, no limiter at the line); an armed car already past its own box stays unlimited and comes in next lap; a car held in its box under throttle is serviced where it stopped, never auto-rescued onto the racing line; the pit wall stands on both sides — a road car cannot run through it, a lane car cannot rejoin through it; and an armed AI is guided (`laneX`/`entryV`, pure calls on stub cars): the pit side of the road for the last 200 m before the entry road, one monotone line that peels off the road and reaches the fast lane's centre at the line and blends back inside the road edge where the exit road ends, and a speed cap that is the limit at the line, a braking curve before it and nothing inside the window — and, at the box, a line that IS the box's own centre, square by `boxSquare` everywhere the latch can fire and dead on the centre for the last `SQUARE_BY_M`, because the diagonal used to finish AT the box while an AI halts `BOX_TOL`/2 short, so every AI parked four fifths of the way across its bay and the latch took it; and WORK ON CAR — the mid-stop GARAGE door: `canWork` is true only for the LOCAL car, stopped in its own box, while the hold is still running (not on the racing surface, not rolling down the lane, not on the way out, not once the hold has run out, and never for a rival), and `addWork` charges the stop `workS` seconds, extending the hold that is left and recording it so the release summary reports what the stop really cost; and the stop's animation schedule (`stopAnim`: on the jacks through the middle, wheels off between 15 % and 80 %, down at the end, nothing outside the box) |
| `elevation-tracks-vm.test.mjs` | Node twin of `elevation-tracks.spec.js` (400 s -> 191-210 s on 2026-09-16, 47/47 green either way — it BUILDS all 40 circuits and steps ~2,200 frames through each): all 47 tests — the four banking-geometry audits on `Tracks.buildCenterline`, the chase-camera bank roll via `camState()`, the per-circuit slope-gravity / climb / road-following probe and the two banked bowls, same launches and bounds. The 42 per-circuit races run through `tools/lib/game-vm-pool.cjs` (four worker VMs, one circuit at a time each, probes in `tests/helpers/elevation-probes.cjs`); every assertion stays in the parent, so the file still declares 47 tests against the spec's 47. `APEX_VM_POOL=0` runs the identical probes serially (400 s). Nothing left in the browser |
| `game-vm-pool.test.mjs` | The pool is a SPEED change and nothing else (~20 s, two VMs): the real elevation probes run on two circuits in a worker and in the parent's own VM, in the same order, and the result objects must be deep-equal float for float — otherwise the twin's thresholds are no longer asserted against the browser spec's numbers. Plus: a probe that throws fails ITS circuit by name (a pool that swallowed an error would turn a red twin green), and `createGame({ carMeshes: false })` stubs the builders and refuses a caller that MEASURES a mesh |
...
| `ai-racecraft-vm.test.mjs` | The complaints that outlived the weld fix (~25 s, one boot): an AI with more pace gets past a slower MOVING AI-driven car within 20 s and stays past (the pace-based `otWant` + the pass latch); two AI cars dropped side by side resolve within 10 s with neither stalling below 25 m/s and no contact afterwards (the one-yielder rule); and a standing start is a launch — speeds span ≥ 6 m/s at t=4 and the order changes ≥ 6 times in ten seconds (the per-car launch plan; before it: 4 m/s and 3); and, since 2026-09-08, the solo-lap JITTER caps on the same monza trace — steering reversals ≤ 6 /km and lateral acceleration RMS ≤ 7.5 m/s² (measured 4.7 and 5.9 after the heading-state controller; 5.4 and 10.0 before). Not a twin of any browser spec |
...
| `determinism-replay-vm.test.mjs` | Replaying one seed must be byte-identical — the player's trace AND the field's finishing order, because the 2026-09-08 break moved only the latter while every player-watching check passed. In `test:game-vm`, which the deploy gate always runs, so this class is caught in ~20 s rather than by whichever browser group happens to select `agent-determinism`. What breaks it: state one car reads OFF ANOTHER during its own update (`_vmaxNow`, `accSm`, `towing`) is written every frame but READ on the first frame before its owner has been updated, so a fresh session sees undefined and a replayed one sees the last episode's value; same for a controller's own smoothing state. Anything of that shape belongs in `apex.js` reset()'s clearing block |
| `player-dynamics-vm.test.mjs` | The SHAPE of the player's driving model, via `tools/check/player-dyn.mjs`'s pinned bench (the car is re-pinned to a straight each step, position only, so skidpad / step-steer / technique tests run without walls): the lateral force peaks and FALLS past its peak (2026-09-16: `tanh` never fell, so a flick at full lock cost nothing); the throttle charges the rear axle and never the front, more at low speed than high, and does not snap the rear either way at a corner exit (no power-on oversteer without a slip ratio — measured, not assumed); lift-off and trail braking rotate the car; full lock at 45 m/s washes wide and never spins; lateral g is monotone in lock up to the front's limit. Relative assertions only — a retune may move every magnitude. Evidence: `docs/notes/PLAYER-PHYSICS-RESEARCH-2026-09.md` |
| `episode-transients.test.mjs` | Every per-car field must be gone after `reset()`, on monza and singapore: calls `tools/check/episode-diff.mjs`'s `episodeLeaks()` to replay one seed for a few episodes in the VM and diff the post-reset snapshot of episode 0 against episode 1, field by field — the mechanical form of `EPISODE_TRANSIENTS`'s own header comment ("every key that differed between consecutive post-reset snapshots"), run automatically instead of by hand after a symptom shows up. Found `passPlan` (game.js's lazy overtake-lane cache) leaking across episodes on first run, 2026-09-16 — added to `EPISODE_TRANSIENTS` |
| `agent-anchors-vm.test.mjs` | The render-interpolation anchors every hook that pumps `update()` by hand has to take, read off `G` through the VM handle (no `__apex` hook exposes them, so this is NOT a twin and has no browser copy): `act()`, `rollout()` and `aiPlace()` must leave all SIX anchors holding the pose the car was in before the step, not the `(s, x)` pair they used to take — the player is drawn from world space, so a stale anchor draws the car, and the camera rig anchored to it, at a pose it was never in. Also that `act()`/`rollout()` clear `G.lightsLit` when they promote out of the countdown. Each assertion takes a step FIRST: on a fresh grid every `rPrev*` already equals its live field |
| `twin-drift.test.mjs` | A VM twin and its browser spec are ONE test in two places: for every test title they share, the NUMBERS each side asserts against must match. Different dialects (`expect(x).toBeGreaterThan(2)` against `gt(x, 2)`) make textual comparison hopeless, so what is compared is the threshold that survives translation; a number on one side only is dialect and ignored, a number on EACH side the other never mentions is drift. Written after 2026-09-08, when two collisions-deep specs whose twins had been corrected months earlier — comment and all — turned up as mystery reds in a 40-minute browser group. Validated against that defect: run on the pre-fix spec it flags it and nothing else |
| `track-line.test.mjs` | `TrackLine.bake` on synthetic tracks: a straight has no line (weight 0), one corner turns in outside / apexes inside / exits outside and stays on the road, a tighter corner turns in later, a chicane is straight-lined through the middle, sub-threshold wiggles are not corners, a corner across the lap seam closes continuously, and the attack zones rate a corner after a long straight on a wide road as prime and a narrow or short-fed one as poor. Since 2026-09-08 also the line FAMILIES (`lineIn` inside earlier at the turn-in, `lineOut` wider through a long corner, `at(track, s, fam)` blends linearly, both on the road) and `def.lineHints` (`apexShift` moves the apex and turn-in later, `apexInside` bounds the apex to one side, a hint far from any corner is dropped). The live check (approach/apex positions on monza) is in `ai-racecraft-vm` |
| `track-centerline-seam.test.mjs` | the dense closing chord is a real interpolation interval (final output node sits on the last→origin segment, not past it with f>1); pins Monza/Suzuka/Singapore/Mont-Tremblant X/Z and a <2° seam tangent turn; also that `Tracks.build` requires `gfx.createMesh` while `buildCenterline` stays the CPU-only entry |
| `track-retained-state.test.mjs` | production `retainGraph:false` nulls the scenery graph after upload and finalizes prop counts (no getters pinning build scope); unchunked road/terrain keep authored-precision `nrm`/`col`/`mat` arrays for late TLX chunking; diagnostic builds retain the graph and exact geometry |
| `track-line-circuits.test.mjs` | The RELAXED line on real circuits (monza, spa, silverstone through the headless track build): on the road at every node, no lurch over 0.5 m/m between nodes, and — in the AI's own corner-speed model on the line's OWN curvature — never slower than the centreline (the knot-only bake failed that everywhere); monza's long corners turn in from the outside and apex on the inside clamp, mirroring `ai-racecraft-vm`'s live check on the baked table |
| `agent-view-vm.test.mjs` | Node twin of `agent-view.spec.js` (~30 s, one boot): all 116 — `world`/`trackInfo`/`scene`/`describe`/`query`/`render`/`map`/`survey`/`objective`/`agentHelp`, typed errors, detail levels, delta mode, the raster invariants; rAF is driven by `pumpFrame` on a synthetic 50 ms clock (the chase camera's damping regime under SwiftShader) and every test starts from the sharedTest fixture's reset (`camera("chase")`: the shipped default is cockpit, from which the raster skips the player). The one renderer read (GLX chunk index) moved to `webgl-probes.spec.js` |
| `wake-lock-vm.test.mjs` | Node twin of `wake-lock.spec.js` (~3 s, one boot): all 8 — the same mocked `navigator.wakeLock`, the hide/show re-acquire, the two degrade-silently cases, the late grant and the stale-sentinel release. Twinned because the browser copy was CI's dominant flake (4 of 5 `selected` runs, 18.6 min a shard) and still raced Playwright's round trips after its waits left rAF polling; the VM has no such clock. One boot: `fresh()` quits any held lock and removes the mocks' visibilitychange listeners between tests |
...
| `driving-line.test.mjs` | the DRIVING LINE ribbon builder (`js/render/shared/driving-line.js`) as behaviour on a synthetic stadium: the strip is the GLX layout and closes the lap; the line sits inside a left-hander and centred on a straight; the speed profile reaches the cornering cap AT the entry with the braking zone before it and vTop beyond the braking distance; CORNERS zones mark turns and braking, not straights; `draw()` honours OFF and reports a backend without the pass |
...
| `appearance-opts.test.mjs` | Settings › APPEARANCE (`js/ui/appearance-opts.js`): THEME / MENU ACCENT / HUD ACCENT claim their SettingRow ids and the shell declares each row plus the live preview, swatch strips and hex text fields; defaults stamp `data-ui-theme="dark"` and brand/team data attrs at eval; SYSTEM re-applies on `matchMedia` change; `setTheme` / named presets / CUSTOM hex (colour well + hex text) persist and write `--red` / `--accent`; TEAM menu writes a concrete team hex while HUD TEAM clears inline accents; `pickInk` picks `--text` vs `--bg` by WCAG contrast; garbage store/hex values fall back; swatch clicks select; every named preset has a `tokens.css` rule; and the five appearance keys are in settings-export |
| `title-fx.test.mjs` | MENU ANIMATIONS + TITLE INTRO + MENU WASH + TITLE ART as player settings (`js/ui/title-fx.js`) under SETTINGS › APPEARANCE: unset motion is ON and an OS asking for reduced motion always wins (a stored `reduce` adds reduction, garbage reads as unset); TITLE INTRO is FULL / QUICK / OFF via `data-title-intro` (QUICK holds 1200 ms, OFF drops at once, garbage → full); MENU WASH is FULL / SOFT / OFF via `data-menu-wash`; TITLE ART is ON / SOFT / OFF via `data-title-art` (garbage → on); `<html data-motion>` / `data-title-intro` / `data-menu-wash` / `data-title-art` land at eval and in the boot script; four SettingRows + REPLAY INTRO are static shell DOM and round-trip store keys; the shell's `#overlay[data-intro]` is held for ONE pass then removed (never replayed on return, dropped at once under reduced motion or INTRO OFF, deferred to the first show when booted past the title), and `replay()` / the button restart on the next frame with no forced layout (under INTRO OFF, replay lifts the stamp for one FULL preview then restores it); a hidden tab sets `data-paused`; a trusted title `.bigbtn` click taps once through `Input.vibrate`; CSS in `responsive.css` / `menus.css` / `components.css` keys off those attributes; all four keys export under the appearance group |
| `race-settings-vm.test.mjs` | RACE SETTINGS lap ladder as behaviour on the game-vm harness: FULL is `def.gpLaps` and differs per circuit; a VS FRIEND host's distance that is OFF the next circuit's ladder (above OR below FULL — 52 (FULL) at Silverstone is not on Monaco's 3/5/10/25/78) snaps to that circuit's FULL instead of leaving no chip lit; TT ladder untouched; the solo flow still resets to the default; and the STRATEGY row follows TYRE WEAR — hidden when wear is off, AUTO when on, a chevron step pins a stop count through `PitLane.setPinnedStops` (persisted as `apex26.pitPlan.<id>`), AUTO clears it, and where the complex is built the stint bar draws one segment per stint with the pit loss beside it |
| `red-flag-fuel-vm.test.mjs` | A red-flag restart rewinds the scoring lap without restoring fuel; the repeated physical lap burns more, and backward crossing cannot refill the tank. Real game VM with tyre wear enabled |
...
| `race-flow-fixes-vm.test.mjs` | three race-flow defects from the 2026-09-22 bug hunt on the game-vm harness: a RED flag holds the field without the stuck-rescue kicking every car, finishers coast home at the floor and never collide with each other, and the only human retiring retires the AI whose failure was already drawn instead of scoring them from a mid-race snapshot |
| `red-flag-pit-vm.test.mjs` | the OTHER half of the standing restart: a stop in flight is scratch, not strategy. `redFlagRestart` cleared the racecraft scratch but not `pitState`/`pitArmed`/`pitCommitted`, and the grid boxes sit INSIDE the pit window on most circuits — so a car holding the lane when the flag flew restarted still reading `pits.inLane()`, pinned at the pit limiter and steered onto the lane offset for ~12 s on Monza at full throttle. Asserts both directions: the arm is dropped, and `pitStops`/`pitNext` survive |
| `generated-docs.test.mjs` | the four doc generators (`gen-tools-readme`, `gen-slider-doc`, `gen-hooks-table`, `gen-ladder-figures`) run with `--check` against the committed output — drift is a red test, not a stale table — plus row-count / no-`undefined` sanity |
...
| `flags.test.mjs` | every circuit's `country` in `js/circuits/` resolves to a drawn flag in `js/ui/flags.js` (never the chequered fallback); the SVG is one self-contained element with literal fills; a country name and its code render identically. |
...
| `tlx-chunked-lifecycle.test.mjs` | TLX geometry teardown ownership: freeing either a plain mesh or every spatial chunk releases pooled renderer geometry before disposing it, exactly once, and clears the dead handles |
...
| `flyby-fleet.test.mjs` | `test:node-slow`. The two FLEET-WIDE flyby sweeps split out of `flyby-shots` (2026-09-25; they build twelve circuits, ~1 min): the DEFAULT sequence at 400 samples on every circuit the 2026-09-24 audit named (no pop over 4 m, no lift of 25 m, no eye underground, grid sightline within 2 m of the road edge, no pan over 45 deg/s, never inside a solid) and seeds 0-5 of `vary()` on monza/monaco/mont_tremblant. Same rule functions as `flyby-shots`, which keeps both sweeps on monza in the edit loop (`tests/helpers/flyby-audit-rules.mjs`) |
| `flyby-pose-inverse.test.mjs` | The free camera's "copy as flyby pose": `FlybySeq.poseFromWorld` / `shotFromView` turn a world eye/target into a track-relative pose. On one Monza build it round-trips every anchor kind (corner, start, centre inside and above its height clamp, landmark) back through `posePoint` to within 0.25 m, checks a corner pose's +x is the outside of the turn, and that the solver renders a `shotFromView` shot where the camera stood. |
| `free-cam.test.mjs` | The FREE CAMERA panel (`js/camera/free-cam.js`) in a VM with mini-DOM: the log 5–200 m/s speed dial and wheel step, the ±45° roll clamp, the `__apex.view()` COPY VIEW line, the car and corner snap framings, `__apex.freeCam()` state/place/snap/exit, and — over the real `photo-cam.js` — the dial driving the fly-cam, Q/E roll and lens on `dbgCam`, arrow keys left to a focused panel control, and every exit (DONE, EXIT, resume) handing the game camera back level. |
| `flyby-panel.test.mjs` | The FLYBY SHOT EDITOR (`js/camera/flyby-panel.js`) and its bake tool (`tools/gen/bake-flyby.mjs`), as data: add inserts after the selection with a non-colliding id, duplicate deep-copies, DELETE refuses the last shot (an empty list makes `FlybySeq.solve` fall back to the shipped DEFAULT, so the live preview would silently stop showing the edit), reorder ignores a move off either end, and NORMALISE rescales durations to 1. Then the interlock: a `FlybyShots` blob round-trips, a DELTA-named one is refused with the reason, an unknown pose `at` is refused, durations that do not sum to 1 are refused, and the anchored bake rewrites only the `DEFAULT` literal — not the header comment that documents the same pose shape. Then the WIRE, added after "once I hit done in the editor it doesn't actually change the start shots" (2026-09-18): `js/game.js` must hand `FlybySeq.solve()` a third argument (it falls back to the shipped `DEFAULT` when handed none, so a two-argument call discards every edit), the panel must persist inside `edited()` — the one funnel every mutation goes through — and must store `null` rather than a copy of the default for an unedited list, and a SAVED list must be read back through `shotErrors()` rather than the bake's `validateShots()`, because `solve()` normalises durations itself and a list nobody pressed NORMALISE on still plays. The saved list is `{ v, shots }` and is read through a real `create(G)` on a stub G: a bare array (saved while bearings were world bearings and a corner's +x its right) or another version is ignored, not played mirrored, and `loadSaved()` hands out a copy, never the store's cached object. COPY VALUES' `// THIS LIST WILL NOT BAKE` header no longer hides the reasons, and the literal fallback is sandboxed. A bake keeps DEFAULT's comments per shot (a round-trip of the shipped list), the corner picker reaches 1-18, the duration slider fits a normalised one-shot list, and `y` on centre/landmark poses reads HEIGHT OFFSET. The panel's own DOM is still NOT covered here |
| `frame-math.test.mjs` | The FRAMING REPORT's pure math (`tools/lib/frame-math.mjs`, driven by `tools/shot/frame-report.mjs`) on hand-built worlds: projection handedness (+right is screen right, matching `M4.lookAt`), ray-vs-box including oriented slabs, a level camera splitting sky/ground at mid-frame, a near wall named as the left-third obstruction, occlusion transmittance (opaque, 50 % hull, terrain crest, a subject not hiding itself), subject centring/thirds/clipping, pan/parallax/zoom rates, the judge's flags firing on their defects and staying quiet on a clean frame, and the ASCII thumbnail. It does NOT check the adapter's opacity/tree/span modelling against a render |
| `frame-fleet.test.mjs` | The FRAMING REPORT's FLEET half (`tools/lib/frame-fleet.mjs`, behind `frame-report.mjs --fleet` / `--diff`), on hand-built fleets: frames keyed `<shot>@start|mid|end` so a `dur` edit still lines them up, repeats get `#n`, flags compared by NAME so a measurement drift is not flag churn, score moves under `minDelta` hidden, frames and circuits that appear / vanish / error reported, the worst-frame summary ordered lowest score first, and a single-circuit report refused as a fleet file |
| `loading-card.test.mjs` | The LOADING CARD's geometry (`js/ui/loading-screen.js`), the three numbers the FLYBY SHOT EDITOR authors: that the registry is complete (a range, a step, a label and a default inside it, per key); that `clampCard()` fills EVERY field from anything at all — null, a fragment, a string, a NaN, an Infinity — because the value comes out of localStorage and a `NaNvw` in a custom property is not an error CSS reports but a dropped declaration, leaving the card at whatever it had; that an out-of-range value is clamped rather than dropped, so a card three times the screen is still a card you can drag back; that `cardPristine()` is true for a geometry equal to the defaults, including one that clamped back onto them, because NULL is what "unchanged" is stored as (the `flybyShots` rule — a stored copy pins the player to today's numbers); that `cardVars()` emits a BARE number for the scale (the stylesheet divides the width cap by it, and a unit invalidates the whole `calc()`) and vw/vh for the offsets (a percentage would be a percentage of the card, so the same saved number would move a scaled card further); and that `css/overlays.css` reads exactly those property names WITH fallbacks — the two halves live in different files and a rename fails silently, rendering the shipped card and looking like "the player never moved it" |
| `aero-zone-tables.test.mjs` | Every AUTHORED activation-zone table in `AERO_ZONE_TURNS` actually resolves against the BUILT centreline. The table is all-or-nothing: one pair that bounds no straight discards that circuit's whole table and falls through to `ZONE_COUNT`'s "the N longest straights" — a sensible fallback, which is precisely what makes the failure dangerous, because the zones still look plausible and the table is simply never read. Monza shipped `[[9,10],[11,1]]` for as long as the table has existed; `[9,10]` bounds no straight, so NEITHER pair was ever used. It cost almost nothing — the fallback picked the same two straights to within 8 m, measured live — but nothing was checking, and geometry in this repo gets re-surveyed. Also pins that a resolved table is the one `zonesFor()` returns (so the test's own rebuilt runs cannot drift into proving something about themselves), that Monaco returns ZERO zones (a number living in `ZONE_COUNT` that no curvature scan can reach), and that `js/ui/track-maps.js` DELEGATES to `AeroZones.zonesFor` instead of carrying the second curvature scan it used to — the two disagreed on 47 of 52 circuits, Monaco drawing 6 zones the race never opens. Node-side via `verify-track.cjs`, so all 21 tables are checked on the fast gate rather than behind a VM game boot per circuit |
| `announcer.test.mjs` | The PRE-RACE ANNOUNCER (`js/audio/announcer.js`), the "Welcome to Apex 26…" read over the loading flyby. The SCRIPT is DERIVED, so it is asserted on circuits nobody wrote it for: it opens with the game's name every time (a null info included — the flyby editor asks with no race set up); it names the circuit AND the event, falling back to the country when a classic has no GP and omitting the venue line entirely rather than saying "This is ."; every number it speaks is the number it was given, and an unbuilt world omits the corner count instead of claiming zero; the character clause is corners per KILOMETRE, with the two cuts measured off the calendar (Monaco 5.7 tight, Monza 1.9 flowing, Spa 2.7 between) because counting corners alone calls Monaco and Spa the same lap; night and weather are stated and only one of them; and every script ends on "Let's go racing" so the voice never trails off. Then the VOICE LADDER, which is mostly about the machines WITHOUT Daniel: en-GB Daniel wins outright, a Daniel in the wrong language still beats a stranger in the right one, then another British male, then any en-GB (not the one whose name says Female), then any English, then null so the platform picks — and the player's own pick outranks the lot, falling back rather than silencing the channel when a save made on a Mac is opened on Windows. Then the gates: inert with no `speechSynthesis`, ZERO `getVoices()` calls at create (the boot-path defect `radio-voice.test.mjs` already pins), ON by default, master SOUND OFF is silence for play and preview alike (speech is outside the WebAudio graph, so nothing else would stop it), the switch stops `play()` but not the editor's `preview()`, the cancel-speak-RESUME triple for Bugzilla 1522074, and that `announcer` is a real `RadioVoice.TONE` channel — which is what makes the settings row work — but NOT a `SPEAKERS` value, so nothing can route a race `kind` through it and bypass the gate that keeps menu cards from being read aloud; the STORY rows (seat, championship, target, forecast, time-trial best) sit below the must-keeps and before the lights cue, a sprint states the sprint distance, the venue rotates between visits, and the post-race wrap-up names the winner, margin, your race against the grid and the fastest lap |
| `flyby-lighting.test.mjs` | The ORDERING that decides what the pre-race flyby is lit by. Reported 2026-09-18: pick DAWN, and the loading screen renders in DAY. The cause is a window, not a lighting bug — `raceIntro()` hands `startRace` to `loadingScreen.run()`, which fires it "once the card is up", and `startRace` only reaches `applyRaceSettings()` after `loadTrack()` and `makeCars()`, so the whole cinematic played over a world nothing had lit for this session's time of day. Guards that `raceIntro` applies the race settings BEFORE it starts the screen, and that `startRace` still re-applies AFTER its rebuild — the first call removes the window, it does not replace the second. Source guards, because this container has no GPU and the menu chrome covers `#game`, so a screenshot here photographs the UI rather than the sky |
| `session-entry-vm.test.mjs` | Real game VM holds the lazy scenery script across event-loop turns, then quits or changes selection/weather; canceled requests do not commit, and overlapping agent race/TT calls report distinct truthful outcomes |
| `start-race-latch.test.mjs` | `startRace()`'s re-entrancy latch (2026-09-22): six fire-and-forget callers (`js/game.js:8799`/`:8805`/`:9157`/the pm-restart handler, `daily-challenge.js:76`, `race-settings.js:393`) had no guard against a concurrent second call re-entering `startRaceBody()` mid-build. Boots the real game.js in the Node VM harness (`tools/lib/game-vm.cjs`) and calls the actual `G.startRace()` twice synchronously: asserts the second call returns the SAME in-flight promise, and that a call after settling gets a fresh one — a live check, not a source pin |
...
| `flicker-metric.test.mjs` | the RENDERED flicker gate's arithmetic (`tools/lib/flicker-metric.mjs`, driven by `tools/shot/flicker-gate.mjs` in ci.yml's non-blocking `flicker-gate` job — docs/notes/SCENERY-QA-PLAN.md §2b G2), on synthetic luma frames: a stable scene scores 0; a z-fight region re-rolling between two surfaces on each of the four sub-mm dolly frames is found (2-of-4 flips, 8-connected clusters >= 9 px) and fails its ceiling; a sub-pixel edge shift and isolated speckle are not counted; A != A2 (an unfrozen clock) fails on its own. Also pins WHY the metric is 2-of-4 and not the AND of one +/-d pair: on the same fight the AND keeps ~10 % of the region (below the percolation threshold, the cluster floor erases it) against ~69 %. And the site table: real circuits, fracs in range, jitter <= 1 cm, a ceiling and a reason per site, the madrid positive-control site present. Says nothing about whether the gate RUNS — that is the CI job's artifact |
...
| `foundation-core-vm.test.mjs` | the CORE contract all sixteen `*-foundation.spec.js` assert, replayed for every one of their circuits in the Node VM (~26 s off one boot, against ~67 min of declared browser budget): every geometry buffer built, no NaN in a collision barrier, `minOverHw` finite, and nothing a circuit marked REQUIRED suppressed, invalid or unsafe. Catches a COMMON-MODE break — one shared-foundation change that breaks every circuit — which otherwise waits for `test:circuits`, a browser group the change-aware gate rarely selects. Measured: a mutation making `validateGeometry` reject every buffer fails 16 of 17. Does NOT replace the specs — each one's circuit-specific claims (Spa's 102 m relief, Monaco's sub-1 `minOverHw`, Interlagos' prop budget) stay where they were measured |
...
| `title-art.test.mjs` | guards the title screen's car art, which `tools/gen/title-art.mjs` writes into `index.html`'s `@gen-shell:title-art` block. Nothing in `tests/` looked at it, and the one pixel golden that does (`menu-baseline.spec.js`) is a browser group that a docs-and-tools change never selects. Each case is a mistake actually made: an open subpath without `fill="none"` (six drafts rendered as wireframes because a group carried `fill: none`), plain `opacity` on a group (it makes the FILLS translucent and the occlusion that carries the depth disappears), `fill="url(#tc-trail)"` as a presentation attribute (specificity 0, so it loses to `#title-car g { fill: var(--carbon) }` and the tyre marks silently paint as bodywork — hence the `[data-trail]` rule the test also pins), a gradient referenced but not defined, the placed art walking outside its own 1400x900 viewBox, and `#title-car` losing `aspect-ratio: 1400 / 900` (without it the art blew up to 1857 px wide on an iPad in portrait, at full opacity, because the >1000px fade never fired) |
...
| `circuit-vocab.test.mjs` | the same question one level down — a def may name a `furniture.tree` species or a `standSet` grandstand livery the engine has no entry for, and every lookup has a fallback, so the circuit renders as something else in silence (Dijon's `"stone"` livery was the steel default). The valid sets are read OUT of `js/track/tracks.js` and `js/track/scenery/data.js` rather than restated. `KNOWN_UNHANDLED` parks `"pine"` with the measurement that blocked its fix, and a second test fails if the engine ever learns a word parked there |
...
| `surface-id-parity.test.mjs` | the shader-side half of the above: every Car3D surface id (`SURFACES`/`FINISH_SURFACE`) is branched on in ALL THREE lit shaders — GLSL, WGSL and TSL keep three hand-maintained copies of one table, so a forgotten backend silently falls the id through to its default branch and the car looks wrong there and nowhere else. Also pins that no branched id sits above `classifiedCar`'s upper bound (a dead branch) and that no id is invented or orphaned |
| `test-coverage-audit.test.mjs` | the coverage auditor itself |
| `prepush-gate-coverage.test.mjs` | every unit file is RUN by the pre-push gate (`deploy.mjs --gate-only` = tooling-fast + ci.yml's node suites), or named in its `SWEEPS_ONLY` list with the CI job that does run it — `test-coverage-audit` asks whether a file is in some group, this asks whether anything runs it before a push, which is the gap that took deploys red on 2026-09-02 and 2026-09-18 |
...
| `reject-lint.test.mjs` | its sibling over `js/`: a `try`/`catch` cannot swallow a promise REJECTION, and `index.html` paints a full-screen overlay on any unhandled one. A ZERO ratchet — the tree is clean and the fix is one line per site — over the APIs whose promise-return is unambiguous, with `close`/`play`/`pause`/`resume` disambiguated by RECEIVER so the rule does not cry wolf at thirty synchronous WebSocket and IndexedDB teardowns and get switched off. Also catches the guarded spellings (`x && x.foo()`, the ternary), because a guard that holds only until someone adds a null check is not holding the class at zero |
| `html-sink-lint.test.mjs` | every `innerHTML` / `outerHTML` / `insertAdjacentHTML` in `js/` writes a source constant (literals, substitution-free templates, `+` of those, a ternary between two of them) or is in `tests/data/html-sink-allowlist.json` with a reason its dynamic parts cannot carry player text (`tools/check/html-sink-lint.mjs`). Keyed by file + receiver + sink with a per-key count, so a second site behind an audited receiver is not waved through, and a stale or miscounted entry fails too. Born of the 2026-09-24 stored XSS (a custom driver name re-wrapped through `innerHTML` by `aria-state.js` paintOnOff); also in `guards` |
| `delta-sign-colour.test.mjs` | two widgets, one sign convention, opposite colours. `js/data/telemetry.js` computes the TELEMETRY compare gauge's delta and the lane board's with the same expression and comments both `>0: … is behind`, so `dh-pos` is the SLOWER side on each — but `css/data.css` painted `.dh-gdelta.dh-pos` `--faster` and `.dh-laneboard-dl.dh-pos` `--slower`, fifty lines apart, so the compare gauge showed the trailing driver green. NEITHER HALF IS WRONG ALONE (the JS sign is right, the CSS is valid, each rule is plausible) — only the pairing is, which is why nothing caught it, and why a screenshot would not either: a green number and a red number both look like a working widget unless you know which car is ahead. Asserts the invariant, not the values |
| `class-usage.test.mjs` | the direction the component inventory never checked. `component-inventory.test.mjs` guards DEFINED-but-unapplied — a rule in `css/` that nothing wears, which costs bytes and nothing else. This is the mirror and the expensive one: an element wearing a class NO rule matches reads as styled, the reviewer takes intent off the name, and the element quietly inherits whatever its siblings give it. Three of the four UI findings of 2026-09-22 were this shape and the sweep that found them was a throwaway script, so the same three could have come back the next week. `tools/check/class-usage.mjs` reads all four ways this tree applies a class (`class=`, `className`, `classList`, and `Dom.el(tag, cls)` — the busiest by far; a scan without it sees a third of the app) and skips anything interpolated or concatenated rather than guessing at a half-name. Asserts an EXPLAINED set, not an empty one: every name is either queried by JS or by a spec locator, or carries its reason in `KNOWN`. It found `pm-pad-tools` — three CONTROLLER buttons welded at 0.0 px where every other row in the panel had 4 |
| `boot-input-shape.test.mjs` | the `DIFF[difficulty]` family, one door further out. Three readers took a stored value on trust: `idxOr` (game.js) — `x >= 0 && x < len` is NOT an index check, and this file had it in BOTH spellings, each leaking the opposite way (`"abc"` fails `< 0 \|\| >= len`, `""` passes `>= 0 && < len`), so a string reached `Teams.LIST[…]` as undefined and the first `team.id` threw; `loadCustomTeam` (custom-team.js) — whatever sat under `apex26.customTeam` was pushed into `Teams.LIST` whole, where `seasonRoster()` does `team.drivers.forEach(...)` at BOOT, so `{}` was a TypeError before the menu painted; `DebrisWorld.reset()` — `_active`, the ONE boolean game.js reads for debris, marbles and the caution they feed, was lowered by a trapped rapier step and raised by nothing any restart calls, so one transient WASM fault turned the surface off for the rest of the SESSION. All three assertions EXECUTE the shipped source, extracted by balanced braces: a regex over `Number.isInteger` would pass the day someone rewrote the clamp into a new wrong form. Writing it that way is how the second clamp site was found |
| `tt-lap-validity-vm.test.mjs` | a time-trial record set off the circuit. The crossing gates `c.best`, the TT board, `G.ttRecord` and the stored ghost on one latch, `incidentInvalidLap` — and track limits were not on it. They had a ladder of their own instead: three warnings, then +5 s, which prices a cut against the race CLASSIFICATION. A time trial has no classification, so nothing priced it, and the cheapest route onto the board was to leave the circuit (measured: a lap run off-track replaced a 42.15 s record with 5 s, ghost and all). Drives the REAL detector — the car is put 40 m off the road and stepped until game.js's own 1.2 s threshold counts a cut — so the test cannot pass by agreeing with a constant, and a second case pins that 0.5 s off is still a lap. NOTE the fixture: repositioning by hand (`s`/`x`) leaves the heading behind and the car drove BACKWARDS over the line, decrementing the lap counter; `apex.jump()` is the only reposition this harness should make |
| `gallery-capture-draws.test.mjs` | a gallery screenshot must be taken with the render loop RUNNING. Three garage specs stop it by default (`__apex.headless` in their `openSetup`) because the live turntable rebuilds the car mesh and repaints the livery atlas on every option click — most of what those files cost, none of what their DOM assertions read. The hazard is SILENT: an undrawn canvas keeps its LAST frame, so a `page.screenshot` with the loop stopped still writes a PNG, of a stale car. The rule is that a screenshotting test opts back in with `{ draw: true }`, and this asserts it rather than leaving it in a comment. Scope is derived from the opener's shape, not a list, so a fourth file adopting the idiom is covered automatically; it also asserts it still finds files to guard, so it cannot rot into dead code. |
...
| `spec-timings.test.mjs` | guards `tools/ci/spec-timings.mjs` and the per-spec half of `select-budget.mjs` — the rolling duration history in `tests/data/spec-timings.json`. Three properties, each with a silent failure mode: the merge is IDEMPOTENT (a run's junit and its live-reporter log are the same run seen twice, and double-counting would drift the median the budget reads), the history is BOUNDED at 10 samples per key (the file is committed, so an unbounded append grows the repo per CI job), and PROVENANCE survives — a spec on the inherited 79.7 s constant must never report like one that was measured. Env buckets are never mixed, so an llvmpipe run cannot be read as a SwiftShader regression, and a `local` history may not set a CI budget at all. One case leaves the fixtures and walks the committed file itself: every sample's bucket is real, every spec-level row agrees with the per-test rows of its own run, and the file is byte-identical to what the writer would produce |
...
| `who-is-on-it.test.mjs` | guards the CLAIM half of `tools/ci/who-is-on-it.mjs`, the shared-branch check that answers "is someone already on this red?" before a second fix is written (`docs/notes/SHARED-BRANCH-COORDINATION.md`). A claim is one tiny `claude/claims/<slug>` branch whose tip is an empty-tree commit carrying the text, and a release is a `released` tombstone tip (the containers' git proxy refuses ref deletion, measured), so the parse from a `for-each-ref` line to `{slug, who, text, ageMin, stale, released}` is pure and pinned on fixtures (newest first, stale past 120 min but still listed, a tombstone flagged and never read as live, a tab in the text survives), the slug rule is pinned, and the commit shape is pinned against the real `git` (the empty tree, the message, the branch and session tag). Importing the module runs nothing: the CLI entry is argv-guarded |
| `deploy-pr-rest.test.mjs` | guards `openPrRest` in `tools/ci/deploy.mjs`, the `--pr` path a container without the `gh` CLI takes: a fake `curl` on PATH answers the three calls (find an open PR for the head, create one from the HEAD commit, enable auto-merge over GraphQL) and records each argv and stdin, so the test pins the call sequence, the reuse of an already-open PR, a refused auto-merge landing in the note rather than a throw, and that the token rides in the curl config on stdin and never on the argv (`ps` shows argv to every process on the box) |
...
| `adapted-specs.test.mjs` | the `test:vm-page` runner: every `ADAPTED` spec in `tools/ci/twinned-specs.mjs` run as itself under the vm-page adapter, one child process per spec so a hang or crash is one red row rather than a dead runner. Refuses a child with no readable `# pass`/`# fail` summary and one that declares zero tests — the "a pass must have run something" rule `run-playwright.mjs` learned from the 0/0 `collisions` verdict. An empty `ADAPTED` map fails too: an empty gate is not a gate |
...
| `nightly-group.test.mjs` | the CONTRACT of `tools/ci/nightly-group.mjs`, which picks the browser group tonight's scheduled `ci.yml` run covers. The nightly used to be pinned to `tiny` — 4 of 119 browser specs — so everything else had scheduled coverage of NONE and a red spec could hide indefinitely; one did (`docs/notes/DEFECT-LEDGER.md`, 2026-09-16). The rotation closes that at the minutes the nightly already spent, and this suite keeps it closed: a new browser group must join the rotation or carry a written reason in `EXCLUDED` (an unexplained gap is exactly how the last hole opened), one full cycle must touch every rotated group, every rotated group must be a runnable `npm` script, the pick must be deterministic and survive a negative or huge day index, and the CLI must print the BARE name because `ci.yml` prefixes `test:` itself |
| `twin-fidelity.test.mjs` | the always-on half of `tools/check/twin-fidelity.mjs`, which proves a VM twin catches what the browser copy catches by BREAKING the code on purpose. Equal test counts are vacuous for an ADAPTED spec (the twin IS the spec), so each mutant in `tests/data/mutants.json` breaks one mechanism and names which tests must go red. This suite costs milliseconds and catches how such a matrix really dies — a refactor moves the code, a `find` string stops matching, and every mutant becomes a silent no-op reporting PASS forever. Also pins that each mutant is ASYMMETRIC (one that reddens everything proves only that the suite runs), that a test missing from the output never reads as a pass, and that the recorded `openQuestions` keep their evidence instead of being deleted |
| `legends.test.mjs` | the contract for `js/data/legends.js`, the eight historic drivers. The risk is not a crash — it is a sourced number quietly drifting, or a rating being tuned because someone preferred a different ranking, so the record is checked for internal consistency (wins <= podiums <= starts) and the ratings are checked to FOLLOW it: each axis is ranked against its OWN formula, which caught a real pair where pure pole-rate ordering disagreed with the documented blend (Mansell out-qualified Prost per start; Prost won far more). Also pins that a legend's three-letter code never collides with the 2026 grid, that livery ids are namespaced `legend_*`, that c1 and c2 stay tellable apart at racing speed, and that the SOURCES block and the 'tribute palette, not a replica' wording survive — these are claims about real people |
...
| `uilayers-modal-order.test.mjs` | `UiLayers.top()` ranks open dialogs by TOP-LAYER order (`querySelectorAll(":modal")`), not DOM order, with a fake-DOM harness; plus the index.html guard that every markup opener precedes the dialog it opens (which is why the change is a no-op on today's tree), and the F9/F11 shape pins; and `TopModal.onEscape` standing aside when the topmost `:modal` is a dialog nested inside the layer (the Data Hub telemetry popup) |
...
| `skidmarks-cadence.test.mjs` | `js/fx/skidmarks.js` in a VM: tyre marks are laid per SECOND of laying (5/60 s), so two seconds of a slide leaves the same trail at 30, 60 and 144 Hz — it was a 5-FRAME countdown that packed marks 2.4x denser on a 144 Hz display; the first stamp of a slide lands at once and a caller without a dt is charged one nominal frame |
...
| `tinyfish-mcp.test.mjs` | TinyFish CLI wrapper (not MCP-attached) + Chrome / Playwright helpers — repo `.mcp.json` is the three-server catalog (`apex-tools`, `playwright-official`, `chrome-devtools`); tinyfish/probe stay CLI-only. Help surfaces (`setup`/`deploy-js`), fixture unwrap/deploy-summary/live-build (search rows render title+url+snippet), every `mcp_post` body must parse as JSON once shell splices are stubbed (the guard that catches the stray-quote class), tracked source has no reusable key and `ensure` names its setup prerequisite, a transient upstream timeout is exit 3 (retried) while a genuine parse failure stays exit 2, `mcp-cli.mjs` uses `chrome-devtools-mcp.sh` with an exact fallback version (no live API), `playwright-mcp.sh` pins `@playwright/mcp@0.0.79` |
...
| `probe-mcp.test.mjs` | Unified probe CLI bridge (not MCP-attached) — prefixes `chrome_*`/`tinyfish_*`, help/route, mock stdio handshake advertises full catalogs, mock chrome daemon (healthz//tools//call + CLI auto-routing to a live daemon) (no Chromium / no TinyFish network). Attached Chrome MCP is `chrome-devtools`. Also `mcp-cli.mjs probe --dry-run`: the pick is written BEFORE the reload in one batch, `--backend three` carries the WebGL2 pin (and only three does), unknown flags exit non-zero rather than probing the default, and the wrapper keeps `--enable-unsafe-webgpu` |
...
| `apex-tools-mcp.test.mjs` | `apex-tools` MCP — `serverInfo.name === apex-tools-mcp`, tools are all `apex_*` (zero chrome/tinyfish; no test-bg wrap), `apex_graph_parity` requires `base`, `apex_frame_report` validates track (Tracks.LIST) / u / frames / shots (existing file under artifacts/ or scratch/, symlinks resolved) and never passes `--out`/`--fleet`, catalog `tools/mcp/apex-tools-mcp.json` locksteps `.mcp.json` stdio + `serve-http` on `127.0.0.1:3713`, week-1–4 pins, lock/occupancy including host `@playwright/mcp` vs `--mcp-config` JSON, `path_escaped` / `port_not_supported`, refuses deploy/github.io, `isError` preserved, stdout JSON-RPC only (mock/`dryRun`, no Chromium) |
| `mcp-smoke.test.mjs` | five-wrapper shell probe — `--dry-run` lists apex-tools / probe / chrome-devtools / playwright / tinyfish CLIs (only three are MCP-attached), never `verify` / `deploy-check` / `test-bg` / `playwright-mcp.sh run`, `apex-tools-mcp.sh smoke` delegates, no Chromium |
| `agent-surface.test.mjs` | wrap map lockstep — `docs/AGENT-SURFACE.md` names every `apex_*` in `tools/mcp/apex-tools-mcp.json`, each CLI/skill exists, never-wrap lists `test-bg` / `--apply` / github.io, indexes point at the map, catalog descriptions start Tree / Browser (lock first), `.mcp.json` has the three repo servers (playwright-official pinned npx), Bootstrap covers Cursor/Claude/Codex, `.agents/skills` symlink-mirrors `.claude/skills`, `.codex/config.toml` MCP tables lockstep `.mcp.json` |
