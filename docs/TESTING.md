...
114 root Playwright spec files (`tests/specs/*.spec.js`) + 193 `node --test` unit suites
...
| `hud-metrics-layout.test.mjs` | HUD metrics layout modes (`auto`/`full`/`timing`/`driver`/`compact`) plus per-widget visibility (`hudMapVis`/`hudGapsVis`: `auto`/`on`/`off`): `resolveMetricsLayout()` picks cluster focus from profile and fitHud caps (not camera — map/gaps are separate toggles); `syncHudVisClasses()` hides minimap/gaps on onboard cameras only when set to `auto`; body classes `hud-met-*` and `hud-hide-map`/`hud-hide-gaps` in `css/hud.css`; pause menu `LAYOUT:` / `MAP:` / `GAPS:` cycle the stored choices |
