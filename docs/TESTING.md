...
| `appearance-opts.test.mjs` | Settings › APPEARANCE (`js/ui/appearance-opts.js`): THEME / MENU ACCENT / HUD ACCENT claim their SettingRow ids and the shell declares each row; defaults stamp `data-ui-theme="dark"` and brand/team data attrs at eval; `setTheme` persists and restamps the html attribute; a named menu preset stores and leaves `--red` to CSS while a custom colour writes an inline `--red`; HUD brand points `--accent` at `var(--red)`; and wiring waits for DOMContentLoaded the same way the other settings modules do |
