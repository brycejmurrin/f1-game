...
| `debug-cameras-docs.test.mjs` | camera-doc claims vs the live tree: `CAM_MODES` cycle order, `camera()` id-or-index matcher (not picker labels), `camState.roll`, Monza T1 `0.0962`, `camTune` knobs including `cornerLead`, `view()` no-args aerial vs `sky()` `skyViewOverride`, and `shot.mjs` / `apex-capture cameras` tokens |
