...
| `ai-strategy.test.mjs` | the field's race strategy in a VM — `stintPlan` scales stop count with the distance (a 3-lap blast never stops, a full distance always does) and its stints cover the race exactly, a 20-car grid produces at least three distinct plans (biasing only the stop COUNT put all 20 on one, which is a procession), a full tank costs tyre life so strategies MIX rather than running one compound all race, the plan is a pure function of its inputs, and `pitNow`'s reactive rules fire in the right order — weather over the free stop over a spent set over the plan |
| `engineer.test.mjs` | the race engineer's ladder in a VM — it ADVISES and never arms a stop or touches a car's pit state (the AI has a plan, the player has an engineer, which is §11 decision 1 made good), it speaks only to the local player, the wrong tread and a free stop under a caution outrank every complaint about the tyre, graining and blistering say DIFFERENT things because one heals if you ease off and the other does not, the axle call names an end of the car and what to do about it but sits BELOW the defects ("brake earlier" is the wrong answer to graining), the wear ladder counts DOWN, it does not nag — one line at a time, no line twice, and a threshold crossed while the banner was busy is not silently spent — and it is ONE VOICE with the pit cue: while `PitLane.lastCue` is giving a direction (`DIRECTIONAL`: enter, keep, stop, merge) the engineer waits and the line it owes is said once the cue lets go |
