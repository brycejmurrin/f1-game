...
| `sheetshape-keyboard.test.mjs` | The software-keyboard band (`--kb`) must not outlive the keyboard: no focused editable element means no band, a `focusout` clears it without waiting for `visualViewport`, and a window resize/rotation re-derives it with a settle pass. Reported from a phone as the garage sheet stuck at 222px after typing a livery name and rotating. VM on `mini-dom` with a fake `visualViewport` |
