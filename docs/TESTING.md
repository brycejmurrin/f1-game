...
| `game-vm-a` | `elevation-tracks-vm.test.mjs` ALONE, on its own CI runner (`node-suites` matrix slice `vm-a`, 2026-09-16): it builds 40 circuits and measures 399 s on this box — the floor of any split of `game-vm`. As one slice the whole group measured 9.2-9.4 min and set the job's wall; with this file alone the slice is ~6.5 min on a runner and everything else fits beside it. Partition — `game-vm` is still the full list locally |
