...
112 root Playwright spec files (`tests/specs/*.spec.js`) + 101 `node --test` unit suites
...
| `jsconfig-globals.test.mjs` | the editor language-service project (Bedrock Phase 1 second half): `jsconfig.json` stays `checkJs:false` + `moduleDetection:force` over `js/` and `types/`; `types/globals.d.ts` plus the `GameModuleFactory` roster in `types/game-ctx.d.ts` declare exactly the globals `tools/scan-globals.mjs` finds; `tsc -p jsconfig.json` stays clean |
