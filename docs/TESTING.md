...
116 root Playwright spec files (`tests/specs/*.spec.js`) + 208 `node --test` unit suites
...
| `menu-traversal.spec.js` | EVERY control of a menu layer is reachable with the arrow keys alone and with a controller's D-pad alone — a breadth-first walk of focus states, compared against `MenuNav.items(layer)`, plus Escape and the pad's B leaving each layer. Four defects shipped in exactly this gap and are pinned here: a closed `<details>` kept its contents' layout boxes so `step()` chose one and `focus()` silently refused (from BACK on DISPLAY no arrow moved at all); a `<select>` owned all four arrows so every ‹ value › row was an island needing Tab, which a pad has not; the across-penalty let a distant full-width slider beat the next row's chevrons; and the ‹ › chevrons formed a focusable column nothing entered. Both walks run in-page on synthetic keydowns — what the pad sends anyway, and ~100x the round-trip speed |
| `menu-traversal.spec.js` | EVERY CONTROL OF A LAYER IS REACHABLE WITH THE ARROW KEYS AND WITH A CONTROLLER'S D-PAD ALONE: a breadth-first walk over focus states (real keydowns, then a faked standard-mapping pad polled through `Input.poll`) compared against `MenuNav.items(layer)` on the title, the circuit picker (the filter tabs are restored after each press), race settings, the four settings pages with every fold open, and in a race the pause menu and the DISPLAY page under it; Escape and the pad's B leave each layer; a fold header opens with Enter and with A; a `<select>` keeps Left/Right, leaves on Down without changing its value, and steps on D-pad Right. Pins the 2026-09-08 findings: closed-`<details>` contents offered as targets that refuse focus, select rows as islands, the across-penalty skipping a chevron row, chevrons beside a select that nothing could enter |
