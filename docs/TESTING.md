...
115 root Playwright spec files (`tests/specs/*.spec.js`) + 127 `node --test` unit suites
...
| `slider-effect.test.mjs` | no-browser guard for `tools/slider-effect.mjs`: `TUNE_DEFS` parse, gate/risk filters, and the documented failure-mode classes (`docs/LIGHTING-TUNER-SLIDERS.md`). No Chromium — pixel MAD is the wrong instrument here; this is consumer + gates only |
| `lighting-tuner-sweep.test.mjs` | pure helpers for `tools/lighting-tuner-sweep.mjs` (no browser): `pushExtreme` direction, `knobGateReason` night/lamp classification on day conditions, `KNOB_OPEN` gate, and `verdict` pass/warn/fail thresholds |
| `menu-capture.test.mjs` | `layout-audit` CLI contracts (no browser): gallery / list / survey flag parsing, `cellFileBase`/`cellPaths` agreement, `listScreenIds`, `pickScreens` filter, and `cellRecorded` file-existence checks |
