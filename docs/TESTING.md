...
| `apex-tools-mcp.test.mjs` | `apex-tools` MCP — `serverInfo.name === apex-tools-mcp`, tools are all `apex_*` (zero chrome/tinyfish; no test-bg / graph-parity wrap), week-1 pins (`--fast`/`--json`, never `--wait`/`--bg`/`--apply`), week-2 frozen `apex_ui_survey` + lock/occupancy, week-3/4 tree pins (`select-specs`/`select-recall`/`cache-bump-only`/`rotate-markings --check` never `--write`, startline `--json`, float/clip `--json`) + tree tools ignore the lock, `path_escaped` / `port_not_supported`, `target=deploy` → `tree_only`/`local_only`, `github.io` → `github_io_blocked`, loopback url → `url_not_supported`, `isError` preserved, stdout JSON-RPC only (mock/`dryRun`, no Chromium) |
