...
| `perf-try.test.mjs` | Pause SETTINGS PERF EXPERIMENTS: all four flags default ON, storage keeps only deviations, `?perftry=` overrides the session, `withWgslConsts` overlays authored WGSL `OPT_*` lines, the injected PERF panel labels ENV CULL, and GLX/WGX/TLX each honour flareGate, lampFogGate, and envCull |
