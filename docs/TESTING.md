...
| `pit-ribbon.test.mjs` | the pit lane as an ACTUAL SECOND ROAD, on real track builds. Three earlier attempts put a lane beside the racing surface and each broke geometry — one forced Monaco's boundary through the buildings, one fitted to the worst node and concluded there was no room anywhere, one widened `hw` and pushed the ROAD through the scenery on a dozen circuits. This holds the shape that works: `hw` and both boundaries are untouched (the lane is a separate ribbon, not a wider road), the lane is a CONSTANT offset and width rather than a strip that snakes, its far edge stays inside the driving boundary at every node, a circuit whose walls sit at the road edge is refused a lane outright rather than given a pinched one, `pitLaneAt`/`inPitLane` agree with the ribbon that was built (a lane you can see and cannot drive on is the same bug as the reverse), and the ribbon geometry is emitted only where the fit says |
