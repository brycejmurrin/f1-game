...
| `game-vm-a` | the first half of `game-vm`, on its own CI runner (`node-suites` matrix slice `vm-a`, 2026-09-16): as ONE slice the group measured 9.2-9.4 min and set the job's wall. Carries elevation-tracks and physics-characterization; a partition, `game-vm` is still the full list locally |
| `game-vm-b` | the second half (`vm-b`): agent-view and mechanics-integration plus every other file alternately. Partition of `game-vm` |
