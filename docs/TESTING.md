...
| `apex-tools-mcp.test.mjs` | Week-1 `apex-tools` MCP — `serverInfo.name === apex-tools-mcp`, tools are all `apex_*` (zero chrome/tinyfish), dryRun/mock pins (`--fast`/`--json`, never `--wait`/`--bg`/`--apply`), `target=deploy` → `tree_only`, `github.io` url → `github_io_blocked`, `isError` preserved on refuses, stdout JSON-RPC only (`APEX_MCP_MOCK=1`, no Chromium) |
