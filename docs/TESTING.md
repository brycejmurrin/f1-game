...
| `pit-lane-vm.test.mjs` | the pit COMMITMENT against the built complex, driven in the VM on Bahrain (~10 s): a car on the racing surface never commits — not from the grid, not holding the pit-side third of the road inside the window (the old painted line sat 3.2 m inside the edge and armed the limiter on the racing line) — and a car on the lane's own entry road, held, does |
