...
4. **Guard-hardening trio** (one commit, no bump): (a) `tests/specs/ui-button-touch.spec.js:459-463` — `if (await throttleBtn.count() > 0) { await expect(throttleBtn).toBeVisible(); }` passes on a screenshot if `#btn-throttle` is removed; delete the `if`, assert unconditionally. (b) `tests/unit/coplanar-faces.test.mjs:58` — `assert.ok(results.length >= 24, ...)` is the pre-roster floor its sibling was already fixed for; copy `tests/unit/prop-clipping.test.mjs:64-67` verbatim (`results.length === MANIFEST.CIRCUITS.length`). Latent (the :68-77 baseline loop catches drops today) but two lines. (c) `tools/fixture-consumer-audit.mjs:26` — `export const FLOOR = 31;` vs 54 real importers (verified by grep); raise to 54 and add a ~5-line slack assertion mirroring `tests/unit/module-size.test.mjs:60-71`.
...
7. `tests/unit/module-size.test.mjs:43`: the ceiling is now **7975** (raised deliberately by the fix train, reason in the comment above it) against a 7972-line file — lower it to ~7795 after the move (convention, not forced; the guard's own slack check trips at 400).
...
3. Imports: 66 specs `./fixtures.js` → `../helpers/fixtures.js` (+ track-helpers/qr-camera/f1-api-mock/output-paths); ~40 unit suites' ROOT `".."` → `"../.."`; `tests/helpers/track-helpers.js:45` manifest require → `../../tools/`.
4. ⚠ `tests/helpers/output-paths.js:7` REPO_ROOT `".."` → `"../.."` AND `tests/specs/output-paths.spec.js:12` expectation together (they drift symmetrically green otherwise, writing galleries into tests/); same in `tests/helpers/qr-camera.js:30`.
5. ⚠ `tools/fit-audit.mjs:27` + `tools/menu-fit.mjs:32` — try/catch dynamic import of `../tests/helpers/f1-api-mock.js` swallows the break; PATH_RE can't see it. Edit explicitly.
6. ⚠ `tests/specs/f1-track-accuracy.spec.js:18` `./data/...` → `../data/...` (browser-only failure; a `../helpers` grep won't find it).
7. ⚠ `tests/manual/` imports of output-paths (`inspect.spec.js:20`, three gallery specs) + `manual/README.md:8-9` prose.
8. Vacuous-pass traps — widen regexes to admit `/`: `tests/unit/test-groups.test.mjs:154` and `tests/unit/docs-integrity.test.mjs:343`; plus their flat readdirs (test-groups :83, :92-93, :125; docs-integrity :236-237, :347) → walk specs/+unit/.
9. `tools/test-coverage-audit.mjs:39` AND its duplicate scan in `tests/unit/test-coverage-audit.test.mjs:60-63`; `tools/fixture-consumer-audit.mjs:42` IMPORTS_FIXTURES + `:58` readSpecs dir (else FLOOR reads 0 — cites re-measured 2026-08-07; the file gained `FLOOR_SLACK`) AND the fixtures in `tests/unit/fixture-consumer-audit.test.mjs` encoding the old import shape — note that suite now ALSO runs the ratchet against the real tests/ tree, so a move breaks it loudly rather than silently.
10. ⚠ `.github/workflows/ci.yml` sweeps filter `tests/(prop-clipping|...)` → `tests/unit/(...)` (fail-open otherwise).
11. Hardcoded cross-file reads: `tests/unit/span-kinds.test.mjs:46`, `tests/unit/docs-integrity.test.mjs:217`, prop-clipping/coplanar ROOT-tools paths.
...
2. lighting/ commit MUST include `tests/specs/lighting-ab.spec.js:15` (`import ... from "../tools/ab-lighting.mjs"`) — outside tooling-fast and CI, breaks silently otherwise.
...
4. Deepen `tests/unit/docs-integrity.test.mjs:395-402` to walk subdirs in the same commit, or every moved tool permanently exits the index guard.
...
- Never-fail asserts in `tests/specs/camera-driving-hooks.spec.js:188-207` and `tests/specs/headless-api.spec.js:290-303` — each needs a per-test design decision, not a mechanical fix.
- `tests/unit/test-groups.test.mjs:6-7` "98 specs sit flat" header — R2 rewrites that header anyway; don't touch twice.
...
|---|---|---|---|---|
| HIGH | UNAPPLIED | `tests/specs/agent-view.spec.js:1535` | The agentHelp() spec still asserts the deprecated-aliases note that commit a6fb6ce2 deleted from agentview.js, so the agent test group now fails on... | session-diff |
...
| LOW | UNAPPLIED | `tests/unit/scenery-api-contract.test.mjs:3` | A cluster of the session's own triaged JS-BATCH fixes never landed: the 938aed50 comment batch covered roughly half the list, leaving false stateme... | session-diff |
...
| LOW | UNAPPLIED | `tests/unit/scenery-api-contract.test.mjs:3` | The circuits-survey JS-BATCH items (triage lines 80-81) were all skipped: the contract test header cites a nonexistent directory and wrong count, a... | triage-completeness |
...
| HIGH | BUG | `tests/specs/ui-button-touch.spec.js:460` | The test "throttle button visible in button mode" cannot fail for the reason its name states: the only expect is inside an existence guard, so remo... | guard-health |
| HIGH | BUG | `tests/unit/coplanar-faces.test.mjs:58` | The coplanar sweep's roster floor is still the pre-fix `>= 24`, so the sweep can silently drop up to 16 of the 40 circuits — the exact defect its s... | guard-health |
...
| HIGH | MESS | `tests/unit/test-groups.test.mjs:83` | A tests/ directory split (specs/unit/helpers/data) requires coordinated edits to nine guard sites; worse, three guards go silently VACUOUS rather t... | guard-health |
| LOW | MESS | `tests/unit/docs-integrity.test.mjs:399` | A tools/ subdirectory reorganization degrades the tools-index guards silently and breaks five guard suites' hardcoded tool paths loudly; the exact ... | guard-health |
| LOW | DOC-DRIFT | `tests/unit/scenery-api-contract.test.mjs:3` | Guard files' own headers drifted and nothing can catch them: scenery-api-contract still says the contract serves "24 consumer files in js/tracks/" ... | guard-health |
