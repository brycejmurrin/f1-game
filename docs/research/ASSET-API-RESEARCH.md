...
**Prerequisite:** `js/track/mesh.js` never allocates a `mat` array, so road and
terrain are permanently `MAT.FLAT` — verified, there is no `_mat` in that file.
The surface on screen 100% of the time is the one the material system cannot
touch. RENDERING-IMPROVEMENTS.md already ranks fixing this as the cheapest win
in the codebase; **the texture array is worth roughly nothing on the road until
that lands.** Do it first.
...
| phase | work | risk | test group |
|---|---|---|---|
| 0 | `MAT` ids for road/terrain in `mesh.js` — **prerequisite** | low | `test:scenery`, `test:visual` |
| 1 | `tools/assets.mjs` + manifest + licence `verify` in `test:tooling` | none (no runtime change) | `test:tooling` |
| 2 | `Assets` global + `createTextureArray` + WebP array, `uMatTexMix` at **0** | low — inert until the knob moves | `test:api`, `test:webgl` |
| 3 | Tune `uMatTexMix` per profile in the LIGHTING TUNER, bake into `light-presets.js` | low, revertible by slider | `test:ab`, `test:visual` |
| 4 | KTX2 container + vendored transcoder, if phase-2 VRAM measurement demands it | medium (new wasm) | `test:webgl`, mobile |
| 5 | `bake-model` + first grandstand/pit-building variants | low — data only | `test:scenery`, `verify-track.cjs` |
| 6 | HDRI → env cube + hemisphere ambient | low | `test:webgl`, `test:ab` |

Phases 1, 5 and 6 are independent of 2-4 and can go in any order.
