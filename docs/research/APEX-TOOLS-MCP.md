...
| `node tools/gen-shell.mjs --check` (no cache bump: tags read `?v=dev` and the deploy stamps the hashes; after a `tools/manifest.cjs` change run `node tools/gen-shell.mjs`) / `--at` / `--merge` | Writes `index.html` / `version.json`; last edit before commit |
