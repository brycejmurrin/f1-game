...
| `.sheet` | a card with a fixed head, ONE scrolling body and a pinned foot; also the `sheet` **query container** every layout decision inside it keys on. Dense sheets may declare `--fit-at` (minimum functional local height); `SheetShape` then caps only that panel's effective zoom when the safe viewport cannot supply it. Phone-landscape floors: select / season / garage / career / settings / race-settings / howtoplay / results / standings / customize / vs-friend / quali use `--compact-at: 480px`; audio / Spotify use `520px`; lighting / camera tuners stay `620px` (12 chips — a phone at 115% is already compact and must stay so); the generic floor is `380px`. Career / select / season keep `--pair-compact: wide`; garage stays `off` |
