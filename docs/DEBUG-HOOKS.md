...
| `multiDraw(on)` | Occlusion culling (GLX only, ships OFF behind apex26.occlusionCull). |
| `occlusionCull(on)` | — |
...
### `multiDraw(on?) → {supported, on, multiCalls, rangesSubmitted, drawElementsAvoided}`
...
`false` off, no argument reports the counters. Also behind `apex26.multiDraw`,
and it ships OFF until an A/B on hardware prices it.

This is the lever the bound test pointed at: props uploaded as ONE mesh — every
vertex, no culling — measured FASTER than 152 culled draws, so draw calls are
what the frame pays for, not vertices
(`docs/notes/RENDER-PERF-PLAN-2026-09-16.md` §2).

`drawElementsAvoided` is the counted oracle. Multi-draw also removes the
contiguity constraint the run-merge was built around: `drawElements` can only
merge chunks adjacent in the index buffer, while a multi-draw group survives
gaps, so chunks sharing a light set now travel together even when culled chunks
sit between them.
