...
_Generated by `node tools/gen/gen-hooks-table.mjs` from `const api = {…}` in `js/agent/apex.js` (214 hooks, 4 async) and `agentHelp()` in `js/agent/agentview.js`. Signatures are as written in source; the summary is the comment directly above each hook. The sections below this index are the hand-written reference and are required for every hook (`tests/unit/hooks-documented.test.mjs`); `--check` guards this block (`tests/unit/generated-docs.test.mjs`)._
...
| `flybyCam(u, shots)` | park the camera at progress `u` (0..1) through the PRE-RACE FLYBY's shot sequence (js/camera/flyby-seq.js) and report where it put the eye, what… |
| `flybyShots()` | the sequence the flyby is currently playing, as data. |
...
| `camState()` | `lens` is what the LAST FRAME was actually built with — near, far, the post-cap vertical FOV, the fog multiplier, the cull radius — as opposed to… |
| `garageCam()` | — |
...
| `garageStep(dt, n)` | — |
| `garageTeam(id, seatWanted)` | Switch garage car without opening #teampicker — same store writes as the tile click in select-screen.js (G.teamIdx, buildSetup, tickUi). |
| `garageFrame(view, opts)` | One-shot preset + framing — mirrors #cs-stack / cs-view-* / cs-pan-*: opts.target [x,y,z] car-space look-at, lamp (preset name \| null), az/el/dist… |
| `garageParts(parts)` | Merge catalog part ids into the current team's garage setup and rebuild. |
| `meshToggle(o)` | Debug: hide/show individual track meshes. |
| `nodesNear(wx, wz, r)` | — |
| `nodeAt(frac, opts)` | World position and orientation of a track node by fraction (0-1). |
...
| `tuning()` | — |
| `physState()` | — |
| `pit(arg)` | The pit lane (js/race/pit-lane.js): the resolved geometry, the limit, and this car's state in it. |
| `pitSigns()` | The bay signs (js/garage/pit-signs.js): cells laid out on the row, whether the atlas and the texMesh uploaded, frames drawn and frames asked. |
| `tyres(arg)` | The whole tyre picture for one car (default: the player) — compound, life in laps at THIS race distance, wear, the grip it costs and the load that… |
| `wallStats()` | — |
| `modelDiagnostics()` | — |
| `geometryDiagnostics()` | — |
| `trackGeometry(keep)` | — |
...
| `freeCam(opts)` | the FREE CAMERA panel: none = state, {eye,target,fov,roll,lens,speed} = place, {snap:"car"\|"corner",n} = snap, false = exit |
| `view(opts)` | — |
...
| `tourShots(n = 12, opts = {})` | — |
| `groundY(f, lat = 0)` | `overRoad` is the number you want; `gap` is the trap it replaces — inside a bankZone the tarmac is not the centreline plane (Lesmo 1 lifts the… |
...
| `aim(relDeg)` | — |
| `go()` | — |
| `carState()` | EVERY numeric/boolean field on every car, raw and unrounded — the whole per-car state, not the curated slice cars() returns. |
| `cars()` | — |
| `buildProfile()` | The track build TIMELINE (js/track/tracks.js, "BUILD PROFILE"): one row per phase in build order, `k` "geo" for emission and "up" for upload. |
| `raceProfile()` | The RACE-ENTRY timeline (js/game.js, "RACE-ENTRY PROFILE"): one row per phase of startRace, the build being only one of them. |
| `multiDraw(on)` | WEBGL_multi_draw (GLX only, apex26.multiDraw). |
| `occlusionCull(on)` | Occlusion culling (GLX only, ships OFF behind apex26.occlusionCull). |
| `corners()` | Lap fractions of curvature-peak apexes (local maxima of \|curvature\|). |
| `race(trackRef, timeOfDay, weather, opts)` | Load any circuit (by index or id, e.g. |
...
| `careerFacility(up)` | — |
| `careerAmbition(i)` | The contract's season target as a RUNG the player picks, not a number the game chose. |
| `careerEra()` | Which regulation era the loaded career is in, and what it outlaws. |
| `careerHire(what)` | — |
...
| `careerRollover()` | — |
| `reliability(level)` | — |
| `drivingLine(mode)` | The DRIVING LINE ribbon (RACE SETTINGS): get/set "off" \| "corner" \| "full" for this session (the RACE SETTINGS row is what persists, like every… |
| `retirements()` | — |
| `retire(idx, reason)` | — |
| `qualiSim(playerTime)` | — |
| `ratings(code)` | — |
...
| `lightState()` | — |
| `gpuTimer(on)` | GPU frame-time probe: availability follows the bound backend. |
...
| `govHold(on)` | hold the governor's feature ladder at the tier it is on now (no shed, no restore). |
...
| `async awaitPresent(timeoutMs)` | wait for the next composited frame before a screenshot tool reads the canvas. |
...
| `settingsFile(mode)` | the SETTINGS FILE object (js/ui/settings-export.js): "changes" (default) = every preference that differs from its shipped default, with the… |
| `garageFile()` | the GARAGE file object (js/ui/settings-export.js): parts, liveries, setup sheets and an invented team, for every team, enumerated by prefix from… |
...
| perceive | `awaitPresent(timeoutMs)?` | WAIT FOR THE PIXELS — resolve once the next real frame has reached the screen; a screenshot tool taken before this resolves can read a stale or black canvas under HeadlessChrome |
...
| know | `spatialUpscale(0\|1)` | GLX + WGX + TLX SGSR1 (flag OFF by default). active only when on AND renderScale<~1; see UPSCALING-2026-09 §6–7 |
...
### `race(trackRef, timeOfDay?, weather?, opts?) → {track, timeOfDay, weather} | false`
...
the same `dbgCam` override photo mode uses.
```js
__apex.flybyCam(0);            // the first frame of the first shot
__apex.flybyCam(0.5).inside;   // null, or the building the camera is in
const shots = __apex.flybyShots();
shots[0].eye[1].y = 30;        // raise the crane
...
frame's projection was actually built with: `{near, far, fovY, fog, cull, cine}`
(`fovY` in radians, after the wide-screen FOV cap; `fog` is the multiplier on the
session's fog density, `null` when unscaled; `cine` is true for the pre-race
flyby and for the FLYBY SHOT EDITOR's preview of it).

It exists because those are different questions and the gap between them hid a
defect for months: the loading screen's flyby and the editor's preview of the
same shot aimed the same camera through two different lenses — far plane 900 m
vs 6000 m, fog ×1.0 vs ×0.15 — which reads as "why does the loading flyby look
way more foggy than what's shown in the editor" and is invisible to any hook
that only reports a position. The two now render `cine` frames identically, and
this is how that is checked rather than asserted.
```js
__apex.flybyCam(0.3).eye;      // aim the editor's preview at a shot
__apex.camState().lens;        // { near: 0.9, far: 6000, fog: 0.15, cine: true, … }
```
...
| Call | Effect |
|---|---|
| `freeCam()` | state only: `open`, `eye`/`target`/`fov` of the live `dbgCam`, `roll` (degrees), `lens` (`"race"`/`"flyby"`), `speed` (m/s), last `corner`, nearest flyby `anchor` (e.g. `"T3 +12 m, +30 m out"`), `poseErr` from the last COPY FLYBY POSE |
| `freeCam(true)` | open, starting at the current game camera |
| `freeCam({ eye, target, fov })` | place (world metres; `fov` clamps to 20–110) |
| `freeCam({ roll, lens, speed })` | `roll` in degrees, clamped to ±45 (Q/E in the panel); `lens:"flyby"` renders through `FlybySeq.FAR/FOG/NEAR` (`dbgCam.cine`), `"race"` the photo-mode far plane and full fog; `speed` 5–200 m/s |
| `freeCam({ snap: "car" })` | behind the player's car, along its heading |
| `freeCam({ snap: "corner", n })` | corner `n` (1-based, wraps) seen from its outside, just before the apex; no `n` steps to the next |
| `freeCam(false)` | close: the game camera comes back, level, race lens (and the settings page, if paused) |

The panel's COPY VIEW writes the `__apex.view({eye,target,fov})` line for this
frame (roll and lens are not part of `view()`), and COPY FLYBY POSE writes
`FlybySeq.shotFromView()`'s held one-shot entry, reporting its round-trip error.
The live camera takes the paused-render path, so drive it from a paused race.

```js
// in a race, paused (Escape) so the held frame keeps rendering
__apex.freeCam({ snap: "corner", n: 3 });           // T3 from the outside
__apex.freeCam({ roll: -12, lens: "flyby" });       // a flyby-lens dutch angle
__apex.freeCam();                                    // → {open:true, anchor:"T3 -18 m, +30 m out", …}
__apex.freeCam(false);                              // back to the race camera
```
...
### `texCensus() → {bytes, count, mb, byKind, biggest, excludes}`
**RESIDENT** texture bytes from the active backend — what is on the GPU now,
not what was ever created. GLX only; WGX and TLX declare it absent, so this
answers `{supported:false, bytes:null}` there rather than a measured zero.

Read it on a full grid before touching any texture resolution policy. It exists
because the biggest texture number in the game was arithmetic until 2026-09-14:
the livery atlases were ESTIMATED at ~147 MB, about ten times the packed world
geometry, and nothing could confirm it. Measured, montreal, full grid:

```js
__apex.texCensus()
// { mb: 158.00, count: 24,
//   byKind: { content2D: 153791000,      // 146.67 MB — 22 livery atlases
//             materialArray: 11883816 }, // 11.33 MB — baked albedo + normal
//   biggest: { kind: "content2D", w: 1024, h: 1280, layers: 1, bytes: 6990500 },
//   excludes: ["shadow maps", "post chain", "env cube", "non-GLX backends"] }
```

`bytes` is the EXACT mip chain, not `w*h*4*4/3` — that rule assumes a square
texture, and the livery atlas is 1024×1280. `excludes` is load-bearing, not
decoration: render targets are created straight against `gl` in `shadow.js` /
`post.js` and never reach this ledger, so a total here is never the whole
picture. A lost context reports nothing resident (plus `contextLost:true`)
rather than the last number it held.

Analysis and the plan built on it: `notes/PERF-FINDINGS.md` §2v,
`plans/2026-09-14-texture-memory-plan.md`.
...
### `settingsFile(mode?) → {format, mode, changed, settings, defaults, where, …}`
The SETTINGS FILE as an object (`js/ui/settings-export.js`, the same thing the
SAVE CHANGED / SAVE ALL SETTINGS buttons under DISPLAY › RENDERER download).
`"changes"` (default) lists every preference that differs from its shipped
default — `settings` grouped as SETTINGS is (audio, display, hud, camera,
lighting, driving, steering, controls, metrics), `defaults` the value each one
replaced, `where` the source file that owns that default; `"all"` lists every
preference's effective value. Only the allowlist is read: never the garage,
saves, accounts or the renderer's self-heal latches.

### `garageFile() → {format, exportedAt, build, count, garage}`
The GARAGE file as an object — the twin of `settingsFile()` and what SAVE
GARAGE FILE downloads. `garage` is keyed by the raw storage key after the
`apex26.` prefix (`"parts.<team>"`, `"livery.<team>"`, `"livery.custom.<team>"`,
`"setup.<team>"`, plus `customTeam`, `customLogo`, `team`, `driver`), and those
prefixes are the allowlist: this is the one place anything enumerates the
...
### `buildProfile() → [{n, k, ms}, …]`
The track build as a TIMELINE, one row per phase in build order: `n` the phase
(`centerline`, `pit`, `surface`, `floor`, `road`, `terrain`, `props`,
`propsSeal`, `batches`, `glassWater`, `trim`, `pitSigns`), `k` either `"geo"`
for geometry emission or `"up"` for the upload beside it, and `ms` the wall
time of that lap. `null` before a track has built.

It exists to answer one question and should be read for it: at race entry the
main thread blocks for 1.8-3.0 s on real hardware, and run 128 established that
the block is JavaScript rather than upload back-pressure — this says WHICH
JavaScript, which is what decides whether moving the build off the main thread
would move anything (`docs/notes/MULTITHREADING-PLAN-2026-09-16.md` §7).

Two cautions. The `up` rows are meaningless in the Node VM, where the renderer
is a stub and an upload is a no-op; only a browser run measures them. And the
laps are WALL TIME on a sequential build, so a row includes anything the
browser did in that span — it is attribution, not a profiler.

### `raceProfile() → [{n, ms}, …]`
The RACE-ENTRY timeline, one row per phase of `startRace()`: `scenery`
(the lazy circuit-module load), `resets`, `loadTrack` (which contains the whole
track build), `makeCars`, `settings`, `gridUp`, `finish`.

`buildProfile()` says which part of the build costs; this says whether the
build is the part of race entry that costs at all. On real hardware it is 23 %
of the main-thread block on the default backend, so the rest of these rows are
where the freeze actually lives
(`docs/notes/MULTITHREADING-PLAN-2026-09-16.md` §8).

Wall-time laps on a sequential path, with the same caution as `buildProfile()`:
a row includes whatever else the browser did in that span. In the Node VM
`loadTrack` is essentially the whole thing, because the VM has no renderer —
that is the harness, not the game, and it is exactly why this needs a browser.

### `multiDraw(on?) → {supported, on, multiCalls, rangesSubmitted, drawElementsAvoided, perChunkDraws, consecutiveGroups, setGroups}`
GLX `WEBGL_multi_draw` — the per-chunk `drawElements` calls collapsed into one
`multiDrawElementsWEBGL` per light-set group. `multiDraw(true)` turns it on,
`false` off, no argument reports the counters (and `false` also zeroes them,
which is how you take a per-frame reading). Also behind `apex26.multiDraw`, and
it ships OFF.

It is OFF for a reason the counters give plainly. The last three are the
grouping oracle — visible chunks in the lamp branch, the groups a NEIGHBOUR
comparison makes of them, and the groups their light SETS make. At vegas with
the clock held at 02:00 that reads **108.5 → 24.5 → 15.0 per frame**: bucketing
by set is worth about nine and a half draw calls a frame, which is real and is
not a frame time (`docs/notes/RENDER-PERF-PLAN-2026-09-16.md` §7).

`drawElementsAvoided` is the other counted oracle. What multi-draw removes is
the contiguity constraint the run-merge was built around: `drawElements` can
merge only chunks adjacent in the index buffer, while a multi-draw group carries
ranges that need not be adjacent at all, so chunks sharing a light set travel
together however far apart they sit. A culled chunk between them is simply never
filed under the group, which is what keeps the cull intact — proved by running
both paths over a fixture with a hole in the middle of the chunk array
(`tests/unit/glx-multidraw.test.mjs`).

Absent extension (Firefox, at 1.4 % support) or flag off, every path is the
`drawElements` one it has always been.

### `occlusionCull(on?) → {supported, on, tested, culled, queries, passes}`
GLX occlusion culling — hardware `ANY_SAMPLES_PASSED_CONSERVATIVE` queries
against the finished opaque depth buffer, one proxy box per chunk, results read
a frame or more later so nothing blocks. `occlusionCull(true)` turns it on,
`false` off, no argument reports the counters. It also ships behind
`apex26.occlusionCull`, and it ships OFF.

The counters are a COUNTED oracle and that is deliberate: `tested` is chunks
that passed the frustum and radial culls, `culled` is those a query said
contribute no pixel, `queries` is boxes drawn this pass. Count the work avoided;
do not time it — this container has no GPU, so a frame rate measured here
measures the box (`docs/notes/CI-RENDERING-PERFORMANCE.md`).

Every uncertainty resolves to VISIBLE: no extension, no program, no query yet,
no result yet, or a chunk outside the frustum all leave the chunk drawn. The
failure mode is "no saving", never "a hole in the world".

### `carState() → [{…every numeric/boolean field…}, …]`
The WHOLE per-car state for every car, raw and unrounded, in `cars[]` order —
where `cars()` returns a curated slice and `carAt()` a curated slice of one car.
Strings, objects and functions are skipped: a car's team record, driver name and
launch plan are identity, not episode state.

It exists for one question: **did `reset()` leave anything behind?** An episode
creates per-car fields that outlive it unless reset() removes them, and several
of them feed the next tick's physics or the AI's racecraft, so a stale one makes
the same seed replay differently. Diff two post-reset snapshots to find them:

```js
__apex.reset(0.02, 55, 0, 42); const a = __apex.carState();
__apex.rollout({ seconds: 2, input: { throttle: true } });
__apex.reset(0.02, 55, 0, 42); const b = __apex.carState();
// any key that differs is leaking across episodes
```

`tests/specs/agent-determinism.spec.js` runs exactly that as a guard, so the
`EPISODE_TRANSIENTS` list in apex.js cannot silently fall behind the code.
...
`transformSceneryApi` wraps every node-taking helper with
`TrackSpace.sceneryNode`, so `anchor(K(0.45))` in `js/circuits/scenery/` and a
bare `nodeAt(0.45)` here land on different nodes on any circuit carrying
`_sceneryShift`. At Montreal that is node 337 against 489 — 152 nodes and about
600 m apart, which is how `montreal-foundation.spec.js` came to measure
floodlight masts for months and call them the casino footbridge's piers.
...
### `wallStats() → {minB, maxB, maxPitB, minOverHw, anyNaN, tightFrac, pitSides, street, n} | null`
...
`pitSides` and left out of `tightFrac` AND out of `maxB`, and reported on
their own as `maxPitB` (null when the track has no pit-owned side). Splitting
them out is why `maxB` is meaningful again: with the complex counted in, 44 of
the 52 circuits read maxB > 20 and every one was over solely because of the pit
side. For verifying every track keeps the car off the models and is recoverable.
...
### `garageTeam(id, seat?) → {ok, switched?, label?, seat?, error?}`
Switch the garage car by team id without opening `#teampicker`. Writes the same
store keys as a team tile click (`team`, `driver`), rebuilds setup, and returns
the uppercase `#cs-team` label. `{ok:false, error:"garage_closed"}` when the
preview is off; `{ok:false, error:"unknown_team"}` when `id` is not in
`Teams.LIST` (including `"custom"` after boot-time `syncCustomTeam()`).
An explicit `seat` (0 / 1) switches the driver — race number, helmet, T-cam —
on the same team too, and is echoed back; without one the first free seat is
taken. `garage-angles --driver=0,1` walks it as an axis.
...
| opt | meaning |
|---|---|
| `target` | `[x, y, z]` in car space: orbit about and look at that point (a preset resets it; `garage-angles --target=crown,fin,wall,…` names the points) |
| `lamp` | aim the inspection lamp by preset name, `null` parks it (a preset re-aims it, so this applies after the framing) |
...
| `inLane` | This car is SERVING a stop — a state, not a position |
...
```js
__apex.tyres({ level: "real" });
__apex.race("monza", null, null, { laps: 25 });
__apex.pit({ arm: true });                 // box this lap
__apex.step(1 / 60, 60 * 90);
__apex.pit();                               // {state: "out", stops: 1, …}
```

### `pitSigns() → {cells, mesh, tex, drawn, calls}`
The pit bay signs (`js/garage/pit-signs.js`): each team's crest, code and
name on the lintel of its bay, one atlas and one texMesh drawn as ONE decal
after the sky. `cells` is how many fascia quads `SceneryPits` laid out on the
row (`track.pitSigns`; 12 on a built complex, 0 on a painted street lane),
`mesh`/`tex` whether the texMesh and the atlas texture uploaded (they need a
canvas and `LiveryTex`; the headless VM builds have neither), `drawn` how many
frames the decal was drawn and `calls` how many frames asked — the draw
declines beyond 350 m of the row's centre (the decal shader has no fog) and
under `meshToggle({pitSigns: true})`.
```js
__apex.race("albert_park"); __apex.pitSigns();
...
`js/physics/tyre-model.js`; `physState()` carries the four most-used fields
(`tyreCompound`, `tyreWear`, `tyreGrip`, `tyreLoad`) so a driving probe does not
have to call both.

| Field | Meaning |
|---|---|
| `level` / `on` | The TYRE WEAR setting (`off` / `light` / `real`) and whether it is doing anything. **Ships `off`**, in which case every multiplier below is exactly 1 |
| `compound` / `code` | The fitted compound's id, and the single letter the HUD shows (`H`/`M`/`S`, or `I`/`W` for the wet treads) |
...
is the single most surprising thing about the model. Real degradation over a
25-lap stint accumulates ~1.5 s against a ~21 s pit loss, so a literal port of
...
survive the next `gridUp` (which re-reads the setting, not the model). It
returns the same record it would have returned anyway, so one call can set and
read:

```js
__apex.tyres({ level: "real" });          // arm it
__apex.race("monza", null, null, {laps: 25});
...
Holds the governor's FEATURE LADDER at the tier it is on now — no shed, no
restore — until `govHold(false)`. `renderScale(v)` does not do this: pinning the
scale leaves the ladder as the governor's only lever, so on a runner missing
frames it sheds FASTER after a pin (`js/perf/governor.js`, `_tierHold`). Any
measurement that diffs two captures of one scene (the image-grade tonal tests,
lighting-ab's fog glow) needs the same tier at both, or it reads a bloom/SSR
shed as the thing it changed (Metal run 3469: "shadows" darkened highlights by
44/255). Holds the tier, not the scale — pair it with `renderScale(1)` when the
capture size has to match too. No arg reports without changing anything.
```js
__apex.renderScale(1);     // same SIZE at every capture
__apex.govHold(true);      // same TIER at every capture
__apex.govHold();          // {tierHold:true, tier:0, autoTier:0, autoShed:0}
```
...
### `await __apex.awaitPresent(timeoutMs)?` — wait for the composited frame before a screenshot

Every renderer (GLX, TLX, WGX) blits its real frame onto an on-screen overlay
**on demand** rather than every tick, and does it under the same two names —
`awaitSoftPresent(timeoutMs)` / `softPresent()` — whichever backend is bound
(`game.js` copies the active backend's descriptors onto the `GLX` global, so
one call is right regardless of which renderer actually took the canvas). A
pixel-based capture tool (`take_screenshot`, `page.screenshot`, a canvas
`toDataURL`) taken before this resolves can read the PREVIOUS frame, or — under
HeadlessChrome, which hides the raw canvas and shows only the overlay — a
black one, because nothing has blitted yet. `awaitPresent()` is the `__apex`
front door for that wait so a capture tool never has to know `GLX` is a bare
lexical global (not `window.GLX`) or that the method lives on it and not on
`__apex` itself. Resolves immediately (no-op) outside HeadlessChrome, where
the canvas composites for real without a soft-present pass. Call it after ANY
change that should show up on screen — a `jump()`/`park()`, a camera move, or
a UI click (a Garage livery swap, a menu navigation) — not only after driving
hooks. See `.claude/skills/mcp-probe/references/traps-chrome.md` for the
measured black-canvas and UI-interaction traps this replaces the manual
`GLX.awaitSoftPresent()` dance for.
...
### `careerEra() → {id, name, cats, left, seasons, banned, cap} | null`
Which **regulation era** the loaded career is in. An era makes the dearest
options in three catalog categories illegal for four seasons — grid-wide, so
every AI factory build re-resolves through the same filter. `null` outside a
career: a Grand Prix and a standalone Season are never regulated.

`banned` is the real option ids, derived from `Parts.CATALOG` rather than listed,
so repricing moves the ban. `cap` is `budgetCap()` under the current ruleset —
it falls during an era, because the top shelf it is computed from is smaller.
Nothing is confiscated: `career.owned` is untouched and a banned part becomes
legal again when the era lapses.

```js
__apex.careerEra();   // → { id: "open", name: "OPEN REGULATIONS", cats: [],
                      //     left: 4, seasons: 0, banned: [], cap: 2105 }
__apex.careerSim(24); __apex.careerRollover();   // ...four seasons on
__apex.careerEra();   // → { id: "powertrain", cats: ["engine","ers","fuel"], … }
```

### `careerAmbition(i?) → {pick, signed, target, rungs}`
The contract's season target as a **rung the player picks**, not a number the
game chose. No argument reads; an index (0 `modest` / 1 `expected` /
2 `ambitious`) sets what the **next** contract signs at and re-stamps the target
on every offer still on the table.

Two fields because they are two different things: `pick` is pending, `signed` is
what the contract currently running was signed at. `rollover()` prices the season
by `signed`, so moving the picker in March cannot re-price a deal already under
way. Rung 1 is what every contract used to be, so a save written before this
existed reads as `expected` and resolves identically.

```js
__apex.careerAmbition();       // → { pick: 1, signed: 1, target: 19, rungs: [...] }
__apex.careerAmbition(2);      // promise more: target moves, signed does not
                               // → { pick: 2, signed: 1, target: 19, rungs: [...] }
```
...
### `drivingLine(mode?) → { mode, built, samples, verts, speedAt(s), zoneAt(s) }`
The glowing suggested line on the road (RACE SETTINGS › DRIVING LINE,
`js/render/shared/driving-line.js`). Get, or set for this session,
`"off"` / `"corner"` / `"full"` (the RACE SETTINGS row is what persists, as
`apex26.drivingLine`). `built` is the circuit id the strip was last
built for (null until the first frame drew it), `samples` the 2.5 m samples
...
__apex.step(60); __apex.drivingLine().speedAt(600)   // the line's speed 600 m in
```
...
(`tests/unit/scenery-api-contract.test.mjs`, which freezes a 114-member API) run
