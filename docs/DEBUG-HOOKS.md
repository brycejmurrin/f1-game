...
| `garageStep(dt, n)` | — |
| `garageTeam(id)` | Switch garage car without opening #teampicker — same store writes as the tile click in select-screen.js (G.teamIdx, buildSetup, tickUi). |
| `garageFrame(view, opts)` | One-shot preset + discrete zoom/pan clicks — mirrors #cs-stack / cs-pan-*. |
...
### `garageTeam(id) → {ok, switched?, label?, error?}`
...
### `garageFrame(view, opts?) → {ok, az, el, dist, pan?, error?}`
One-shot camera preset plus discrete zoom/pan clicks — mirrors `#cs-stack` and
`#cs-pan-*`. `opts.zoom`, `opts.strafe`, and `opts.dolly` are counted nudges
(positive zoom = in, positive strafe = right, positive dolly = forward). Returns
`garageCam()` after framing. Used by multi-team `garage-angles` surveys instead
of DOM clicks (~15 s per team saved).
