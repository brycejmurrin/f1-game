...
| `camTune(mode, obj)` | the CAMERA TUNER's per-camera-mode framing offsets (js/game/cam-tune.js), the camera counterpart of lightTune(). |
...
reach 4 on a low preset. See `js/game/perf.js` `_autoShed`.
