...
| `garageFrame(view, opts)` | One-shot preset + zoom/pan/az/el framing — mirrors #cs-stack / cs-view-* / cs-pan-*. |
...
One-shot camera preset plus discrete framing clicks — mirrors `#cs-stack`,
`#cs-view-*`, and `#cs-pan-*`.
...
Returns `garageCam()` after framing. Used by multi-team `garage-angles` surveys
instead of DOM clicks (~15 s per team saved). Named presets include `bay`
(angled left three-quarter with the garage back wall in frame).
