...
## 0. The suite is not green — 12 pre-existing failures
...
So **97 Playwright specs and 32 Node suites gate nothing** on push or PR. Every
...
## 3. Skips that swallow the regression the test exists for
...
## 5. `tools/ssr-probe.mjs` — two dead anchors
...
## 12. `css/career.css` closes its cascade layer 199 lines early

`:1` opens `@layer components {`; brace-matching (comments stripped) puts the
close at **line 206 of 405**. Lines 207-405 are therefore **unlayered**, and
unlayered rules beat all layered rules regardless of specificity — so
`.cr-slot*`, `#career-guide`, `.cg-card`, `.res-settle*`, `.cr-ladder*` and
`.cr-cheats` now outrank `@layer base` and `@layer overlays`. Nothing visibly
breaks today (no property collisions), but any future responsive override of
those selectors will silently do nothing. Every other sheet closes its layer and
marks it with a trailing comment; career.css is the only one without.
